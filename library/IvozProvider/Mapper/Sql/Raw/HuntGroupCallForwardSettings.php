...
class HuntGroupCallForwardSettings extends MapperAbstract
{
    protected $_modelName = 'IvozProvider\\Model\\HuntGroupCallForwardSettings';
     ...
     *
     * @param IvozProvider\Model\Raw\HuntGroupCallForwardSettings $model
     * @return array
     */
    public function toArray($model, $fields = array())
    {

        if (!$model instanceof \IvozProvider\Model\Raw\HuntGroupCallForwardSettings) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \IvozProvider\Model\Raw\HuntGroupCallForwardSettings object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\IvozProvider\Model\\HuntGroupCallForwardSettings object in toArray for " . get_class($this);
            }
        ...
        if (empty($fields)) {
            $result = array(
                ...
                'callTargetType' => $model->getCallTargetType(),
                'callNumberValue' => $model->getCallNumberValue(),
                'callExtensionId' => $model->getCallExtensionId(),
                'callVoiceMailUserId' => $model->getCallVoiceMailUserId(),
            );
        } else {
            $result = array();
     ...
     *
     * @return IvozProvider\\Mapper\\Sql\\DbTable\\HuntGroupCallForwardSettings
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('IvozProvider\\Mapper\\Sql\\DbTable\\HuntGroupCallForwardSettings');
        }
     ...
     *
     * @param IvozProvider\Model\Raw\HuntGroupCallForwardSettings $model The model to delete
    ...
    public function delete(\IvozProvider\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \IvozProvider\Model\Raw\HuntGroupCallForwardSettings) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\IvozProvider\\Model\\HuntGroupCallForwardSettings object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\IvozProvider\\Model\\HuntGroupCallForwardSettings object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('IvozProvider\\Mapper\\Sql\\DbTable\\HuntGroupCallForwardSettings', $capitalizedFk);
     ...
     */
    public function save(\IvozProvider\Model\Raw\HuntGroupCallForwardSettings $model, $forceInsert = false)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\IvozProvider\Model\Raw\HuntGroupCallForwardSettings $model, $useTransaction = true,
            $transactionTag = null, $forceInsert = false)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag, $forceInsert);
    }

    protected function _save(\IvozProvider\Model\Raw\HuntGroupCallForwardSettings $model,
     ...
     * @param IvozProvider\Model\Raw\HuntGroupCallForwardSettings|null $entry The object to load the data into, or null to have one created
     * @return IvozProvider\Model\Raw\HuntGroupCallForwardSettings The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \IvozProvider\Model\HuntGroupCallForwardSettings();
        }
        ...
        if (is_array($data)) {
            $entry->setId($data['id'])
                  ...
                  ->setCallTargetType($data['callTargetType'])
                  ->setCallNumberValue($data['callNumberValue'])
                  ->setCallExtensionId($data['callExtensionId'])
                  ->setCallVoiceMailUserId($data['callVoiceMailUserId']);
        ...
        } else if ($data instanceof \IvozProvider\Model\Raw\HuntGroupCallForwardSettings) {
            $entry->setId($data->getId())
                  ->setHuntGroupId($data->getHuntGroupId())
                  ->setCallTypeFilter($data->getCallTypeFilter())
                  ->setCallTargetType($data->getCallTargetType())
                  ->setCallNumberValue($data->getCallNumberValue())
                  ->setCallExtensionId($data->getCallExtensionId())
                  ->setCallVoiceMailUserId($data->getCallVoiceMailUserId());
    ...
    protected function _etagChange()
    {
        ...
        $etag = $etags->findOneByField('table', 'HuntGroupCallForwardSettings');

        if (empty($etag)) {
            $etag = new \IvozProvider\Model\EtagVersions();
            $etag->setTable('HuntGroupCallForwardSettings');
        }
