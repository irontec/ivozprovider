...
/**
 * Data Mapper implementation for IvozProvider\Model\HuntGroupsRelUsers
 *
 * @package IvozProvider\Mapper\Sql
 * @subpackage Raw
 * @author Luis Felipe Garcia
...
class HuntGroupsRelUsers extends MapperAbstract
{
    protected $_modelName = 'IvozProvider\\Model\\HuntGroupsRelUsers';
     ...
     *
     * @param IvozProvider\Model\Raw\HuntGroupsRelUsers $model
     * @return array
     */
    public function toArray($model, $fields = array())
    {

        if (!$model instanceof \IvozProvider\Model\Raw\HuntGroupsRelUsers) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \IvozProvider\Model\Raw\HuntGroupsRelUsers object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\IvozProvider\Model\\HuntGroupsRelUsers object in toArray for " . get_class($this);
            }
        ...
        if (empty($fields)) {
            $result = array(
                ...
                'timeoutTime' => $model->getTimeoutTime(),
     ...
     *
     * @return IvozProvider\\Mapper\\Sql\\DbTable\\HuntGroupsRelUsers
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('IvozProvider\\Mapper\\Sql\\DbTable\\HuntGroupsRelUsers');
        }
     ...
     *
     * @param IvozProvider\Model\Raw\HuntGroupsRelUsers $model The model to delete
    ...
    public function delete(\IvozProvider\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \IvozProvider\Model\Raw\HuntGroupsRelUsers) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\IvozProvider\\Model\\HuntGroupsRelUsers object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\IvozProvider\\Model\\HuntGroupsRelUsers object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('IvozProvider\\Mapper\\Sql\\DbTable\\HuntGroupsRelUsers', $capitalizedFk);
     ...
     */
    public function save(\IvozProvider\Model\Raw\HuntGroupsRelUsers $model, $forceInsert = false)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\IvozProvider\Model\Raw\HuntGroupsRelUsers $model, $useTransaction = true,
            $transactionTag = null, $forceInsert = false)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag, $forceInsert);
    }

    protected function _save(\IvozProvider\Model\Raw\HuntGroupsRelUsers $model,
     ...
     * @param IvozProvider\Model\Raw\HuntGroupsRelUsers|null $entry The object to load the data into, or null to have one created
     * @return IvozProvider\Model\Raw\HuntGroupsRelUsers The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \IvozProvider\Model\HuntGroupsRelUsers();
        }
        ...
        if (is_array($data)) {
            $entry->setId($data['id'])
                  ...
                  ->setTimeoutTime($data['timeoutTime'])
        ...
        } else if ($data instanceof \IvozProvider\Model\Raw\HuntGroupsRelUsers) {
            $entry->setId($data->getId())
                  ->setHuntGroupId($data->getHuntGroupId())
                  ->setUserId($data->getUserId())
                  ->setTimeoutTime($data->getTimeoutTime())
    ...
    protected function _etagChange()
    {
        ...
        $etag = $etags->findOneByField('table', 'HuntGroupsRelUsers');

        if (empty($etag)) {
            $etag = new \IvozProvider\Model\EtagVersions();
            $etag->setTable('HuntGroupsRelUsers');
        }
