...
class QueueMembers extends MapperAbstract
{
    protected $_modelName = 'IvozProvider\\Model\\QueueMembers';
     ...
     *
     * @param IvozProvider\Model\Raw\QueueMembers $model
     * @return array
     */
    public function toArray($model, $fields = array())
    {

        if (!$model instanceof \IvozProvider\Model\Raw\QueueMembers) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \IvozProvider\Model\Raw\QueueMembers object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\IvozProvider\Model\\QueueMembers object in toArray for " . get_class($this);
            }
     ...
     *
     * @return IvozProvider\\Mapper\\Sql\\DbTable\\QueueMembers
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('IvozProvider\\Mapper\\Sql\\DbTable\\QueueMembers');
        }
     ...
     *
     * @param IvozProvider\Model\Raw\QueueMembers $model The model to delete
    ...
    public function delete(\IvozProvider\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \IvozProvider\Model\Raw\QueueMembers) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\IvozProvider\\Model\\QueueMembers object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\IvozProvider\\Model\\QueueMembers object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('IvozProvider\\Mapper\\Sql\\DbTable\\QueueMembers', $capitalizedFk);
     ...
     */
    public function save(\IvozProvider\Model\Raw\QueueMembers $model, $forceInsert = false)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\IvozProvider\Model\Raw\QueueMembers $model, $useTransaction = true,
    ...
    )
    {
        ...
        try {
            ...
            if ($recursive) {
                if ($model->getAstQueueMembers(null, null, true) !== null) {
                    $astQueueMembers = $model->getAstQueueMembers();

                    if (!is_array($astQueueMembers)) {

                        $astQueueMembers = array($astQueueMembers);
                    }

                    foreach ($astQueueMembers as $value) {
                        $value->setQueueMemberId($primaryKey)
     ...
     * @param IvozProvider\Model\Raw\QueueMembers|null $entry The object to load the data into, or null to have one created
     * @return IvozProvider\Model\Raw\QueueMembers The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \IvozProvider\Model\QueueMembers();
        }
        ...
        } else if ($data instanceof \IvozProvider\Model\Raw\QueueMembers) {
    ...
    protected function _etagChange()
    {
        ...
        $etag = $etags->findOneByField('table', 'QueueMembers');

        if (empty($etag)) {
            $etag = new \IvozProvider\Model\EtagVersions();
            $etag->setTable('QueueMembers');
        }
