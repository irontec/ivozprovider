...
/**
 * Data Mapper implementation for IvozProvider\Model\BrandURLs
 *
 * @package IvozProvider\Mapper\Sql
 * @subpackage Raw
 * @author Luis Felipe Garcia
...
class BrandURLs extends MapperAbstract
{
    protected $_modelName = 'IvozProvider\\Model\\BrandURLs';
     ...
     *
     * @param IvozProvider\Model\Raw\BrandURLs $model
     * @return array
     */
    public function toArray($model, $fields = array())
    {

        if (!$model instanceof \IvozProvider\Model\Raw\BrandURLs) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \IvozProvider\Model\Raw\BrandURLs object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\IvozProvider\Model\\BrandURLs object in toArray for " . get_class($this);
            }
        ...
        if (empty($fields)) {
            $result = array(
                ...
                'url' => $model->getUrl(),
                'klearTheme' => $model->getKlearTheme(),
     ...
     *
     * @return IvozProvider\\Mapper\\Sql\\DbTable\\BrandURLs
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('IvozProvider\\Mapper\\Sql\\DbTable\\BrandURLs');
        }
     ...
     *
     * @param IvozProvider\Model\Raw\BrandURLs $model The model to delete
    ...
    public function delete(\IvozProvider\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \IvozProvider\Model\Raw\BrandURLs) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\IvozProvider\\Model\\BrandURLs object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\IvozProvider\\Model\\BrandURLs object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('IvozProvider\\Mapper\\Sql\\DbTable\\BrandURLs', $capitalizedFk);
     ...
     */
    public function save(\IvozProvider\Model\Raw\BrandURLs $model, $forceInsert = false)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\IvozProvider\Model\Raw\BrandURLs $model, $useTransaction = true,
            $transactionTag = null, $forceInsert = false)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag, $forceInsert);
    }

    protected function _save(\IvozProvider\Model\Raw\BrandURLs $model,
     ...
     * @param IvozProvider\Model\Raw\BrandURLs|null $entry The object to load the data into, or null to have one created
     * @return IvozProvider\Model\Raw\BrandURLs The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \IvozProvider\Model\BrandURLs();
        }
        ...
        } else if ($data instanceof \IvozProvider\Model\Raw\BrandURLs) {
            $entry->setId($data->getId())
                  ->setBrandId($data->getBrandId())
                  ->setUrl($data->getUrl())
                  ->setKlearTheme($data->getKlearTheme())
                  ->setUrlType($data->getUrlType())
                  ->setName($data->getName())
                  ->setLogoFileSize($data->getLogoFileSize())
                  ->setLogoMimeType($data->getLogoMimeType())
                  ->setLogoBaseName($data->getLogoBaseName())
                  ->setUserTheme($data->getUserTheme());
    ...
    protected function _etagChange()
    {
        ...
        $etag = $etags->findOneByField('table', 'BrandURLs');

        if (empty($etag)) {
            $etag = new \IvozProvider\Model\EtagVersions();
            $etag->setTable('BrandURLs');
        }
