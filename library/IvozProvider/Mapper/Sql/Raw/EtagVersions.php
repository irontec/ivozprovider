...
/**
 * Data Mapper implementation for IvozProvider\Model\EtagVersions
 *
 * @package IvozProvider\Mapper\Sql
 * @subpackage Raw
 * @author Luis Felipe Garcia
...
{
    protected $_modelName = 'IvozProvider\\Model\\EtagVersions';
     ...
     *
     * @param IvozProvider\Model\Raw\EtagVersions $model
     * @return array
     */
    public function toArray($model, $fields = array())
    {

        if (!$model instanceof \IvozProvider\Model\Raw\EtagVersions) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \IvozProvider\Model\Raw\EtagVersions object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\IvozProvider\Model\\EtagVersions object in toArray for " . get_class($this);
            }
     ...
     *
     * @return IvozProvider\\Mapper\\Sql\\DbTable\\EtagVersions
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('IvozProvider\\Mapper\\Sql\\DbTable\\EtagVersions');
        }
     ...
     *
     * @param IvozProvider\Model\Raw\EtagVersions $model The model to delete
    ...
    public function delete(\IvozProvider\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \IvozProvider\Model\Raw\EtagVersions) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\IvozProvider\\Model\\EtagVersions object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\IvozProvider\\Model\\EtagVersions object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('IvozProvider\\Mapper\\Sql\\DbTable\\EtagVersions', $capitalizedFk);
     ...
     */
    public function save(\IvozProvider\Model\Raw\EtagVersions $model, $forceInsert = false)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\IvozProvider\Model\Raw\EtagVersions $model, $useTransaction = true,
            $transactionTag = null, $forceInsert = false)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag, $forceInsert);
    }

    protected function _save(\IvozProvider\Model\Raw\EtagVersions $model,
     ...
     * @param IvozProvider\Model\Raw\EtagVersions|null $entry The object to load the data into, or null to have one created
     * @return IvozProvider\Model\Raw\EtagVersions The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \IvozProvider\Model\EtagVersions();
        }
        ...
        } else if ($data instanceof \IvozProvider\Model\Raw\EtagVersions) {
