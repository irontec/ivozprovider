...
/**
 * Data Mapper implementation for IvozProvider\Model\ConditionalRoutesConditionsRelMatchLists
 *
 * @package IvozProvider\Mapper\Sql
 * @subpackage Raw
 * @author Luis Felipe Garcia
...
class ConditionalRoutesConditionsRelMatchLists extends MapperAbstract
{
    protected $_modelName = 'IvozProvider\\Model\\ConditionalRoutesConditionsRelMatchLists';
     ...
     *
     * @param IvozProvider\Model\Raw\ConditionalRoutesConditionsRelMatchLists $model
     * @return array
     */
    public function toArray($model, $fields = array())
    {

        if (!$model instanceof \IvozProvider\Model\Raw\ConditionalRoutesConditionsRelMatchLists) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \IvozProvider\Model\Raw\ConditionalRoutesConditionsRelMatchLists object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\IvozProvider\Model\\ConditionalRoutesConditionsRelMatchLists object in toArray for " . get_class($this);
            }
     ...
     *
     * @return IvozProvider\\Mapper\\Sql\\DbTable\\ConditionalRoutesConditionsRelMatchLists
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('IvozProvider\\Mapper\\Sql\\DbTable\\ConditionalRoutesConditionsRelMatchLists');
        }
     ...
     *
     * @param IvozProvider\Model\Raw\ConditionalRoutesConditionsRelMatchLists $model The model to delete
    ...
    public function delete(\IvozProvider\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \IvozProvider\Model\Raw\ConditionalRoutesConditionsRelMatchLists) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\IvozProvider\\Model\\ConditionalRoutesConditionsRelMatchLists object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\IvozProvider\\Model\\ConditionalRoutesConditionsRelMatchLists object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('IvozProvider\\Mapper\\Sql\\DbTable\\ConditionalRoutesConditionsRelMatchLists', $capitalizedFk);
     ...
     */
    public function save(\IvozProvider\Model\Raw\ConditionalRoutesConditionsRelMatchLists $model, $forceInsert = false)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\IvozProvider\Model\Raw\ConditionalRoutesConditionsRelMatchLists $model, $useTransaction = true,
            $transactionTag = null, $forceInsert = false)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag, $forceInsert);
    }

    protected function _save(\IvozProvider\Model\Raw\ConditionalRoutesConditionsRelMatchLists $model,
     ...
     * @param IvozProvider\Model\Raw\ConditionalRoutesConditionsRelMatchLists|null $entry The object to load the data into, or null to have one created
     * @return IvozProvider\Model\Raw\ConditionalRoutesConditionsRelMatchLists The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \IvozProvider\Model\ConditionalRoutesConditionsRelMatchLists();
        }
        ...
        } else if ($data instanceof \IvozProvider\Model\Raw\ConditionalRoutesConditionsRelMatchLists) {
    ...
    protected function _etagChange()
    {
        ...
        $etag = $etags->findOneByField('table', 'ConditionalRoutesConditionsRelMatchLists');

        if (empty($etag)) {
            $etag = new \IvozProvider\Model\EtagVersions();
            $etag->setTable('ConditionalRoutesConditionsRelMatchLists');
        }
