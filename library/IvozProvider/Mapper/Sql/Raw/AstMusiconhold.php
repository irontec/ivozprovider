...
/**
 * Data Mapper implementation for IvozProvider\Model\AstMusiconhold
 *
 * @package IvozProvider\Mapper\Sql
 * @subpackage Raw
 * @author Luis Felipe Garcia
...
{
    protected $_modelName = 'IvozProvider\\Model\\AstMusiconhold';
     ...
     *
     * @param IvozProvider\Model\Raw\AstMusiconhold $model
     * @return array
     */
    public function toArray($model, $fields = array())
    {

        if (!$model instanceof \IvozProvider\Model\Raw\AstMusiconhold) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \IvozProvider\Model\Raw\AstMusiconhold object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\IvozProvider\Model\\AstMusiconhold object in toArray for " . get_class($this);
            }
     ...
     *
     * @return IvozProvider\\Mapper\\Sql\\DbTable\\AstMusiconhold
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('IvozProvider\\Mapper\\Sql\\DbTable\\AstMusiconhold');
        }
     ...
     *
     * @param IvozProvider\Model\Raw\AstMusiconhold $model The model to delete
    ...
    public function delete(\IvozProvider\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \IvozProvider\Model\Raw\AstMusiconhold) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\IvozProvider\\Model\\AstMusiconhold object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\IvozProvider\\Model\\AstMusiconhold object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('IvozProvider\\Mapper\\Sql\\DbTable\\AstMusiconhold', $capitalizedFk);
     ...
     */
    public function save(\IvozProvider\Model\Raw\AstMusiconhold $model, $forceInsert = false)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\IvozProvider\Model\Raw\AstMusiconhold $model, $useTransaction = true,
            $transactionTag = null, $forceInsert = false)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag, $forceInsert);
    }

    protected function _save(\IvozProvider\Model\Raw\AstMusiconhold $model,
     ...
     * @param IvozProvider\Model\Raw\AstMusiconhold|null $entry The object to load the data into, or null to have one created
     * @return IvozProvider\Model\Raw\AstMusiconhold The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \IvozProvider\Model\AstMusiconhold();
        }
    ...
    protected function _etagChange()
    {
        ...
        $etag = $etags->findOneByField('table', 'AstMusiconhold');

        if (empty($etag)) {
            $etag = new \IvozProvider\Model\EtagVersions();
            $etag->setTable('AstMusiconhold');
        }
