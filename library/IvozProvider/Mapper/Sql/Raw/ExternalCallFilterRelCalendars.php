...
/**
 * Data Mapper implementation for IvozProvider\Model\ExternalCallFilterRelCalendars
 *
 * @package IvozProvider\Mapper\Sql
 * @subpackage Raw
 * @author Luis Felipe Garcia
...
class ExternalCallFilterRelCalendars extends MapperAbstract
{
    protected $_modelName = 'IvozProvider\\Model\\ExternalCallFilterRelCalendars';
     ...
     *
     * @param IvozProvider\Model\Raw\ExternalCallFilterRelCalendars $model
     * @return array
     */
    public function toArray($model, $fields = array())
    {

        if (!$model instanceof \IvozProvider\Model\Raw\ExternalCallFilterRelCalendars) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \IvozProvider\Model\Raw\ExternalCallFilterRelCalendars object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\IvozProvider\Model\\ExternalCallFilterRelCalendars object in toArray for " . get_class($this);
            }
     ...
     *
     * @return IvozProvider\\Mapper\\Sql\\DbTable\\ExternalCallFilterRelCalendars
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('IvozProvider\\Mapper\\Sql\\DbTable\\ExternalCallFilterRelCalendars');
        }
     ...
     *
     * @param IvozProvider\Model\Raw\ExternalCallFilterRelCalendars $model The model to delete
    ...
    public function delete(\IvozProvider\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \IvozProvider\Model\Raw\ExternalCallFilterRelCalendars) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\IvozProvider\\Model\\ExternalCallFilterRelCalendars object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\IvozProvider\\Model\\ExternalCallFilterRelCalendars object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('IvozProvider\\Mapper\\Sql\\DbTable\\ExternalCallFilterRelCalendars', $capitalizedFk);
     ...
     */
    public function save(\IvozProvider\Model\Raw\ExternalCallFilterRelCalendars $model, $forceInsert = false)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\IvozProvider\Model\Raw\ExternalCallFilterRelCalendars $model, $useTransaction = true,
            $transactionTag = null, $forceInsert = false)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag, $forceInsert);
    }

    protected function _save(\IvozProvider\Model\Raw\ExternalCallFilterRelCalendars $model,
     ...
     * @param IvozProvider\Model\Raw\ExternalCallFilterRelCalendars|null $entry The object to load the data into, or null to have one created
     * @return IvozProvider\Model\Raw\ExternalCallFilterRelCalendars The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \IvozProvider\Model\ExternalCallFilterRelCalendars();
        }
        ...
        } else if ($data instanceof \IvozProvider\Model\Raw\ExternalCallFilterRelCalendars) {
            $entry->setId($data->getId())
                  ->setFilterId($data->getFilterId())
    ...
    protected function _etagChange()
    {
        ...
        $etag = $etags->findOneByField('table', 'ExternalCallFilterRelCalendars');

        if (empty($etag)) {
            $etag = new \IvozProvider\Model\EtagVersions();
            $etag->setTable('ExternalCallFilterRelCalendars');
        }
