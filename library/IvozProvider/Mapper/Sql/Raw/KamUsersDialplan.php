...
/**
 * Data Mapper implementation for IvozProvider\Model\KamUsersDialplan
 *
 * @package IvozProvider\Mapper\Sql
 * @subpackage Raw
 * @author Luis Felipe Garcia
...
class KamUsersDialplan extends MapperAbstract
{
    protected $_modelName = 'IvozProvider\\Model\\KamUsersDialplan';
     ...
     *
     * @param IvozProvider\Model\Raw\KamUsersDialplan $model
     * @return array
     */
    public function toArray($model, $fields = array())
    {

        if (!$model instanceof \IvozProvider\Model\Raw\KamUsersDialplan) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \IvozProvider\Model\Raw\KamUsersDialplan object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\IvozProvider\Model\\KamUsersDialplan object in toArray for " . get_class($this);
            }
        ...
        if (empty($fields)) {
            $result = array(
                ...
                'dpid' => $model->getDpid(),
                'pr' => $model->getPr(),
                'match_op' => $model->getMatchOp(),
                'match_exp' => $model->getMatchExp(),
                'match_len' => $model->getMatchLen(),
                'subst_exp' => $model->getSubstExp(),
                'repl_exp' => $model->getReplExp(),
                'attrs' => $model->getAttrs(),
                'transformationRulesetGroupsUsersId' => $model->getTransformationRulesetGroupsUsersId(),
            );
        } else {
            $result = array();
     ...
     *
     * @return IvozProvider\\Mapper\\Sql\\DbTable\\KamUsersDialplan
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('IvozProvider\\Mapper\\Sql\\DbTable\\KamUsersDialplan');
        }
     ...
     *
     * @param IvozProvider\Model\Raw\KamUsersDialplan $model The model to delete
    ...
    public function delete(\IvozProvider\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \IvozProvider\Model\Raw\KamUsersDialplan) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\IvozProvider\\Model\\KamUsersDialplan object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\IvozProvider\\Model\\KamUsersDialplan object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('IvozProvider\\Mapper\\Sql\\DbTable\\KamUsersDialplan', $capitalizedFk);
     ...
     */
    public function save(\IvozProvider\Model\Raw\KamUsersDialplan $model, $forceInsert = false)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\IvozProvider\Model\Raw\KamUsersDialplan $model, $useTransaction = true,
            $transactionTag = null, $forceInsert = false)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag, $forceInsert);
    }

    protected function _save(\IvozProvider\Model\Raw\KamUsersDialplan $model,
     ...
     * @param IvozProvider\Model\Raw\KamUsersDialplan|null $entry The object to load the data into, or null to have one created
     * @return IvozProvider\Model\Raw\KamUsersDialplan The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \IvozProvider\Model\KamUsersDialplan();
        }
        ...
        if (is_array($data)) {
            $entry->setId($data['id'])
                  ->setDpid($data['dpid'])
                  ->setPr($data['pr'])
                  ->setMatchOp($data['match_op'])
                  ->setMatchExp($data['match_exp'])
                  ->setMatchLen($data['match_len'])
                  ->setSubstExp($data['subst_exp'])
                  ->setReplExp($data['repl_exp'])
                  ->setAttrs($data['attrs'])
                  ->setTransformationRulesetGroupsUsersId($data['transformationRulesetGroupsUsersId']);
        ...
        } else if ($data instanceof \IvozProvider\Model\Raw\KamUsersDialplan) {
            $entry->setId($data->getId())
                  ->setDpid($data->getDpid())
                  ->setPr($data->getPr())
                  ->setMatchOp($data->getMatchOp())
                  ->setMatchExp($data->getMatchExp())
                  ->setMatchLen($data->getMatchLen())
                  ->setSubstExp($data->getSubstExp())
                  ->setReplExp($data->getReplExp())
                  ->setAttrs($data->getAttrs())
                  ->setTransformationRulesetGroupsUsersId($data->getTransformationRulesetGroupsUsersId());
