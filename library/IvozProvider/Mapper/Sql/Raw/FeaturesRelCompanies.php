...
/**
 * Data Mapper implementation for IvozProvider\Model\FeaturesRelCompanies
 *
 * @package IvozProvider\Mapper\Sql
 * @subpackage Raw
 * @author Luis Felipe Garcia
...
class FeaturesRelCompanies extends MapperAbstract
{
    protected $_modelName = 'IvozProvider\\Model\\FeaturesRelCompanies';
     ...
     *
     * @param IvozProvider\Model\Raw\FeaturesRelCompanies $model
     * @return array
     */
    public function toArray($model, $fields = array())
    {

        if (!$model instanceof \IvozProvider\Model\Raw\FeaturesRelCompanies) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \IvozProvider\Model\Raw\FeaturesRelCompanies object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\IvozProvider\Model\\FeaturesRelCompanies object in toArray for " . get_class($this);
            }
     ...
     *
     * @return IvozProvider\\Mapper\\Sql\\DbTable\\FeaturesRelCompanies
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('IvozProvider\\Mapper\\Sql\\DbTable\\FeaturesRelCompanies');
        }
     ...
     *
     * @param IvozProvider\Model\Raw\FeaturesRelCompanies $model The model to delete
    ...
    public function delete(\IvozProvider\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \IvozProvider\Model\Raw\FeaturesRelCompanies) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\IvozProvider\\Model\\FeaturesRelCompanies object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\IvozProvider\\Model\\FeaturesRelCompanies object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('IvozProvider\\Mapper\\Sql\\DbTable\\FeaturesRelCompanies', $capitalizedFk);
     ...
     */
    public function save(\IvozProvider\Model\Raw\FeaturesRelCompanies $model, $forceInsert = false)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\IvozProvider\Model\Raw\FeaturesRelCompanies $model, $useTransaction = true,
            $transactionTag = null, $forceInsert = false)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag, $forceInsert);
    }

    protected function _save(\IvozProvider\Model\Raw\FeaturesRelCompanies $model,
     ...
     * @param IvozProvider\Model\Raw\FeaturesRelCompanies|null $entry The object to load the data into, or null to have one created
     * @return IvozProvider\Model\Raw\FeaturesRelCompanies The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \IvozProvider\Model\FeaturesRelCompanies();
        }
        ...
        } else if ($data instanceof \IvozProvider\Model\Raw\FeaturesRelCompanies) {
    ...
    protected function _etagChange()
    {
        ...
        $etag = $etags->findOneByField('table', 'FeaturesRelCompanies');

        if (empty($etag)) {
            $etag = new \IvozProvider\Model\EtagVersions();
            $etag->setTable('FeaturesRelCompanies');
        }
