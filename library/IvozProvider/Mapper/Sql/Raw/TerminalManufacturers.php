...
/**
 * Data Mapper implementation for IvozProvider\Model\TerminalManufacturers
 *
 * @package IvozProvider\Mapper\Sql
 * @subpackage Raw
 * @author Luis Felipe Garcia
...
class TerminalManufacturers extends MapperAbstract
{
    protected $_modelName = 'IvozProvider\\Model\\TerminalManufacturers';
     ...
     *
     * @param IvozProvider\Model\Raw\TerminalManufacturers $model
     * @return array
     */
    public function toArray($model, $fields = array())
    {

        if (!$model instanceof \IvozProvider\Model\Raw\TerminalManufacturers) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \IvozProvider\Model\Raw\TerminalManufacturers object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\IvozProvider\Model\\TerminalManufacturers object in toArray for " . get_class($this);
            }
     ...
     *
     * @return IvozProvider\\Mapper\\Sql\\DbTable\\TerminalManufacturers
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('IvozProvider\\Mapper\\Sql\\DbTable\\TerminalManufacturers');
        }
     ...
     *
     * @param IvozProvider\Model\Raw\TerminalManufacturers $model The model to delete
    ...
    public function delete(\IvozProvider\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \IvozProvider\Model\Raw\TerminalManufacturers) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\IvozProvider\\Model\\TerminalManufacturers object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\IvozProvider\\Model\\TerminalManufacturers object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('IvozProvider\\Mapper\\Sql\\DbTable\\TerminalManufacturers', $capitalizedFk);
     ...
     */
    public function save(\IvozProvider\Model\Raw\TerminalManufacturers $model, $forceInsert = false)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\IvozProvider\Model\Raw\TerminalManufacturers $model, $useTransaction = true,
            $transactionTag = null, $forceInsert = false)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag, $forceInsert);
    }

    protected function _save(\IvozProvider\Model\Raw\TerminalManufacturers $model,
    ...
    )
    {
        ...
        try {
            ...
            if ($recursive) {
                if ($model->getTerminalModels(null, null, true) !== null) {
                    $terminalModels = $model->getTerminalModels();

                    if (!is_array($terminalModels)) {

                        $terminalModels = array($terminalModels);
                    }

                    foreach ($terminalModels as $value) {
                        $value->setTerminalManufacturerId($primaryKey)
     ...
     * @param IvozProvider\Model\Raw\TerminalManufacturers|null $entry The object to load the data into, or null to have one created
     * @return IvozProvider\Model\Raw\TerminalManufacturers The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \IvozProvider\Model\TerminalManufacturers();
        }
        ...
        } else if ($data instanceof \IvozProvider\Model\Raw\TerminalManufacturers) {
    ...
    protected function _etagChange()
    {
        ...
        $etag = $etags->findOneByField('table', 'TerminalManufacturers');

        if (empty($etag)) {
            $etag = new \IvozProvider\Model\EtagVersions();
            $etag->setTable('TerminalManufacturers');
        }
