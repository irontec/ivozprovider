...
class ApplicationServers extends MapperAbstract
{
    protected $_modelName = 'IvozProvider\\Model\\ApplicationServers';
     ...
     *
     * @param IvozProvider\Model\Raw\ApplicationServers $model
     * @return array
     */
    public function toArray($model, $fields = array())
    {

        if (!$model instanceof \IvozProvider\Model\Raw\ApplicationServers) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \IvozProvider\Model\Raw\ApplicationServers object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\IvozProvider\Model\\ApplicationServers object in toArray for " . get_class($this);
            }
     ...
     *
     * @return IvozProvider\\Mapper\\Sql\\DbTable\\ApplicationServers
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('IvozProvider\\Mapper\\Sql\\DbTable\\ApplicationServers');
        }
     ...
     *
     * @param IvozProvider\Model\Raw\ApplicationServers $model The model to delete
    ...
    public function delete(\IvozProvider\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \IvozProvider\Model\Raw\ApplicationServers) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\IvozProvider\\Model\\ApplicationServers object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\IvozProvider\\Model\\ApplicationServers object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('IvozProvider\\Mapper\\Sql\\DbTable\\ApplicationServers', $capitalizedFk);
     ...
     */
    public function save(\IvozProvider\Model\Raw\ApplicationServers $model, $forceInsert = false)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\IvozProvider\Model\Raw\ApplicationServers $model, $useTransaction = true,
            ...
            if ($recursive) {
                if ($model->getCompanies(null, null, true) !== null) {
                    ...
                    foreach ($companies as $value) {
                        $value->setApplicationServerId($primaryKey)
                              ->saveRecursive(false, $transactionTag);
                    }
                }

                if ($model->getKamDispatcher(null, null, true) !== null) {
                    $kamDispatcher = $model->getKamDispatcher();

                    if (!is_array($kamDispatcher)) {

                        $kamDispatcher = array($kamDispatcher);
                    }

                    foreach ($kamDispatcher as $value) {
                        $value->setApplicationServerId($primaryKey)
                              ->saveRecursive(false, $transactionTag);
                    }
                }
     ...
     * @param IvozProvider\Model\Raw\ApplicationServers|null $entry The object to load the data into, or null to have one created
     * @return IvozProvider\Model\Raw\ApplicationServers The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \IvozProvider\Model\ApplicationServers();
        }
    ...
    protected function _etagChange()
    {
        ...
        $etag = $etags->findOneByField('table', 'ApplicationServers');

        if (empty($etag)) {
            $etag = new \IvozProvider\Model\EtagVersions();
            $etag->setTable('ApplicationServers');
        }
