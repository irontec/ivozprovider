...
/**
 * Data Mapper implementation for IvozProvider\Model\HolidayDates
 *
 * @package IvozProvider\Mapper\Sql
 * @subpackage Raw
 * @author Luis Felipe Garcia
...
class HolidayDates extends MapperAbstract
{
    protected $_modelName = 'IvozProvider\\Model\\HolidayDates';
     ...
     *
     * @param IvozProvider\Model\Raw\HolidayDates $model
     * @return array
     */
    public function toArray($model, $fields = array())
    {

        if (!$model instanceof \IvozProvider\Model\Raw\HolidayDates) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \IvozProvider\Model\Raw\HolidayDates object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\IvozProvider\Model\\HolidayDates object in toArray for " . get_class($this);
            }
     ...
     *
     * @return IvozProvider\\Mapper\\Sql\\DbTable\\HolidayDates
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('IvozProvider\\Mapper\\Sql\\DbTable\\HolidayDates');
        }
     ...
     *
     * @param IvozProvider\Model\Raw\HolidayDates $model The model to delete
    ...
    public function delete(\IvozProvider\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \IvozProvider\Model\Raw\HolidayDates) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\IvozProvider\\Model\\HolidayDates object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\IvozProvider\\Model\\HolidayDates object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('IvozProvider\\Mapper\\Sql\\DbTable\\HolidayDates', $capitalizedFk);
     ...
     */
    public function save(\IvozProvider\Model\Raw\HolidayDates $model, $forceInsert = false)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\IvozProvider\Model\Raw\HolidayDates $model, $useTransaction = true,
            $transactionTag = null, $forceInsert = false)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag, $forceInsert);
    }

    protected function _save(\IvozProvider\Model\Raw\HolidayDates $model,
     ...
     * @param IvozProvider\Model\Raw\HolidayDates|null $entry The object to load the data into, or null to have one created
     * @return IvozProvider\Model\Raw\HolidayDates The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \IvozProvider\Model\HolidayDates();
        }
        ...
        } else if ($data instanceof \IvozProvider\Model\Raw\HolidayDates) {
            $entry->setId($data->getId())
                  ->setCalendarId($data->getCalendarId())
                  ->setName($data->getName())
                  ->setEventDate($data->getEventDate())
    ...
    protected function _etagChange()
    {
        ...
        $etag = $etags->findOneByField('table', 'HolidayDates');

        if (empty($etag)) {
            $etag = new \IvozProvider\Model\EtagVersions();
            $etag->setTable('HolidayDates');
        }
