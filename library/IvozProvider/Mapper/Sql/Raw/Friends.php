...
class Friends extends MapperAbstract
{
    protected $_modelName = 'IvozProvider\\Model\\Friends';
     ...
     *
     * @param IvozProvider\Model\Raw\Friends $model
     * @return array
     */
    public function toArray($model, $fields = array())
    {

        if (!$model instanceof \IvozProvider\Model\Raw\Friends) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \IvozProvider\Model\Raw\Friends object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\IvozProvider\Model\\Friends object in toArray for " . get_class($this);
            }
     ...
     *
     * @return IvozProvider\\Mapper\\Sql\\DbTable\\Friends
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('IvozProvider\\Mapper\\Sql\\DbTable\\Friends');
        }
     ...
     *
     * @param IvozProvider\Model\Raw\Friends $model The model to delete
    ...
    public function delete(\IvozProvider\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \IvozProvider\Model\Raw\Friends) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\IvozProvider\\Model\\Friends object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\IvozProvider\\Model\\Friends object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('IvozProvider\\Mapper\\Sql\\DbTable\\Friends', $capitalizedFk);
     ...
     */
    public function save(\IvozProvider\Model\Raw\Friends $model, $forceInsert = false)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\IvozProvider\Model\Raw\Friends $model, $useTransaction = true,
    ...
    )
    {
        ...
        try {
            ...
            if ($recursive) {
                if ($model->getFriendsPatterns(null, null, true) !== null) {
                    $friendsPatterns = $model->getFriendsPatterns();

                    if (!is_array($friendsPatterns)) {

                        $friendsPatterns = array($friendsPatterns);
                    }

                    foreach ($friendsPatterns as $value) {
                        $value->setFriendId($primaryKey)
     ...
     * @param IvozProvider\Model\Raw\Friends|null $entry The object to load the data into, or null to have one created
     * @return IvozProvider\Model\Raw\Friends The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \IvozProvider\Model\Friends();
        }
    ...
    protected function _etagChange()
    {
        ...
        $etag = $etags->findOneByField('table', 'Friends');

        if (empty($etag)) {
            $etag = new \IvozProvider\Model\EtagVersions();
            $etag->setTable('Friends');
        }
