...
/**
 * Data Mapper implementation for IvozProvider\Model\Calendars
 *
 * @package IvozProvider\Mapper\Sql
 * @subpackage Raw
 * @author Luis Felipe Garcia
...
class Calendars extends MapperAbstract
{
    protected $_modelName = 'IvozProvider\\Model\\Calendars';
     ...
     *
     * @param IvozProvider\Model\Raw\Calendars $model
     * @return array
     */
    public function toArray($model, $fields = array())
    {

        if (!$model instanceof \IvozProvider\Model\Raw\Calendars) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \IvozProvider\Model\Raw\Calendars object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\IvozProvider\Model\\Calendars object in toArray for " . get_class($this);
            }
     ...
     *
     * @return IvozProvider\\Mapper\\Sql\\DbTable\\Calendars
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('IvozProvider\\Mapper\\Sql\\DbTable\\Calendars');
        }
     ...
     *
     * @param IvozProvider\Model\Raw\Calendars $model The model to delete
    ...
    public function delete(\IvozProvider\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \IvozProvider\Model\Raw\Calendars) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\IvozProvider\\Model\\Calendars object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\IvozProvider\\Model\\Calendars object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('IvozProvider\\Mapper\\Sql\\DbTable\\Calendars', $capitalizedFk);
     ...
     */
    public function save(\IvozProvider\Model\Raw\Calendars $model, $forceInsert = false)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\IvozProvider\Model\Raw\Calendars $model, $useTransaction = true,
            $transactionTag = null, $forceInsert = false)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag, $forceInsert);
    }

    protected function _save(\IvozProvider\Model\Raw\Calendars $model,
     ...
     * @param IvozProvider\Model\Raw\Calendars|null $entry The object to load the data into, or null to have one created
     * @return IvozProvider\Model\Raw\Calendars The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \IvozProvider\Model\Calendars();
        }
        ...
        } else if ($data instanceof \IvozProvider\Model\Raw\Calendars) {
    ...
    protected function _etagChange()
    {
        ...
        $etag = $etags->findOneByField('table', 'Calendars');

        if (empty($etag)) {
            $etag = new \IvozProvider\Model\EtagVersions();
            $etag->setTable('Calendars');
        }
