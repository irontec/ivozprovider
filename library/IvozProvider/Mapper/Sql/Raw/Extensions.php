...
class Extensions extends MapperAbstract
{
    protected $_modelName = 'IvozProvider\\Model\\Extensions';
     ...
     *
     * @param IvozProvider\Model\Raw\Extensions $model
     * @return array
     */
    public function toArray($model, $fields = array())
    {

        if (!$model instanceof \IvozProvider\Model\Raw\Extensions) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \IvozProvider\Model\Raw\Extensions object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\IvozProvider\Model\\Extensions object in toArray for " . get_class($this);
            }
     ...
     *
     * @return IvozProvider\\Mapper\\Sql\\DbTable\\Extensions
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('IvozProvider\\Mapper\\Sql\\DbTable\\Extensions');
        }
     ...
     *
     * @param IvozProvider\Model\Raw\Extensions $model The model to delete
    ...
    public function delete(\IvozProvider\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \IvozProvider\Model\Raw\Extensions) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\IvozProvider\\Model\\Extensions object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\IvozProvider\\Model\\Extensions object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('IvozProvider\\Mapper\\Sql\\DbTable\\Extensions', $capitalizedFk);
     ...
     */
    public function save(\IvozProvider\Model\Raw\Extensions $model, $forceInsert = false)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\IvozProvider\Model\Raw\Extensions $model, $useTransaction = true,
            ...
            if ($recursive) {
                ...
                if ($model->getExternalCallFiltersByHolidayExtension(null, null, true) !== null) {
                    ...
                    foreach ($externalCallFilters as $value) {
                        $value->setHolidayExtensionId($primaryKey)
                              ->saveRecursive(false, $transactionTag);
                    }
                }

                if ($model->getExternalCallFiltersByOutOfScheduleExtension(null, null, true) !== null) {
                    $externalCallFilters = $model->getExternalCallFiltersByOutOfScheduleExtension();
                ...
                if ($model->getIVRCommonByErrorExtension(null, null, true) !== null) {
                    $iVRCommon = $model->getIVRCommonByErrorExtension();
                    ...
                    foreach ($iVRCommon as $value) {
                        $value->setErrorExtensionId($primaryKey)
                              ->saveRecursive(false, $transactionTag);
                    }
                }

                if ($model->getIVRCustomByTimeoutExtension(null, null, true) !== null) {
                    $iVRCustom = $model->getIVRCustomByTimeoutExtension();
                ...
                if ($model->getIVRCustomByErrorExtension(null, null, true) !== null) {
                    $iVRCustom = $model->getIVRCustomByErrorExtension();
                ...
                if ($model->getIVRCustomEntries(null, null, true) !== null) {
                    ...
                    foreach ($iVRCustomEntries as $value) {
                        $value->setTargetExtensionId($primaryKey)
                              ->saveRecursive(false, $transactionTag);
                    }
                }
     ...
     * @param IvozProvider\Model\Raw\Extensions|null $entry The object to load the data into, or null to have one created
     * @return IvozProvider\Model\Raw\Extensions The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \IvozProvider\Model\Extensions();
        }
    ...
    protected function _etagChange()
    {
        ...
        $etag = $etags->findOneByField('table', 'Extensions');

        if (empty($etag)) {
            $etag = new \IvozProvider\Model\EtagVersions();
            $etag->setTable('Extensions');
        }
