...
/**
 * Data Mapper implementation for IvozProvider\Model\TransformationRulesetGroupsUsers
 *
 * @package IvozProvider\Mapper\Sql
 * @subpackage Raw
 * @author Luis Felipe Garcia
...
class TransformationRulesetGroupsUsers extends MapperAbstract
{
    protected $_modelName = 'IvozProvider\\Model\\TransformationRulesetGroupsUsers';
     ...
     *
     * @param IvozProvider\Model\Raw\TransformationRulesetGroupsUsers $model
     * @return array
     */
    public function toArray($model, $fields = array())
    {

        if (!$model instanceof \IvozProvider\Model\Raw\TransformationRulesetGroupsUsers) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \IvozProvider\Model\Raw\TransformationRulesetGroupsUsers object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\IvozProvider\Model\\TransformationRulesetGroupsUsers object in toArray for " . get_class($this);
            }
     ...
     *
     * @return IvozProvider\\Mapper\\Sql\\DbTable\\TransformationRulesetGroupsUsers
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('IvozProvider\\Mapper\\Sql\\DbTable\\TransformationRulesetGroupsUsers');
        }
     ...
     *
     * @param IvozProvider\Model\Raw\TransformationRulesetGroupsUsers $model The model to delete
    ...
    public function delete(\IvozProvider\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \IvozProvider\Model\Raw\TransformationRulesetGroupsUsers) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\IvozProvider\\Model\\TransformationRulesetGroupsUsers object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\IvozProvider\\Model\\TransformationRulesetGroupsUsers object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('IvozProvider\\Mapper\\Sql\\DbTable\\TransformationRulesetGroupsUsers', $capitalizedFk);
     ...
     */
    public function save(\IvozProvider\Model\Raw\TransformationRulesetGroupsUsers $model, $forceInsert = false)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\IvozProvider\Model\Raw\TransformationRulesetGroupsUsers $model, $useTransaction = true,
            $transactionTag = null, $forceInsert = false)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag, $forceInsert);
    }

    protected function _save(\IvozProvider\Model\Raw\TransformationRulesetGroupsUsers $model,
     ...
     * @param IvozProvider\Model\Raw\TransformationRulesetGroupsUsers|null $entry The object to load the data into, or null to have one created
     * @return IvozProvider\Model\Raw\TransformationRulesetGroupsUsers The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \IvozProvider\Model\TransformationRulesetGroupsUsers();
        }
        ...
        } else if ($data instanceof \IvozProvider\Model\Raw\TransformationRulesetGroupsUsers) {
