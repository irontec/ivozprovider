...
/**
 * Data Mapper implementation for IvozProvider\Model\PickUpGroups
 *
 * @package IvozProvider\Mapper\Sql
 * @subpackage Raw
 * @author Luis Felipe Garcia
...
class PickUpGroups extends MapperAbstract
{
    protected $_modelName = 'IvozProvider\\Model\\PickUpGroups';
     ...
     *
     * @param IvozProvider\Model\Raw\PickUpGroups $model
     * @return array
     */
    public function toArray($model, $fields = array())
    {

        if (!$model instanceof \IvozProvider\Model\Raw\PickUpGroups) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \IvozProvider\Model\Raw\PickUpGroups object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\IvozProvider\Model\\PickUpGroups object in toArray for " . get_class($this);
            }
     ...
     *
     * @return IvozProvider\\Mapper\\Sql\\DbTable\\PickUpGroups
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('IvozProvider\\Mapper\\Sql\\DbTable\\PickUpGroups');
        }
     ...
     *
     * @param IvozProvider\Model\Raw\PickUpGroups $model The model to delete
    ...
    public function delete(\IvozProvider\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \IvozProvider\Model\Raw\PickUpGroups) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\IvozProvider\\Model\\PickUpGroups object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\IvozProvider\\Model\\PickUpGroups object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('IvozProvider\\Mapper\\Sql\\DbTable\\PickUpGroups', $capitalizedFk);
     ...
     */
    public function save(\IvozProvider\Model\Raw\PickUpGroups $model, $forceInsert = false)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\IvozProvider\Model\Raw\PickUpGroups $model, $useTransaction = true,
            $transactionTag = null, $forceInsert = false)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag, $forceInsert);
    }

    protected function _save(\IvozProvider\Model\Raw\PickUpGroups $model,
            ...
            if ($recursive) {
                if ($model->getPickUpRelUsers(null, null, true) !== null) {
                    ...
                    foreach ($pickUpRelUsers as $value) {
                        $value->setPickUpGroupId($primaryKey)
     ...
     * @param IvozProvider\Model\Raw\PickUpGroups|null $entry The object to load the data into, or null to have one created
     * @return IvozProvider\Model\Raw\PickUpGroups The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \IvozProvider\Model\PickUpGroups();
        }
        ...
        } else if ($data instanceof \IvozProvider\Model\Raw\PickUpGroups) {
    ...
    protected function _etagChange()
    {
        ...
        $etag = $etags->findOneByField('table', 'PickUpGroups');

        if (empty($etag)) {
            $etag = new \IvozProvider\Model\EtagVersions();
            $etag->setTable('PickUpGroups');
        }
