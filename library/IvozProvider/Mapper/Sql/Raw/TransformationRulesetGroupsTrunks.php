...
class TransformationRulesetGroupsTrunks extends MapperAbstract
{
    protected $_modelName = 'IvozProvider\\Model\\TransformationRulesetGroupsTrunks';
     ...
     *
     * @param IvozProvider\Model\Raw\TransformationRulesetGroupsTrunks $model
     * @return array
     */
    public function toArray($model, $fields = array())
    {

        if (!$model instanceof \IvozProvider\Model\Raw\TransformationRulesetGroupsTrunks) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \IvozProvider\Model\Raw\TransformationRulesetGroupsTrunks object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\IvozProvider\Model\\TransformationRulesetGroupsTrunks object in toArray for " . get_class($this);
            }
        ...
        if (empty($fields)) {
            $result = array(
                ...
                'name' => $model->getName(),
                'caller_in' => $model->getCallerIn(),
                'callee_in' => $model->getCalleeIn(),
     ...
     *
     * @return IvozProvider\\Mapper\\Sql\\DbTable\\TransformationRulesetGroupsTrunks
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('IvozProvider\\Mapper\\Sql\\DbTable\\TransformationRulesetGroupsTrunks');
        }
     ...
     *
     * @param IvozProvider\Model\Raw\TransformationRulesetGroupsTrunks $model The model to delete
    ...
    public function delete(\IvozProvider\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \IvozProvider\Model\Raw\TransformationRulesetGroupsTrunks) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\IvozProvider\\Model\\TransformationRulesetGroupsTrunks object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\IvozProvider\\Model\\TransformationRulesetGroupsTrunks object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('IvozProvider\\Mapper\\Sql\\DbTable\\TransformationRulesetGroupsTrunks', $capitalizedFk);
     ...
     */
    public function save(\IvozProvider\Model\Raw\TransformationRulesetGroupsTrunks $model, $forceInsert = false)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\IvozProvider\Model\Raw\TransformationRulesetGroupsTrunks $model, $useTransaction = true,
            ...
            if ($recursive) {
                if ($model->getPeeringContracts(null, null, true) !== null) {
                    ...
                    foreach ($peeringContracts as $value) {
                        $value->setTransformationRulesetGroupsTrunksId($primaryKey)
                              ->saveRecursive(false, $transactionTag);
                    }
                }

                if ($model->getKamTrunksDialplan(null, null, true) !== null) {
                    $kamTrunksDialplan = $model->getKamTrunksDialplan();

                    if (!is_array($kamTrunksDialplan)) {

                        $kamTrunksDialplan = array($kamTrunksDialplan);
                    }

                    foreach ($kamTrunksDialplan as $value) {
                        $value->setTransformationRulesetGroupsTrunksId($primaryKey)
                              ->saveRecursive(false, $transactionTag);
                    }
                }
     ...
     * @param IvozProvider\Model\Raw\TransformationRulesetGroupsTrunks|null $entry The object to load the data into, or null to have one created
     * @return IvozProvider\Model\Raw\TransformationRulesetGroupsTrunks The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \IvozProvider\Model\TransformationRulesetGroupsTrunks();
        }
    ...
    protected function _etagChange()
    {
        ...
        $etag = $etags->findOneByField('table', 'TransformationRulesetGroupsTrunks');

        if (empty($etag)) {
            $etag = new \IvozProvider\Model\EtagVersions();
            $etag->setTable('TransformationRulesetGroupsTrunks');
        }
