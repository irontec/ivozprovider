...
/**
 * Data Mapper implementation for IvozProvider\Model\KamUsersDomain
 *
 * @package IvozProvider\Mapper\Sql
 * @subpackage Raw
 * @author Luis Felipe Garcia
...
class KamUsersDomain extends MapperAbstract
{
    protected $_modelName = 'IvozProvider\\Model\\KamUsersDomain';
     ...
     *
     * @param IvozProvider\Model\Raw\KamUsersDomain $model
     * @return array
     */
    public function toArray($model, $fields = array())
    {

        if (!$model instanceof \IvozProvider\Model\Raw\KamUsersDomain) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \IvozProvider\Model\Raw\KamUsersDomain object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\IvozProvider\Model\\KamUsersDomain object in toArray for " . get_class($this);
            }
     ...
     *
     * @return IvozProvider\\Mapper\\Sql\\DbTable\\KamUsersDomain
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('IvozProvider\\Mapper\\Sql\\DbTable\\KamUsersDomain');
        }
     ...
     *
     * @param IvozProvider\Model\Raw\KamUsersDomain $model The model to delete
    ...
    public function delete(\IvozProvider\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \IvozProvider\Model\Raw\KamUsersDomain) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\IvozProvider\\Model\\KamUsersDomain object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\IvozProvider\\Model\\KamUsersDomain object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('IvozProvider\\Mapper\\Sql\\DbTable\\KamUsersDomain', $capitalizedFk);
     ...
     */
    public function save(\IvozProvider\Model\Raw\KamUsersDomain $model, $forceInsert = false)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\IvozProvider\Model\Raw\KamUsersDomain $model, $useTransaction = true,
            $transactionTag = null, $forceInsert = false)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag, $forceInsert);
    }

    protected function _save(\IvozProvider\Model\Raw\KamUsersDomain $model,
     ...
     * @param IvozProvider\Model\Raw\KamUsersDomain|null $entry The object to load the data into, or null to have one created
     * @return IvozProvider\Model\Raw\KamUsersDomain The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \IvozProvider\Model\KamUsersDomain();
        }
        ...
        } else if ($data instanceof \IvozProvider\Model\Raw\KamUsersDomain) {
