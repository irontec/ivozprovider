...
class PeeringContracts extends MapperAbstract
{
    protected $_modelName = 'IvozProvider\\Model\\PeeringContracts';
     ...
     *
     * @param IvozProvider\Model\Raw\PeeringContracts $model
     * @return array
     */
    public function toArray($model, $fields = array())
    {

        if (!$model instanceof \IvozProvider\Model\Raw\PeeringContracts) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \IvozProvider\Model\Raw\PeeringContracts object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\IvozProvider\Model\\PeeringContracts object in toArray for " . get_class($this);
            }
     ...
     *
     * @return IvozProvider\\Mapper\\Sql\\DbTable\\PeeringContracts
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('IvozProvider\\Mapper\\Sql\\DbTable\\PeeringContracts');
        }
     ...
     *
     * @param IvozProvider\Model\Raw\PeeringContracts $model The model to delete
    ...
    public function delete(\IvozProvider\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \IvozProvider\Model\Raw\PeeringContracts) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\IvozProvider\\Model\\PeeringContracts object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\IvozProvider\\Model\\PeeringContracts object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('IvozProvider\\Mapper\\Sql\\DbTable\\PeeringContracts', $capitalizedFk);
     ...
     */
    public function save(\IvozProvider\Model\Raw\PeeringContracts $model, $forceInsert = false)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\IvozProvider\Model\Raw\PeeringContracts $model, $useTransaction = true,
     ...
     * @param IvozProvider\Model\Raw\PeeringContracts|null $entry The object to load the data into, or null to have one created
     * @return IvozProvider\Model\Raw\PeeringContracts The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \IvozProvider\Model\PeeringContracts();
        }
