...
class PricingPlans extends MapperAbstract
{
    protected $_modelName = 'IvozProvider\\Model\\PricingPlans';
     ...
     *
     * @param IvozProvider\Model\Raw\PricingPlans $model
     * @return array
     */
    public function toArray($model, $fields = array())
    {

        if (!$model instanceof \IvozProvider\Model\Raw\PricingPlans) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \IvozProvider\Model\Raw\PricingPlans object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\IvozProvider\Model\\PricingPlans object in toArray for " . get_class($this);
            }
     ...
     *
     * @return IvozProvider\\Mapper\\Sql\\DbTable\\PricingPlans
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('IvozProvider\\Mapper\\Sql\\DbTable\\PricingPlans');
        }
     ...
     *
     * @param IvozProvider\Model\Raw\PricingPlans $model The model to delete
    ...
    public function delete(\IvozProvider\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \IvozProvider\Model\Raw\PricingPlans) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\IvozProvider\\Model\\PricingPlans object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\IvozProvider\\Model\\PricingPlans object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('IvozProvider\\Mapper\\Sql\\DbTable\\PricingPlans', $capitalizedFk);
     ...
     */
    public function save(\IvozProvider\Model\Raw\PricingPlans $model, $forceInsert = false)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\IvozProvider\Model\Raw\PricingPlans $model, $useTransaction = true,
     ...
     * @param IvozProvider\Model\Raw\PricingPlans|null $entry The object to load the data into, or null to have one created
     * @return IvozProvider\Model\Raw\PricingPlans The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \IvozProvider\Model\PricingPlans();
        }
        ...
        } else if ($data instanceof \IvozProvider\Model\Raw\PricingPlans) {
    ...
    protected function _etagChange()
    {
        ...
        $etag = $etags->findOneByField('table', 'PricingPlans');

        if (empty($etag)) {
            $etag = new \IvozProvider\Model\EtagVersions();
            $etag->setTable('PricingPlans');
        }
