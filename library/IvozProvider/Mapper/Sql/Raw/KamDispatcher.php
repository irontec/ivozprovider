...
/**
 * Data Mapper implementation for IvozProvider\Model\KamDispatcher
 *
 * @package IvozProvider\Mapper\Sql
 * @subpackage Raw
 * @author Luis Felipe Garcia
...
class KamDispatcher extends MapperAbstract
{
    protected $_modelName = 'IvozProvider\\Model\\KamDispatcher';
     ...
     *
     * @param IvozProvider\Model\Raw\KamDispatcher $model
     * @return array
     */
    public function toArray($model, $fields = array())
    {

        if (!$model instanceof \IvozProvider\Model\Raw\KamDispatcher) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \IvozProvider\Model\Raw\KamDispatcher object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\IvozProvider\Model\\KamDispatcher object in toArray for " . get_class($this);
            }
     ...
     *
     * @return IvozProvider\\Mapper\\Sql\\DbTable\\KamDispatcher
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('IvozProvider\\Mapper\\Sql\\DbTable\\KamDispatcher');
        }
     ...
     *
     * @param IvozProvider\Model\Raw\KamDispatcher $model The model to delete
    ...
    public function delete(\IvozProvider\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \IvozProvider\Model\Raw\KamDispatcher) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\IvozProvider\\Model\\KamDispatcher object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\IvozProvider\\Model\\KamDispatcher object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('IvozProvider\\Mapper\\Sql\\DbTable\\KamDispatcher', $capitalizedFk);
     ...
     */
    public function save(\IvozProvider\Model\Raw\KamDispatcher $model, $forceInsert = false)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\IvozProvider\Model\Raw\KamDispatcher $model, $useTransaction = true,
            $transactionTag = null, $forceInsert = false)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag, $forceInsert);
    }

    protected function _save(\IvozProvider\Model\Raw\KamDispatcher $model,
     ...
     * @param IvozProvider\Model\Raw\KamDispatcher|null $entry The object to load the data into, or null to have one created
     * @return IvozProvider\Model\Raw\KamDispatcher The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \IvozProvider\Model\KamDispatcher();
        }
        ...
        } else if ($data instanceof \IvozProvider\Model\Raw\KamDispatcher) {
    ...
    protected function _etagChange()
    {
        ...
        $etag = $etags->findOneByField('table', 'KamDispatcher');

        if (empty($etag)) {
            $etag = new \IvozProvider\Model\EtagVersions();
            $etag->setTable('KamDispatcher');
        }
