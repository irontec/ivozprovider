...
class PricingPlansRelCompanies extends MapperAbstract
{
    protected $_modelName = 'IvozProvider\\Model\\PricingPlansRelCompanies';
     ...
     *
     * @param IvozProvider\Model\Raw\PricingPlansRelCompanies $model
     * @return array
     */
    public function toArray($model, $fields = array())
    {

        if (!$model instanceof \IvozProvider\Model\Raw\PricingPlansRelCompanies) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \IvozProvider\Model\Raw\PricingPlansRelCompanies object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\IvozProvider\Model\\PricingPlansRelCompanies object in toArray for " . get_class($this);
            }
     ...
     *
     * @return IvozProvider\\Mapper\\Sql\\DbTable\\PricingPlansRelCompanies
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('IvozProvider\\Mapper\\Sql\\DbTable\\PricingPlansRelCompanies');
        }
     ...
     *
     * @param IvozProvider\Model\Raw\PricingPlansRelCompanies $model The model to delete
    ...
    public function delete(\IvozProvider\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \IvozProvider\Model\Raw\PricingPlansRelCompanies) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\IvozProvider\\Model\\PricingPlansRelCompanies object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\IvozProvider\\Model\\PricingPlansRelCompanies object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('IvozProvider\\Mapper\\Sql\\DbTable\\PricingPlansRelCompanies', $capitalizedFk);
     ...
     */
    public function save(\IvozProvider\Model\Raw\PricingPlansRelCompanies $model, $forceInsert = false)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\IvozProvider\Model\Raw\PricingPlansRelCompanies $model, $useTransaction = true,
     ...
     * @param IvozProvider\Model\Raw\PricingPlansRelCompanies|null $entry The object to load the data into, or null to have one created
     * @return IvozProvider\Model\Raw\PricingPlansRelCompanies The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \IvozProvider\Model\PricingPlansRelCompanies();
        }
        ...
        } else if ($data instanceof \IvozProvider\Model\Raw\PricingPlansRelCompanies) {
    ...
    protected function _etagChange()
    {
        ...
        $etag = $etags->findOneByField('table', 'PricingPlansRelCompanies');

        if (empty($etag)) {
            $etag = new \IvozProvider\Model\EtagVersions();
            $etag->setTable('PricingPlansRelCompanies');
        }
