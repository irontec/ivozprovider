...
/**
 * Data Mapper implementation for IvozProvider\Model\CallACL
 *
 * @package IvozProvider\Mapper\Sql
 * @subpackage Raw
 * @author Luis Felipe Garcia
...
class CallACL extends MapperAbstract
{
    protected $_modelName = 'IvozProvider\\Model\\CallACL';
     ...
     *
     * @param IvozProvider\Model\Raw\CallACL $model
     * @return array
     */
    public function toArray($model, $fields = array())
    {

        if (!$model instanceof \IvozProvider\Model\Raw\CallACL) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \IvozProvider\Model\Raw\CallACL object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\IvozProvider\Model\\CallACL object in toArray for " . get_class($this);
            }
        ...
        if (empty($fields)) {
            $result = array(
                ...
                'name' => $model->getName(),
                'defaultPolicy' => $model->getDefaultPolicy(),
            );
        } else {
            $result = array();
     ...
     *
     * @return IvozProvider\\Mapper\\Sql\\DbTable\\CallACL
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('IvozProvider\\Mapper\\Sql\\DbTable\\CallACL');
        }
     ...
     *
     * @param IvozProvider\Model\Raw\CallACL $model The model to delete
    ...
    public function delete(\IvozProvider\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \IvozProvider\Model\Raw\CallACL) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\IvozProvider\\Model\\CallACL object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\IvozProvider\\Model\\CallACL object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('IvozProvider\\Mapper\\Sql\\DbTable\\CallACL', $capitalizedFk);
     ...
     */
    public function save(\IvozProvider\Model\Raw\CallACL $model, $forceInsert = false)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\IvozProvider\Model\Raw\CallACL $model, $useTransaction = true,
            $transactionTag = null, $forceInsert = false)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag, $forceInsert);
    }

    protected function _save(\IvozProvider\Model\Raw\CallACL $model,
     ...
     * @param IvozProvider\Model\Raw\CallACL|null $entry The object to load the data into, or null to have one created
     * @return IvozProvider\Model\Raw\CallACL The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \IvozProvider\Model\CallACL();
        }
        ...
        } else if ($data instanceof \IvozProvider\Model\Raw\CallACL) {
            $entry->setId($data->getId())
                  ->setCompanyId($data->getCompanyId())
                  ->setName($data->getName())
                  ->setDefaultPolicy($data->getDefaultPolicy());
    ...
    protected function _etagChange()
    {
        ...
        $etag = $etags->findOneByField('table', 'CallACL');

        if (empty($etag)) {
            $etag = new \IvozProvider\Model\EtagVersions();
            $etag->setTable('CallACL');
        }
