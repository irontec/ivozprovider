...
class Queues extends MapperAbstract
{
    protected $_modelName = 'IvozProvider\\Model\\Queues';
     ...
     *
     * @param IvozProvider\Model\Raw\Queues $model
     * @return array
     */
    public function toArray($model, $fields = array())
    {

        if (!$model instanceof \IvozProvider\Model\Raw\Queues) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \IvozProvider\Model\Raw\Queues object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\IvozProvider\Model\\Queues object in toArray for " . get_class($this);
            }
        ...
        if (empty($fields)) {
            $result = array(
                ...
                'name' => $model->getName(),
                'maxWaitTime' => $model->getMaxWaitTime(),
                'timeoutLocutionId' => $model->getTimeoutLocutionId(),
                'timeoutTargetType' => $model->getTimeoutTargetType(),
                'timeoutNumberValue' => $model->getTimeoutNumberValue(),
                'timeoutExtensionId' => $model->getTimeoutExtensionId(),
                'timeoutVoiceMailUserId' => $model->getTimeoutVoiceMailUserId(),
                'maxlen' => $model->getMaxlen(),
                'fullLocutionId' => $model->getFullLocutionId(),
                'fullTargetType' => $model->getFullTargetType(),
                'fullNumberValue' => $model->getFullNumberValue(),
                'fullExtensionId' => $model->getFullExtensionId(),
                'fullVoiceMailUserId' => $model->getFullVoiceMailUserId(),
                'periodicAnnounceLocutionId' => $model->getPeriodicAnnounceLocutionId(),
                'periodicAnnounceFrequency' => $model->getPeriodicAnnounceFrequency(),
                'memberCallRest' => $model->getMemberCallRest(),
                'memberCallTimeout' => $model->getMemberCallTimeout(),
     ...
     *
     * @return IvozProvider\\Mapper\\Sql\\DbTable\\Queues
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('IvozProvider\\Mapper\\Sql\\DbTable\\Queues');
        }
     ...
     *
     * @param IvozProvider\Model\Raw\Queues $model The model to delete
    ...
    public function delete(\IvozProvider\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \IvozProvider\Model\Raw\Queues) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\IvozProvider\\Model\\Queues object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\IvozProvider\\Model\\Queues object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('IvozProvider\\Mapper\\Sql\\DbTable\\Queues', $capitalizedFk);
     ...
     */
    public function save(\IvozProvider\Model\Raw\Queues $model, $forceInsert = false)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\IvozProvider\Model\Raw\Queues $model, $useTransaction = true,
    ...
    )
    {
        ...
        try {
            ...
            if ($recursive) {
                ...
                if ($model->getAstQueues(null, null, true) !== null) {
                    $astQueues = $model->getAstQueues();

                    if (!is_array($astQueues)) {

                        $astQueues = array($astQueues);
                    }

                    foreach ($astQueues as $value) {
                        $value->setQueueId($primaryKey)
                              ->saveRecursive(false, $transactionTag);
                    }
                }
     ...
     * @param IvozProvider\Model\Raw\Queues|null $entry The object to load the data into, or null to have one created
     * @return IvozProvider\Model\Raw\Queues The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \IvozProvider\Model\Queues();
        }
        ...
        if (is_array($data)) {
            $entry->setId($data['id'])
                  ...
                  ->setName($data['name'])
                  ->setMaxWaitTime($data['maxWaitTime'])
                  ->setTimeoutLocutionId($data['timeoutLocutionId'])
                  ->setTimeoutTargetType($data['timeoutTargetType'])
                  ->setTimeoutNumberValue($data['timeoutNumberValue'])
                  ->setTimeoutExtensionId($data['timeoutExtensionId'])
                  ->setTimeoutVoiceMailUserId($data['timeoutVoiceMailUserId'])
                  ->setMaxlen($data['maxlen'])
                  ->setFullLocutionId($data['fullLocutionId'])
                  ->setFullTargetType($data['fullTargetType'])
                  ->setFullNumberValue($data['fullNumberValue'])
                  ->setFullExtensionId($data['fullExtensionId'])
                  ->setFullVoiceMailUserId($data['fullVoiceMailUserId'])
                  ->setPeriodicAnnounceLocutionId($data['periodicAnnounceLocutionId'])
                  ->setPeriodicAnnounceFrequency($data['periodicAnnounceFrequency'])
                  ->setMemberCallRest($data['memberCallRest'])
                  ->setMemberCallTimeout($data['memberCallTimeout'])
        ...
        } else if ($data instanceof \IvozProvider\Model\Raw\Queues) {
            $entry->setId($data->getId())
                  ->setCompanyId($data->getCompanyId())
                  ->setName($data->getName())
                  ->setMaxWaitTime($data->getMaxWaitTime())
                  ->setTimeoutLocutionId($data->getTimeoutLocutionId())
                  ->setTimeoutTargetType($data->getTimeoutTargetType())
                  ->setTimeoutNumberValue($data->getTimeoutNumberValue())
                  ->setTimeoutExtensionId($data->getTimeoutExtensionId())
                  ->setTimeoutVoiceMailUserId($data->getTimeoutVoiceMailUserId())
                  ->setMaxlen($data->getMaxlen())
                  ->setFullLocutionId($data->getFullLocutionId())
                  ->setFullTargetType($data->getFullTargetType())
                  ->setFullNumberValue($data->getFullNumberValue())
                  ->setFullExtensionId($data->getFullExtensionId())
                  ->setFullVoiceMailUserId($data->getFullVoiceMailUserId())
                  ->setPeriodicAnnounceLocutionId($data->getPeriodicAnnounceLocutionId())
                  ->setPeriodicAnnounceFrequency($data->getPeriodicAnnounceFrequency())
                  ->setMemberCallRest($data->getMemberCallRest())
                  ->setMemberCallTimeout($data->getMemberCallTimeout())
    ...
    protected function _etagChange()
    {
        ...
        $etag = $etags->findOneByField('table', 'Queues');

        if (empty($etag)) {
            $etag = new \IvozProvider\Model\EtagVersions();
            $etag->setTable('Queues');
        }
