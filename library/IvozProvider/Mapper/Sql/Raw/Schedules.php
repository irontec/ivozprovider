...
/**
 * Data Mapper implementation for IvozProvider\Model\Schedules
 *
 * @package IvozProvider\Mapper\Sql
 * @subpackage Raw
 * @author Luis Felipe Garcia
...
class Schedules extends MapperAbstract
{
    protected $_modelName = 'IvozProvider\\Model\\Schedules';
     ...
     *
     * @param IvozProvider\Model\Raw\Schedules $model
     * @return array
     */
    public function toArray($model, $fields = array())
    {

        if (!$model instanceof \IvozProvider\Model\Raw\Schedules) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \IvozProvider\Model\Raw\Schedules object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\IvozProvider\Model\\Schedules object in toArray for " . get_class($this);
            }
        ...
        if (empty($fields)) {
            $result = array(
                ...
                'monday' => $model->getMonday(),
                'tuesday' => $model->getTuesday(),
                'wednesday' => $model->getWednesday(),
                'thursday' => $model->getThursday(),
                'friday' => $model->getFriday(),
                'saturday' => $model->getSaturday(),
                'sunday' => $model->getSunday(),
            );
        } else {
            $result = array();
     ...
     *
     * @return IvozProvider\\Mapper\\Sql\\DbTable\\Schedules
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('IvozProvider\\Mapper\\Sql\\DbTable\\Schedules');
        }
     ...
     *
     * @param IvozProvider\Model\Raw\Schedules $model The model to delete
    ...
    public function delete(\IvozProvider\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \IvozProvider\Model\Raw\Schedules) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\IvozProvider\\Model\\Schedules object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\IvozProvider\\Model\\Schedules object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('IvozProvider\\Mapper\\Sql\\DbTable\\Schedules', $capitalizedFk);
     ...
     */
    public function save(\IvozProvider\Model\Raw\Schedules $model, $forceInsert = false)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\IvozProvider\Model\Raw\Schedules $model, $useTransaction = true,
            $transactionTag = null, $forceInsert = false)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag, $forceInsert);
    }

    protected function _save(\IvozProvider\Model\Raw\Schedules $model,
     ...
     * @param IvozProvider\Model\Raw\Schedules|null $entry The object to load the data into, or null to have one created
     * @return IvozProvider\Model\Raw\Schedules The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \IvozProvider\Model\Schedules();
        }
        ...
        } else if ($data instanceof \IvozProvider\Model\Raw\Schedules) {
            $entry->setId($data->getId())
                  ->setCompanyId($data->getCompanyId())
                  ->setName($data->getName())
                  ->setTimeIn($data->getTimeIn())
                  ->setTimeout($data->getTimeout())
                  ->setMonday($data->getMonday())
                  ->setTuesday($data->getTuesday())
                  ->setWednesday($data->getWednesday())
                  ->setThursday($data->getThursday())
                  ->setFriday($data->getFriday())
                  ->setSaturday($data->getSaturday())
                  ->setSunday($data->getSunday());
    ...
    protected function _etagChange()
    {
        ...
        $etag = $etags->findOneByField('table', 'Schedules');

        if (empty($etag)) {
            $etag = new \IvozProvider\Model\EtagVersions();
            $etag->setTable('Schedules');
        }
