...
/**
 * Data Mapper implementation for IvozProvider\Model\CallACLPatterns
 *
 * @package IvozProvider\Mapper\Sql
 * @subpackage Raw
 * @author Luis Felipe Garcia
...
class CallACLPatterns extends MapperAbstract
{
    protected $_modelName = 'IvozProvider\\Model\\CallACLPatterns';
     ...
     *
     * @param IvozProvider\Model\Raw\CallACLPatterns $model
     * @return array
     */
    public function toArray($model, $fields = array())
    {

        if (!$model instanceof \IvozProvider\Model\Raw\CallACLPatterns) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \IvozProvider\Model\Raw\CallACLPatterns object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\IvozProvider\Model\\CallACLPatterns object in toArray for " . get_class($this);
            }
     ...
     *
     * @return IvozProvider\\Mapper\\Sql\\DbTable\\CallACLPatterns
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('IvozProvider\\Mapper\\Sql\\DbTable\\CallACLPatterns');
        }
     ...
     *
     * @param IvozProvider\Model\Raw\CallACLPatterns $model The model to delete
    ...
    public function delete(\IvozProvider\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \IvozProvider\Model\Raw\CallACLPatterns) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\IvozProvider\\Model\\CallACLPatterns object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\IvozProvider\\Model\\CallACLPatterns object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('IvozProvider\\Mapper\\Sql\\DbTable\\CallACLPatterns', $capitalizedFk);
     ...
     */
    public function save(\IvozProvider\Model\Raw\CallACLPatterns $model, $forceInsert = false)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\IvozProvider\Model\Raw\CallACLPatterns $model, $useTransaction = true,
            $transactionTag = null, $forceInsert = false)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag, $forceInsert);
    }

    protected function _save(\IvozProvider\Model\Raw\CallACLPatterns $model,
    ...
    )
    {
        ...
        try {
            ...
            if ($recursive) {
                if ($model->getCallACLRelPatterns(null, null, true) !== null) {
                    $callACLRelPatterns = $model->getCallACLRelPatterns();

                    if (!is_array($callACLRelPatterns)) {

                        $callACLRelPatterns = array($callACLRelPatterns);
                    }

                    foreach ($callACLRelPatterns as $value) {
                        $value->setCallACLPatternId($primaryKey)
     ...
     * @param IvozProvider\Model\Raw\CallACLPatterns|null $entry The object to load the data into, or null to have one created
     * @return IvozProvider\Model\Raw\CallACLPatterns The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \IvozProvider\Model\CallACLPatterns();
        }
        ...
        } else if ($data instanceof \IvozProvider\Model\Raw\CallACLPatterns) {
    ...
    protected function _etagChange()
    {
        ...
        $etag = $etags->findOneByField('table', 'CallACLPatterns');

        if (empty($etag)) {
            $etag = new \IvozProvider\Model\EtagVersions();
            $etag->setTable('CallACLPatterns');
        }
