...
class InvoiceTemplates extends MapperAbstract
{
    protected $_modelName = 'IvozProvider\\Model\\InvoiceTemplates';
     ...
     *
     * @param IvozProvider\Model\Raw\InvoiceTemplates $model
     * @return array
     */
    public function toArray($model, $fields = array())
    {

        if (!$model instanceof \IvozProvider\Model\Raw\InvoiceTemplates) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \IvozProvider\Model\Raw\InvoiceTemplates object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\IvozProvider\Model\\InvoiceTemplates object in toArray for " . get_class($this);
            }
     ...
     *
     * @return IvozProvider\\Mapper\\Sql\\DbTable\\InvoiceTemplates
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('IvozProvider\\Mapper\\Sql\\DbTable\\InvoiceTemplates');
        }
     ...
     *
     * @param IvozProvider\Model\Raw\InvoiceTemplates $model The model to delete
    ...
    public function delete(\IvozProvider\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \IvozProvider\Model\Raw\InvoiceTemplates) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\IvozProvider\\Model\\InvoiceTemplates object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\IvozProvider\\Model\\InvoiceTemplates object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('IvozProvider\\Mapper\\Sql\\DbTable\\InvoiceTemplates', $capitalizedFk);
     ...
     */
    public function save(\IvozProvider\Model\Raw\InvoiceTemplates $model, $forceInsert = false)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\IvozProvider\Model\Raw\InvoiceTemplates $model, $useTransaction = true,
            ...
            if ($recursive) {
                if ($model->getInvoices(null, null, true) !== null) {
                    ...
                    foreach ($invoices as $value) {
                        $value->setInvoiceTemplateId($primaryKey)
     ...
     * @param IvozProvider\Model\Raw\InvoiceTemplates|null $entry The object to load the data into, or null to have one created
     * @return IvozProvider\Model\Raw\InvoiceTemplates The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \IvozProvider\Model\InvoiceTemplates();
        }
    ...
    protected function _etagChange()
    {
        ...
        $etag = $etags->findOneByField('table', 'InvoiceTemplates');

        if (empty($etag)) {
            $etag = new \IvozProvider\Model\EtagVersions();
            $etag->setTable('InvoiceTemplates');
        }
