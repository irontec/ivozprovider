...
/**
 * Data Mapper implementation for IvozProvider\Model\ConditionalRoutesConditionsRelCalendars
 *
 * @package IvozProvider\Mapper\Sql
 * @subpackage Raw
 * @author Luis Felipe Garcia
...
class ConditionalRoutesConditionsRelCalendars extends MapperAbstract
{
    protected $_modelName = 'IvozProvider\\Model\\ConditionalRoutesConditionsRelCalendars';
     ...
     *
     * @param IvozProvider\Model\Raw\ConditionalRoutesConditionsRelCalendars $model
     * @return array
     */
    public function toArray($model, $fields = array())
    {

        if (!$model instanceof \IvozProvider\Model\Raw\ConditionalRoutesConditionsRelCalendars) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \IvozProvider\Model\Raw\ConditionalRoutesConditionsRelCalendars object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\IvozProvider\Model\\ConditionalRoutesConditionsRelCalendars object in toArray for " . get_class($this);
            }
        ...
        if (empty($fields)) {
            $result = array(
                ...
                'calendarId' => $model->getCalendarId(),
            );
        } else {
            $result = array();
     ...
     *
     * @return IvozProvider\\Mapper\\Sql\\DbTable\\ConditionalRoutesConditionsRelCalendars
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('IvozProvider\\Mapper\\Sql\\DbTable\\ConditionalRoutesConditionsRelCalendars');
        }
     ...
     *
     * @param IvozProvider\Model\Raw\ConditionalRoutesConditionsRelCalendars $model The model to delete
    ...
    public function delete(\IvozProvider\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \IvozProvider\Model\Raw\ConditionalRoutesConditionsRelCalendars) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\IvozProvider\\Model\\ConditionalRoutesConditionsRelCalendars object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\IvozProvider\\Model\\ConditionalRoutesConditionsRelCalendars object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('IvozProvider\\Mapper\\Sql\\DbTable\\ConditionalRoutesConditionsRelCalendars', $capitalizedFk);
     ...
     */
    public function save(\IvozProvider\Model\Raw\ConditionalRoutesConditionsRelCalendars $model, $forceInsert = false)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\IvozProvider\Model\Raw\ConditionalRoutesConditionsRelCalendars $model, $useTransaction = true,
            $transactionTag = null, $forceInsert = false)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag, $forceInsert);
    }

    protected function _save(\IvozProvider\Model\Raw\ConditionalRoutesConditionsRelCalendars $model,
     ...
     * @param IvozProvider\Model\Raw\ConditionalRoutesConditionsRelCalendars|null $entry The object to load the data into, or null to have one created
     * @return IvozProvider\Model\Raw\ConditionalRoutesConditionsRelCalendars The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \IvozProvider\Model\ConditionalRoutesConditionsRelCalendars();
        }
        ...
        } else if ($data instanceof \IvozProvider\Model\Raw\ConditionalRoutesConditionsRelCalendars) {
    ...
    protected function _etagChange()
    {
        ...
        $etag = $etags->findOneByField('table', 'ConditionalRoutesConditionsRelCalendars');

        if (empty($etag)) {
            $etag = new \IvozProvider\Model\EtagVersions();
            $etag->setTable('ConditionalRoutesConditionsRelCalendars');
        }
