...
class KamTrunksDialplan extends MapperAbstract
{
    protected $_modelName = 'IvozProvider\\Model\\KamTrunksDialplan';
     ...
     *
     * @param IvozProvider\Model\Raw\KamTrunksDialplan $model
     * @return array
     */
    public function toArray($model, $fields = array())
    {

        if (!$model instanceof \IvozProvider\Model\Raw\KamTrunksDialplan) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \IvozProvider\Model\Raw\KamTrunksDialplan object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\IvozProvider\Model\\KamTrunksDialplan object in toArray for " . get_class($this);
            }
     ...
     *
     * @return IvozProvider\\Mapper\\Sql\\DbTable\\KamTrunksDialplan
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('IvozProvider\\Mapper\\Sql\\DbTable\\KamTrunksDialplan');
        }
     ...
     *
     * @param IvozProvider\Model\Raw\KamTrunksDialplan $model The model to delete
    ...
    public function delete(\IvozProvider\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \IvozProvider\Model\Raw\KamTrunksDialplan) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\IvozProvider\\Model\\KamTrunksDialplan object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\IvozProvider\\Model\\KamTrunksDialplan object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('IvozProvider\\Mapper\\Sql\\DbTable\\KamTrunksDialplan', $capitalizedFk);
     ...
     */
    public function save(\IvozProvider\Model\Raw\KamTrunksDialplan $model, $forceInsert = false)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\IvozProvider\Model\Raw\KamTrunksDialplan $model, $useTransaction = true,
     ...
     * @param IvozProvider\Model\Raw\KamTrunksDialplan|null $entry The object to load the data into, or null to have one created
     * @return IvozProvider\Model\Raw\KamTrunksDialplan The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \IvozProvider\Model\KamTrunksDialplan();
        }
        ...
        } else if ($data instanceof \IvozProvider\Model\Raw\KamTrunksDialplan) {
    ...
    protected function _etagChange()
    {
        ...
        $etag = $etags->findOneByField('table', 'KamTrunksDialplan');

        if (empty($etag)) {
            $etag = new \IvozProvider\Model\EtagVersions();
            $etag->setTable('KamTrunksDialplan');
        }
