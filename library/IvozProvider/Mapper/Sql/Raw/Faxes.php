...
class Faxes extends MapperAbstract
{
    protected $_modelName = 'IvozProvider\\Model\\Faxes';
     ...
     *
     * @param IvozProvider\Model\Raw\Faxes $model
     * @return array
     */
    public function toArray($model, $fields = array())
    {

        if (!$model instanceof \IvozProvider\Model\Raw\Faxes) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \IvozProvider\Model\Raw\Faxes object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\IvozProvider\Model\\Faxes object in toArray for " . get_class($this);
            }
     ...
     *
     * @return IvozProvider\\Mapper\\Sql\\DbTable\\Faxes
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('IvozProvider\\Mapper\\Sql\\DbTable\\Faxes');
        }
     ...
     *
     * @param IvozProvider\Model\Raw\Faxes $model The model to delete
    ...
    public function delete(\IvozProvider\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \IvozProvider\Model\Raw\Faxes) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\IvozProvider\\Model\\Faxes object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\IvozProvider\\Model\\Faxes object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('IvozProvider\\Mapper\\Sql\\DbTable\\Faxes', $capitalizedFk);
     ...
     */
    public function save(\IvozProvider\Model\Raw\Faxes $model, $forceInsert = false)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\IvozProvider\Model\Raw\Faxes $model, $useTransaction = true,
            ...
            if ($recursive) {
                if ($model->getDDIs(null, null, true) !== null) {
                    ...
                    foreach ($dDIs as $value) {
                        $value->setFaxId($primaryKey)
                              ->saveRecursive(false, $transactionTag);
                    }
                }

                if ($model->getFaxesInOut(null, null, true) !== null) {
                    $faxesInOut = $model->getFaxesInOut();

                    if (!is_array($faxesInOut)) {

                        $faxesInOut = array($faxesInOut);
                    }

                    foreach ($faxesInOut as $value) {
                        $value->setFaxId($primaryKey)
                              ->saveRecursive(false, $transactionTag);
                    }
                }
     ...
     * @param IvozProvider\Model\Raw\Faxes|null $entry The object to load the data into, or null to have one created
     * @return IvozProvider\Model\Raw\Faxes The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \IvozProvider\Model\Faxes();
        }
    ...
    protected function _etagChange()
    {
        ...
        $etag = $etags->findOneByField('table', 'Faxes');
