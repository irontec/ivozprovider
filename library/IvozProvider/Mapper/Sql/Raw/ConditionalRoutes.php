...
/**
 * Data Mapper implementation for IvozProvider\Model\ConditionalRoutes
 *
 * @package IvozProvider\Mapper\Sql
 * @subpackage Raw
 * @author Luis Felipe Garcia
...
class ConditionalRoutes extends MapperAbstract
{
    protected $_modelName = 'IvozProvider\\Model\\ConditionalRoutes';
     ...
     *
     * @param IvozProvider\Model\Raw\ConditionalRoutes $model
     * @return array
     */
    public function toArray($model, $fields = array())
    {

        if (!$model instanceof \IvozProvider\Model\Raw\ConditionalRoutes) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \IvozProvider\Model\Raw\ConditionalRoutes object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\IvozProvider\Model\\ConditionalRoutes object in toArray for " . get_class($this);
            }
     ...
     *
     * @return IvozProvider\\Mapper\\Sql\\DbTable\\ConditionalRoutes
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('IvozProvider\\Mapper\\Sql\\DbTable\\ConditionalRoutes');
        }
     ...
     *
     * @param IvozProvider\Model\Raw\ConditionalRoutes $model The model to delete
    ...
    public function delete(\IvozProvider\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \IvozProvider\Model\Raw\ConditionalRoutes) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\IvozProvider\\Model\\ConditionalRoutes object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\IvozProvider\\Model\\ConditionalRoutes object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('IvozProvider\\Mapper\\Sql\\DbTable\\ConditionalRoutes', $capitalizedFk);
     ...
     */
    public function save(\IvozProvider\Model\Raw\ConditionalRoutes $model, $forceInsert = false)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\IvozProvider\Model\Raw\ConditionalRoutes $model, $useTransaction = true,
            $transactionTag = null, $forceInsert = false)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag, $forceInsert);
    }

    protected function _save(\IvozProvider\Model\Raw\ConditionalRoutes $model,
            ...
            if ($recursive) {
                if ($model->getConditionalRoutesConditions(null, null, true) !== null) {
                    ...
                    foreach ($conditionalRoutesConditions as $value) {
                        $value->setConditionalRouteId($primaryKey)
                ...
                if ($model->getIVRCustomEntries(null, null, true) !== null) {
                    ...
                    foreach ($iVRCustomEntries as $value) {
                        $value->setTargetConditionalRouteId($primaryKey)
                              ->saveRecursive(false, $transactionTag);
                    }
                }
     ...
     * @param IvozProvider\Model\Raw\ConditionalRoutes|null $entry The object to load the data into, or null to have one created
     * @return IvozProvider\Model\Raw\ConditionalRoutes The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \IvozProvider\Model\ConditionalRoutes();
        }
        ...
        } else if ($data instanceof \IvozProvider\Model\Raw\ConditionalRoutes) {
    ...
    protected function _etagChange()
    {
        ...
        $etag = $etags->findOneByField('table', 'ConditionalRoutes');

        if (empty($etag)) {
            $etag = new \IvozProvider\Model\EtagVersions();
            $etag->setTable('ConditionalRoutes');
        }
