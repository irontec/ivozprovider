...
/**
 * Data Mapper implementation for IvozProvider\Model\AstQueueMembers
 *
 * @package IvozProvider\Mapper\Sql
 * @subpackage Raw
 * @author Luis Felipe Garcia
...
class AstQueueMembers extends MapperAbstract
{
    protected $_modelName = 'IvozProvider\\Model\\AstQueueMembers';
     ...
     *
     * @param IvozProvider\Model\Raw\AstQueueMembers $model
     * @return array
     */
    public function toArray($model, $fields = array())
    {

        if (!$model instanceof \IvozProvider\Model\Raw\AstQueueMembers) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \IvozProvider\Model\Raw\AstQueueMembers object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\IvozProvider\Model\\AstQueueMembers object in toArray for " . get_class($this);
            }
        ...
        if (empty($fields)) {
            $result = array(
                ...
                'queue_name' => $model->getQueueName(),
                'interface' => $model->getInterface(),
                'membername' => $model->getMembername(),
                'state_interface' => $model->getStateInterface(),
                'penalty' => $model->getPenalty(),
                'paused' => $model->getPaused(),
                'queueMemberId' => $model->getQueueMemberId(),
            );
        } else {
            $result = array();
     ...
     *
     * @return IvozProvider\\Mapper\\Sql\\DbTable\\AstQueueMembers
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('IvozProvider\\Mapper\\Sql\\DbTable\\AstQueueMembers');
        }
     ...
     *
     * @param IvozProvider\Model\Raw\AstQueueMembers $model The model to delete
    ...
    public function delete(\IvozProvider\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \IvozProvider\Model\Raw\AstQueueMembers) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\IvozProvider\\Model\\AstQueueMembers object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\IvozProvider\\Model\\AstQueueMembers object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('IvozProvider\\Mapper\\Sql\\DbTable\\AstQueueMembers', $capitalizedFk);
     ...
     */
    public function save(\IvozProvider\Model\Raw\AstQueueMembers $model, $forceInsert = false)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\IvozProvider\Model\Raw\AstQueueMembers $model, $useTransaction = true,
            $transactionTag = null, $forceInsert = false)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag, $forceInsert);
    }

    protected function _save(\IvozProvider\Model\Raw\AstQueueMembers $model,
     ...
     * @param IvozProvider\Model\Raw\AstQueueMembers|null $entry The object to load the data into, or null to have one created
     * @return IvozProvider\Model\Raw\AstQueueMembers The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \IvozProvider\Model\AstQueueMembers();
        }
        ...
        if (is_array($data)) {
            $entry->setUniqueid($data['uniqueid'])
                  ->setQueueName($data['queue_name'])
                  ->setInterface($data['interface'])
                  ->setMembername($data['membername'])
                  ->setStateInterface($data['state_interface'])
                  ->setPenalty($data['penalty'])
                  ->setPaused($data['paused'])
                  ->setQueueMemberId($data['queueMemberId']);
        ...
        } else if ($data instanceof \IvozProvider\Model\Raw\AstQueueMembers) {
            $entry->setUniqueid($data->getUniqueid())
                  ->setQueueName($data->getQueueName())
                  ->setInterface($data->getInterface())
                  ->setMembername($data->getMembername())
                  ->setStateInterface($data->getStateInterface())
                  ->setPenalty($data->getPenalty())
                  ->setPaused($data->getPaused())
                  ->setQueueMemberId($data->getQueueMemberId());
    ...
    protected function _etagChange()
    {
        ...
        $etag = $etags->findOneByField('table', 'AstQueueMembers');

        if (empty($etag)) {
            $etag = new \IvozProvider\Model\EtagVersions();
            $etag->setTable('AstQueueMembers');
        }
