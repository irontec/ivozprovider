...
class PickUpRelUsers extends MapperAbstract
{
    protected $_modelName = 'IvozProvider\\Model\\PickUpRelUsers';
     ...
     *
     * @param IvozProvider\Model\Raw\PickUpRelUsers $model
     * @return array
     */
    public function toArray($model, $fields = array())
    {

        if (!$model instanceof \IvozProvider\Model\Raw\PickUpRelUsers) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \IvozProvider\Model\Raw\PickUpRelUsers object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\IvozProvider\Model\\PickUpRelUsers object in toArray for " . get_class($this);
            }
        ...
        if (empty($fields)) {
            $result = array(
                'id' => $model->getId(),
                'pickUpGroupId' => $model->getPickUpGroupId(),
     ...
     *
     * @return IvozProvider\\Mapper\\Sql\\DbTable\\PickUpRelUsers
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('IvozProvider\\Mapper\\Sql\\DbTable\\PickUpRelUsers');
        }
    ...
    public function delete(\IvozProvider\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \IvozProvider\Model\Raw\PickUpRelUsers) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\IvozProvider\\Model\\PickUpRelUsers object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\IvozProvider\\Model\\PickUpRelUsers object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('IvozProvider\\Mapper\\Sql\\DbTable\\PickUpRelUsers', $capitalizedFk);
     ...
     */
    public function save(\IvozProvider\Model\Raw\PickUpRelUsers $model, $forceInsert = false)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\IvozProvider\Model\Raw\PickUpRelUsers $model, $useTransaction = true,
     ...
     * @param IvozProvider\Model\Raw\PickUpRelUsers|null $entry The object to load the data into, or null to have one created
     * @return IvozProvider\Model\Raw\PickUpRelUsers The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \IvozProvider\Model\PickUpRelUsers();
        }
        ...
        if (is_array($data)) {
            $entry->setId($data['id'])
                  ->setPickUpGroupId($data['pickUpGroupId'])
        ...
        } else if ($data instanceof \IvozProvider\Model\Raw\PickUpRelUsers) {
            $entry->setId($data->getId())
                  ->setPickUpGroupId($data->getPickUpGroupId())
    ...
    protected function _etagChange()
    {
        ...
        $etag = $etags->findOneByField('table', 'PickUpRelUsers');

        if (empty($etag)) {
            $etag = new \IvozProvider\Model\EtagVersions();
            $etag->setTable('PickUpRelUsers');
        }
