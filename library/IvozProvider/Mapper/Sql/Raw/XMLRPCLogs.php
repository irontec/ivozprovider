...
/**
 * Data Mapper implementation for IvozProvider\Model\XMLRPCLogs
 *
 * @package IvozProvider\Mapper\Sql
 * @subpackage Raw
 * @author Luis Felipe Garcia
...
class XMLRPCLogs extends MapperAbstract
{
    protected $_modelName = 'IvozProvider\\Model\\XMLRPCLogs';
     ...
     *
     * @param IvozProvider\Model\Raw\XMLRPCLogs $model
     * @return array
     */
    public function toArray($model, $fields = array())
    {

        if (!$model instanceof \IvozProvider\Model\Raw\XMLRPCLogs) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \IvozProvider\Model\Raw\XMLRPCLogs object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\IvozProvider\Model\\XMLRPCLogs object in toArray for " . get_class($this);
            }
        ...
        if (empty($fields)) {
            $result = array(
                ...
                'mapperName' => $model->getMapperName(),
                'startDate' => $model->getStartDate(),
                'execDate' => $model->getExecDate(),
     ...
     *
     * @return IvozProvider\\Mapper\\Sql\\DbTable\\XMLRPCLogs
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('IvozProvider\\Mapper\\Sql\\DbTable\\XMLRPCLogs');
        }
     ...
     *
     * @param IvozProvider\Model\Raw\XMLRPCLogs $model The model to delete
    ...
    public function delete(\IvozProvider\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \IvozProvider\Model\Raw\XMLRPCLogs) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\IvozProvider\\Model\\XMLRPCLogs object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\IvozProvider\\Model\\XMLRPCLogs object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('IvozProvider\\Mapper\\Sql\\DbTable\\XMLRPCLogs', $capitalizedFk);
     ...
     */
    public function save(\IvozProvider\Model\Raw\XMLRPCLogs $model, $forceInsert = false)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\IvozProvider\Model\Raw\XMLRPCLogs $model, $useTransaction = true,
            $transactionTag = null, $forceInsert = false)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag, $forceInsert);
    }

    protected function _save(\IvozProvider\Model\Raw\XMLRPCLogs $model,
     ...
     * @param IvozProvider\Model\Raw\XMLRPCLogs|null $entry The object to load the data into, or null to have one created
     * @return IvozProvider\Model\Raw\XMLRPCLogs The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \IvozProvider\Model\XMLRPCLogs();
        }
        ...
        } else if ($data instanceof \IvozProvider\Model\Raw\XMLRPCLogs) {
            $entry->setId($data->getId())
                  ->setProxy($data->getProxy())
                  ->setModule($data->getModule())
                  ->setMethod($data->getMethod())
                  ->setMapperName($data->getMapperName())
                  ->setStartDate($data->getStartDate())
                  ->setExecDate($data->getExecDate())
    ...
    protected function _etagChange()
    {
        ...
        $etag = $etags->findOneByField('table', 'XMLRPCLogs');

        if (empty($etag)) {
            $etag = new \IvozProvider\Model\EtagVersions();
            $etag->setTable('XMLRPCLogs');
        }
