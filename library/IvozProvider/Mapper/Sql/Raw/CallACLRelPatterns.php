...
/**
 * Data Mapper implementation for IvozProvider\Model\CallACLRelPatterns
 *
 * @package IvozProvider\Mapper\Sql
 * @subpackage Raw
 * @author Luis Felipe Garcia
...
class CallACLRelPatterns extends MapperAbstract
{
    protected $_modelName = 'IvozProvider\\Model\\CallACLRelPatterns';
     ...
     *
     * @param IvozProvider\Model\Raw\CallACLRelPatterns $model
     * @return array
     */
    public function toArray($model, $fields = array())
    {

        if (!$model instanceof \IvozProvider\Model\Raw\CallACLRelPatterns) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \IvozProvider\Model\Raw\CallACLRelPatterns object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\IvozProvider\Model\\CallACLRelPatterns object in toArray for " . get_class($this);
            }
        ...
        if (empty($fields)) {
            $result = array(
                ...
                'CallACLPatternId' => $model->getCallACLPatternId(),
     ...
     *
     * @return IvozProvider\\Mapper\\Sql\\DbTable\\CallACLRelPatterns
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('IvozProvider\\Mapper\\Sql\\DbTable\\CallACLRelPatterns');
        }
     ...
     *
     * @param IvozProvider\Model\Raw\CallACLRelPatterns $model The model to delete
    ...
    public function delete(\IvozProvider\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \IvozProvider\Model\Raw\CallACLRelPatterns) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\IvozProvider\\Model\\CallACLRelPatterns object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\IvozProvider\\Model\\CallACLRelPatterns object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('IvozProvider\\Mapper\\Sql\\DbTable\\CallACLRelPatterns', $capitalizedFk);
     ...
     */
    public function save(\IvozProvider\Model\Raw\CallACLRelPatterns $model, $forceInsert = false)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\IvozProvider\Model\Raw\CallACLRelPatterns $model, $useTransaction = true,
            $transactionTag = null, $forceInsert = false)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag, $forceInsert);
    }

    protected function _save(\IvozProvider\Model\Raw\CallACLRelPatterns $model,
     ...
     * @param IvozProvider\Model\Raw\CallACLRelPatterns|null $entry The object to load the data into, or null to have one created
     * @return IvozProvider\Model\Raw\CallACLRelPatterns The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \IvozProvider\Model\CallACLRelPatterns();
        }
        ...
        if (is_array($data)) {
            $entry->setId($data['id'])
                  ...
                  ->setCallACLPatternId($data['CallACLPatternId'])
        ...
        } else if ($data instanceof \IvozProvider\Model\Raw\CallACLRelPatterns) {
            $entry->setId($data->getId())
                  ->setCallACLId($data->getCallACLId())
                  ->setCallACLPatternId($data->getCallACLPatternId())
    ...
    protected function _etagChange()
    {
        ...
        $etag = $etags->findOneByField('table', 'CallACLRelPatterns');

        if (empty($etag)) {
            $etag = new \IvozProvider\Model\EtagVersions();
            $etag->setTable('CallACLRelPatterns');
        }
