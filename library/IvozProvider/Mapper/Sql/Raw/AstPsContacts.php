...
class AstPsContacts extends MapperAbstract
{
    protected $_modelName = 'IvozProvider\\Model\\AstPsContacts';
     ...
     *
     * @param IvozProvider\Model\Raw\AstPsContacts $model
     * @return array
     */
    public function toArray($model, $fields = array())
    {

        if (!$model instanceof \IvozProvider\Model\Raw\AstPsContacts) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \IvozProvider\Model\Raw\AstPsContacts object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\IvozProvider\Model\\AstPsContacts object in toArray for " . get_class($this);
            }
        ...
        if (empty($fields)) {
            $result = array(
                ...
                'uri' => $model->getUri(),
                'expiration_time' => $model->getExpirationTime(),
                'qualify_frequency' => $model->getQualifyFrequency(),
                'qualify_timeout' => $model->getQualifyTimeout(),
     ...
     *
     * @return IvozProvider\\Mapper\\Sql\\DbTable\\AstPsContacts
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('IvozProvider\\Mapper\\Sql\\DbTable\\AstPsContacts');
        }
     ...
     *
     * @param IvozProvider\Model\Raw\AstPsContacts $model The model to delete
    ...
    public function delete(\IvozProvider\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \IvozProvider\Model\Raw\AstPsContacts) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\IvozProvider\\Model\\AstPsContacts object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\IvozProvider\\Model\\AstPsContacts object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('IvozProvider\\Mapper\\Sql\\DbTable\\AstPsContacts', $capitalizedFk);
     ...
     */
    public function save(\IvozProvider\Model\Raw\AstPsContacts $model, $forceInsert = false)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\IvozProvider\Model\Raw\AstPsContacts $model, $useTransaction = true,
            $transactionTag = null, $forceInsert = false)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag, $forceInsert);
    }

    protected function _save(\IvozProvider\Model\Raw\AstPsContacts $model,
     ...
     * @param IvozProvider\Model\Raw\AstPsContacts|null $entry The object to load the data into, or null to have one created
     * @return IvozProvider\Model\Raw\AstPsContacts The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \IvozProvider\Model\AstPsContacts();
        }
        ...
        if (is_array($data)) {
            $entry->setSorceryId($data['sorcery_id'])
                  ...
                  ->setQualifyTimeout($data['qualify_timeout'])
        ...
        } else if ($data instanceof \IvozProvider\Model\Raw\AstPsContacts) {
            $entry->setSorceryId($data->getSorceryId())
                  ->setUri($data->getUri())
                  ->setExpirationTime($data->getExpirationTime())
                  ->setQualifyFrequency($data->getQualifyFrequency())
                  ->setQualifyTimeout($data->getQualifyTimeout())
