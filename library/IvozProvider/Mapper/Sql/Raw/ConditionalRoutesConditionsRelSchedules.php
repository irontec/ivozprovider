...
/**
 * Data Mapper implementation for IvozProvider\Model\ConditionalRoutesConditionsRelSchedules
 *
 * @package IvozProvider\Mapper\Sql
 * @subpackage Raw
 * @author Luis Felipe Garcia
...
class ConditionalRoutesConditionsRelSchedules extends MapperAbstract
{
    protected $_modelName = 'IvozProvider\\Model\\ConditionalRoutesConditionsRelSchedules';
     ...
     *
     * @param IvozProvider\Model\Raw\ConditionalRoutesConditionsRelSchedules $model
     * @return array
     */
    public function toArray($model, $fields = array())
    {

        if (!$model instanceof \IvozProvider\Model\Raw\ConditionalRoutesConditionsRelSchedules) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \IvozProvider\Model\Raw\ConditionalRoutesConditionsRelSchedules object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\IvozProvider\Model\\ConditionalRoutesConditionsRelSchedules object in toArray for " . get_class($this);
            }
     ...
     *
     * @return IvozProvider\\Mapper\\Sql\\DbTable\\ConditionalRoutesConditionsRelSchedules
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('IvozProvider\\Mapper\\Sql\\DbTable\\ConditionalRoutesConditionsRelSchedules');
        }
     ...
     *
     * @param IvozProvider\Model\Raw\ConditionalRoutesConditionsRelSchedules $model The model to delete
    ...
    public function delete(\IvozProvider\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \IvozProvider\Model\Raw\ConditionalRoutesConditionsRelSchedules) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\IvozProvider\\Model\\ConditionalRoutesConditionsRelSchedules object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\IvozProvider\\Model\\ConditionalRoutesConditionsRelSchedules object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('IvozProvider\\Mapper\\Sql\\DbTable\\ConditionalRoutesConditionsRelSchedules', $capitalizedFk);
     ...
     */
    public function save(\IvozProvider\Model\Raw\ConditionalRoutesConditionsRelSchedules $model, $forceInsert = false)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\IvozProvider\Model\Raw\ConditionalRoutesConditionsRelSchedules $model, $useTransaction = true,
            $transactionTag = null, $forceInsert = false)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag, $forceInsert);
    }

    protected function _save(\IvozProvider\Model\Raw\ConditionalRoutesConditionsRelSchedules $model,
     ...
     * @param IvozProvider\Model\Raw\ConditionalRoutesConditionsRelSchedules|null $entry The object to load the data into, or null to have one created
     * @return IvozProvider\Model\Raw\ConditionalRoutesConditionsRelSchedules The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \IvozProvider\Model\ConditionalRoutesConditionsRelSchedules();
        }
        ...
        } else if ($data instanceof \IvozProvider\Model\Raw\ConditionalRoutesConditionsRelSchedules) {
    ...
    protected function _etagChange()
    {
        ...
        $etag = $etags->findOneByField('table', 'ConditionalRoutesConditionsRelSchedules');

        if (empty($etag)) {
            $etag = new \IvozProvider\Model\EtagVersions();
            $etag->setTable('ConditionalRoutesConditionsRelSchedules');
        }
