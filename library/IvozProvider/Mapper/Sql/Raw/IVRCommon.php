...
/**
 * Data Mapper implementation for IvozProvider\Model\IVRCommon
 *
 * @package IvozProvider\Mapper\Sql
 * @subpackage Raw
 * @author Luis Felipe Garcia
...
class IVRCommon extends MapperAbstract
{
    protected $_modelName = 'IvozProvider\\Model\\IVRCommon';
     ...
     *
     * @param IvozProvider\Model\Raw\IVRCommon $model
     * @return array
     */
    public function toArray($model, $fields = array())
    {

        if (!$model instanceof \IvozProvider\Model\Raw\IVRCommon) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \IvozProvider\Model\Raw\IVRCommon object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\IvozProvider\Model\\IVRCommon object in toArray for " . get_class($this);
            }
        ...
        if (empty($fields)) {
            $result = array(
                ...
                'errorTargetType' => $model->getErrorTargetType(),
                'errorNumberValue' => $model->getErrorNumberValue(),
                'errorExtensionId' => $model->getErrorExtensionId(),
                'errorVoiceMailUserId' => $model->getErrorVoiceMailUserId(),
            );
        } else {
            $result = array();
     ...
     *
     * @return IvozProvider\\Mapper\\Sql\\DbTable\\IVRCommon
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('IvozProvider\\Mapper\\Sql\\DbTable\\IVRCommon');
        }
     ...
     *
     * @param IvozProvider\Model\Raw\IVRCommon $model The model to delete
    ...
    public function delete(\IvozProvider\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \IvozProvider\Model\Raw\IVRCommon) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\IvozProvider\\Model\\IVRCommon object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\IvozProvider\\Model\\IVRCommon object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('IvozProvider\\Mapper\\Sql\\DbTable\\IVRCommon', $capitalizedFk);
     ...
     */
    public function save(\IvozProvider\Model\Raw\IVRCommon $model, $forceInsert = false)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\IvozProvider\Model\Raw\IVRCommon $model, $useTransaction = true,
            $transactionTag = null, $forceInsert = false)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag, $forceInsert);
    }

    protected function _save(\IvozProvider\Model\Raw\IVRCommon $model,
            ...
            if ($recursive) {
                if ($model->getDDIs(null, null, true) !== null) {
                    ...
                    foreach ($dDIs as $value) {
                        $value->setIVRCommonId($primaryKey)
     ...
     * @param IvozProvider\Model\Raw\IVRCommon|null $entry The object to load the data into, or null to have one created
     * @return IvozProvider\Model\Raw\IVRCommon The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \IvozProvider\Model\IVRCommon();
        }
        ...
        if (is_array($data)) {
            $entry->setId($data['id'])
                  ...
                  ->setErrorTargetType($data['errorTargetType'])
                  ->setErrorNumberValue($data['errorNumberValue'])
                  ->setErrorExtensionId($data['errorExtensionId'])
                  ->setErrorVoiceMailUserId($data['errorVoiceMailUserId']);
        ...
        } else if ($data instanceof \IvozProvider\Model\Raw\IVRCommon) {
            $entry->setId($data->getId())
                  ...
                  ->setErrorTargetType($data->getErrorTargetType())
                  ->setErrorNumberValue($data->getErrorNumberValue())
                  ->setErrorExtensionId($data->getErrorExtensionId())
                  ->setErrorVoiceMailUserId($data->getErrorVoiceMailUserId());
