...
class KamTrunksUacreg extends MapperAbstract
{
    protected $_modelName = 'IvozProvider\\Model\\KamTrunksUacreg';
     ...
     *
     * @param IvozProvider\Model\Raw\KamTrunksUacreg $model
     * @return array
     */
    public function toArray($model, $fields = array())
    {

        if (!$model instanceof \IvozProvider\Model\Raw\KamTrunksUacreg) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \IvozProvider\Model\Raw\KamTrunksUacreg object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\IvozProvider\Model\\KamTrunksUacreg object in toArray for " . get_class($this);
            }
        ...
        if (empty($fields)) {
            $result = array(
                'id' => $model->getId(),
                'l_uuid' => $model->getLUuid(),
                'l_username' => $model->getLUsername(),
                'l_domain' => $model->getLDomain(),
                'r_username' => $model->getRUsername(),
                'r_domain' => $model->getRDomain(),
                'realm' => $model->getRealm(),
                'auth_username' => $model->getAuthUsername(),
     ...
     *
     * @return IvozProvider\\Mapper\\Sql\\DbTable\\KamTrunksUacreg
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('IvozProvider\\Mapper\\Sql\\DbTable\\KamTrunksUacreg');
        }
     ...
     *
     * @param IvozProvider\Model\Raw\KamTrunksUacreg $model The model to delete
    ...
    public function delete(\IvozProvider\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \IvozProvider\Model\Raw\KamTrunksUacreg) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\IvozProvider\\Model\\KamTrunksUacreg object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\IvozProvider\\Model\\KamTrunksUacreg object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('IvozProvider\\Mapper\\Sql\\DbTable\\KamTrunksUacreg', $capitalizedFk);
     ...
     */
    public function save(\IvozProvider\Model\Raw\KamTrunksUacreg $model, $forceInsert = false)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\IvozProvider\Model\Raw\KamTrunksUacreg $model, $useTransaction = true,
     ...
     * @param IvozProvider\Model\Raw\KamTrunksUacreg|null $entry The object to load the data into, or null to have one created
     * @return IvozProvider\Model\Raw\KamTrunksUacreg The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \IvozProvider\Model\KamTrunksUacreg();
        }
        ...
        if (is_array($data)) {
            $entry->setId($data['id'])
                  ...
                  ->setLUsername($data['l_username'])
                  ->setLDomain($data['l_domain'])
                  ->setRUsername($data['r_username'])
                  ->setRDomain($data['r_domain'])
                  ->setRealm($data['realm'])
            ...
            $entry->setId($data->getId())
                  ->setLUuid($data->getLUuid())
                  ->setLUsername($data->getLUsername())
                  ->setLDomain($data->getLDomain())
                  ->setRUsername($data->getRUsername())
                  ->setRDomain($data->getRDomain())
                  ->setRealm($data->getRealm())
                  ->setAuthUsername($data->getAuthUsername())
    ...
    protected function _etagChange()
    {
        ...
        $etag = $etags->findOneByField('table', 'KamTrunksUacreg');

        if (empty($etag)) {
            $etag = new \IvozProvider\Model\EtagVersions();
            $etag->setTable('KamTrunksUacreg');
        }
