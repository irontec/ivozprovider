...
/**
 * Data Mapper implementation for Oasis\Model\KamUsersDomainAttrs
 *
 * @package Oasis\Mapper\Sql
 * @subpackage Raw
 * @author Luis Felipe Garcia
...
{
    protected $_modelName = 'Oasis\\Model\\KamUsersDomainAttrs';
     ...
     *
     * @param Oasis\Model\Raw\KamUsersDomainAttrs $model
     * @return array
     */
    public function toArray($model)
    {
        if (!$model instanceof \Oasis\Model\Raw\KamUsersDomainAttrs) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Oasis\Model\Raw\KamUsersDomainAttrs object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Oasis\Model\\KamUsersDomainAttrs object in toArray for " . get_class($this);
            }
     ...
     *
     * @return Oasis\\Mapper\\Sql\\DbTable\\KamUsersDomainAttrs
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Oasis\\Mapper\\Sql\\DbTable\\KamUsersDomainAttrs');
        }
     ...
     *
     * @param Oasis\Model\Raw\KamUsersDomainAttrs $model The model to delete
    ...
    public function delete(\Oasis\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Oasis\Model\Raw\KamUsersDomainAttrs) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Oasis\\Model\\KamUsersDomainAttrs object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Oasis\\Model\\KamUsersDomainAttrs object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Oasis\\Mapper\\Sql\\DbTable\\KamUsersDomainAttrs', $capitalizedFk);
     ...
     */
    public function save(\Oasis\Model\Raw\KamUsersDomainAttrs $model)
    {
     ...
     * @return boolean If the save action was successful
     */
    public function saveRecursive(\Oasis\Model\Raw\KamUsersDomainAttrs $model, $useTransaction = true, $transactionTag = null)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag);
    }

    protected function _save(\Oasis\Model\Raw\KamUsersDomainAttrs $model,
     ...
     * @param Oasis\Model\Raw\KamUsersDomainAttrs|null $entry The object to load the data into, or null to have one created
     * @return Oasis\Model\Raw\KamUsersDomainAttrs The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Oasis\Model\KamUsersDomainAttrs();
        }
        ...
        } else if ($data instanceof \Oasis\Model\Raw\KamUsersDomainAttrs) {
