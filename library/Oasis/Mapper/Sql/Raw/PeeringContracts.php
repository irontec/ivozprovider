...
{
    protected $_modelName = 'Oasis\\Model\\PeeringContracts';
     ...
     *
     * @param Oasis\Model\Raw\PeeringContracts $model
     * @return array
     */
    public function toArray($model, $fields = array())
    {

        if (!$model instanceof \Oasis\Model\Raw\PeeringContracts) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Oasis\Model\Raw\PeeringContracts object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Oasis\Model\\PeeringContracts object in toArray for " . get_class($this);
            }
     ...
     *
     * @return Oasis\\Mapper\\Sql\\DbTable\\PeeringContracts
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Oasis\\Mapper\\Sql\\DbTable\\PeeringContracts');
        }
     ...
     *
     * @param Oasis\Model\Raw\PeeringContracts $model The model to delete
    ...
    public function delete(\Oasis\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Oasis\Model\Raw\PeeringContracts) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Oasis\\Model\\PeeringContracts object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Oasis\\Model\\PeeringContracts object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Oasis\\Mapper\\Sql\\DbTable\\PeeringContracts', $capitalizedFk);
     ...
     */
    public function save(\Oasis\Model\Raw\PeeringContracts $model, $forceInsert = false)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Oasis\Model\Raw\PeeringContracts $model, $useTransaction = true,
     ...
     * @param Oasis\Model\Raw\PeeringContracts|null $entry The object to load the data into, or null to have one created
     * @return Oasis\Model\Raw\PeeringContracts The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Oasis\Model\PeeringContracts();
        }
        ...
        } else if ($data instanceof \Oasis\Model\Raw\PeeringContracts) {
