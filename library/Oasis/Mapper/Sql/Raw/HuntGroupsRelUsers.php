...
/**
 * Data Mapper implementation for Oasis\Model\HuntGroupsRelUsers
 *
 * @package Oasis\Mapper\Sql
 * @subpackage Raw
 * @author Luis Felipe Garcia
...
{
    protected $_modelName = 'Oasis\\Model\\HuntGroupsRelUsers';
     ...
     *
     * @param Oasis\Model\Raw\HuntGroupsRelUsers $model
     * @return array
     */
    public function toArray($model, $fields = array())
    {

        if (!$model instanceof \Oasis\Model\Raw\HuntGroupsRelUsers) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Oasis\Model\Raw\HuntGroupsRelUsers object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Oasis\Model\\HuntGroupsRelUsers object in toArray for " . get_class($this);
            }
     ...
     *
     * @return Oasis\\Mapper\\Sql\\DbTable\\HuntGroupsRelUsers
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Oasis\\Mapper\\Sql\\DbTable\\HuntGroupsRelUsers');
        }
     ...
     *
     * @param Oasis\Model\Raw\HuntGroupsRelUsers $model The model to delete
    ...
    public function delete(\Oasis\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Oasis\Model\Raw\HuntGroupsRelUsers) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Oasis\\Model\\HuntGroupsRelUsers object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Oasis\\Model\\HuntGroupsRelUsers object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Oasis\\Mapper\\Sql\\DbTable\\HuntGroupsRelUsers', $capitalizedFk);
     ...
     */
    public function save(\Oasis\Model\Raw\HuntGroupsRelUsers $model, $forceInsert = false)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Oasis\Model\Raw\HuntGroupsRelUsers $model, $useTransaction = true,
            $transactionTag = null, $forceInsert = false)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag, $forceInsert);
    }

    protected function _save(\Oasis\Model\Raw\HuntGroupsRelUsers $model,
     ...
     * @param Oasis\Model\Raw\HuntGroupsRelUsers|null $entry The object to load the data into, or null to have one created
     * @return Oasis\Model\Raw\HuntGroupsRelUsers The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Oasis\Model\HuntGroupsRelUsers();
        }
        ...
        } else if ($data instanceof \Oasis\Model\Raw\HuntGroupsRelUsers) {
