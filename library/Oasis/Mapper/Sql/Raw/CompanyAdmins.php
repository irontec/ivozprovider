...
/**
 * Data Mapper implementation for Oasis\Model\CompanyAdmins
 *
 * @package Oasis\Mapper\Sql
 * @subpackage Raw
 * @author Luis Felipe Garcia
...
{
    protected $_modelName = 'Oasis\\Model\\CompanyAdmins';
     ...
     *
     * @param Oasis\Model\Raw\CompanyAdmins $model
     * @return array
     */
    public function toArray($model, $fields = array())
    {

        if (!$model instanceof \Oasis\Model\Raw\CompanyAdmins) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Oasis\Model\Raw\CompanyAdmins object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Oasis\Model\\CompanyAdmins object in toArray for " . get_class($this);
            }
     ...
     *
     * @return Oasis\\Mapper\\Sql\\DbTable\\CompanyAdmins
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Oasis\\Mapper\\Sql\\DbTable\\CompanyAdmins');
        }
     ...
     *
     * @param Oasis\Model\Raw\CompanyAdmins $model The model to delete
    ...
    public function delete(\Oasis\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Oasis\Model\Raw\CompanyAdmins) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Oasis\\Model\\CompanyAdmins object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Oasis\\Model\\CompanyAdmins object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Oasis\\Mapper\\Sql\\DbTable\\CompanyAdmins', $capitalizedFk);
     ...
     */
    public function save(\Oasis\Model\Raw\CompanyAdmins $model, $forceInsert = false)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Oasis\Model\Raw\CompanyAdmins $model, $useTransaction = true,
            $transactionTag = null, $forceInsert = false)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag, $forceInsert);
    }

    protected function _save(\Oasis\Model\Raw\CompanyAdmins $model,
     ...
     * @param Oasis\Model\Raw\CompanyAdmins|null $entry The object to load the data into, or null to have one created
     * @return Oasis\Model\Raw\CompanyAdmins The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Oasis\Model\CompanyAdmins();
        }
        ...
        } else if ($data instanceof \Oasis\Model\Raw\CompanyAdmins) {
