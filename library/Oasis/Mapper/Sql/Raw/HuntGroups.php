...
/**
 * Data Mapper implementation for Oasis\Model\HuntGroups
 *
 * @package Oasis\Mapper\Sql
 * @subpackage Raw
 * @author Luis Felipe Garcia
...
class HuntGroups extends MapperAbstract
{
    protected $_modelName = 'Oasis\\Model\\HuntGroups';
     ...
     *
     * @param Oasis\Model\Raw\HuntGroups $model
     * @return array
     */
    public function toArray($model, $fields = array())
    {

        if (!$model instanceof \Oasis\Model\Raw\HuntGroups) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Oasis\Model\Raw\HuntGroups object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Oasis\Model\\HuntGroups object in toArray for " . get_class($this);
            }
     ...
     *
     * @return Oasis\\Mapper\\Sql\\DbTable\\HuntGroups
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Oasis\\Mapper\\Sql\\DbTable\\HuntGroups');
        }
     ...
     *
     * @param Oasis\Model\Raw\HuntGroups $model The model to delete
    ...
    public function delete(\Oasis\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Oasis\Model\Raw\HuntGroups) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Oasis\\Model\\HuntGroups object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Oasis\\Model\\HuntGroups object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Oasis\\Mapper\\Sql\\DbTable\\HuntGroups', $capitalizedFk);
     ...
     */
    public function save(\Oasis\Model\Raw\HuntGroups $model, $forceInsert = false)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Oasis\Model\Raw\HuntGroups $model, $useTransaction = true,
            $transactionTag = null, $forceInsert = false)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag, $forceInsert);
    }

    protected function _save(\Oasis\Model\Raw\HuntGroups $model,
     ...
     * @param Oasis\Model\Raw\HuntGroups|null $entry The object to load the data into, or null to have one created
     * @return Oasis\Model\Raw\HuntGroups The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Oasis\Model\HuntGroups();
        }
        ...
        } else if ($data instanceof \Oasis\Model\Raw\HuntGroups) {
