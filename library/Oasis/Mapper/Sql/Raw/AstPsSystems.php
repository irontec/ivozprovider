...
/**
 * Data Mapper implementation for Oasis\Model\AstPsSystems
 *
 * @package Oasis\Mapper\Sql
 * @subpackage Raw
 * @author Luis Felipe Garcia
...
class AstPsSystems extends MapperAbstract
{
    protected $_modelName = 'Oasis\\Model\\AstPsSystems';
     ...
     *
     * @param Oasis\Model\Raw\AstPsSystems $model
     * @return array
     */
    public function toArray($model)
    {
        if (!$model instanceof \Oasis\Model\Raw\AstPsSystems) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Oasis\Model\Raw\AstPsSystems object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Oasis\Model\\AstPsSystems object in toArray for " . get_class($this);
            }
        ...
        $result = array(
            ...
            'timer_t1' => $model->getTimerT1(),
            'timer_b' => $model->getTimerB(),
            'compact_headers' => $model->getCompactHeaders(),
            'threadpool_initial_size' => $model->getThreadpoolInitialSize(),
            'threadpool_auto_increment' => $model->getThreadpoolAutoIncrement(),
            'threadpool_idle_timeout' => $model->getThreadpoolIdleTimeout(),
            'threadpool_max_size' => $model->getThreadpoolMaxSize(),
        );
     ...
     *
     * @return Oasis\\Mapper\\Sql\\DbTable\\AstPsSystems
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Oasis\\Mapper\\Sql\\DbTable\\AstPsSystems');
        }
     ...
     *
     * @param Oasis\Model\Raw\AstPsSystems $model The model to delete
    ...
    public function delete(\Oasis\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Oasis\Model\Raw\AstPsSystems) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Oasis\\Model\\AstPsSystems object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Oasis\\Model\\AstPsSystems object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Oasis\\Mapper\\Sql\\DbTable\\AstPsSystems', $capitalizedFk);
     ...
     */
    public function save(\Oasis\Model\Raw\AstPsSystems $model)
    {
     ...
     * @return boolean If the save action was successful
     */
    public function saveRecursive(\Oasis\Model\Raw\AstPsSystems $model, $useTransaction = true, $transactionTag = null)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag);
    }

    protected function _save(\Oasis\Model\Raw\AstPsSystems $model,
     ...
     * @param Oasis\Model\Raw\AstPsSystems|null $entry The object to load the data into, or null to have one created
     * @return Oasis\Model\Raw\AstPsSystems The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Oasis\Model\AstPsSystems();
        }
        ...
        if (is_array($data)) {
            $entry->setSorceryId($data['sorcery_id'])
                  ->setTimerT1($data['timer_t1'])
                  ->setTimerB($data['timer_b'])
                  ->setCompactHeaders($data['compact_headers'])
                  ->setThreadpoolInitialSize($data['threadpool_initial_size'])
                  ->setThreadpoolAutoIncrement($data['threadpool_auto_increment'])
                  ->setThreadpoolIdleTimeout($data['threadpool_idle_timeout'])
                  ->setThreadpoolMaxSize($data['threadpool_max_size']);
        ...
        } else if ($data instanceof \Oasis\Model\Raw\AstPsSystems) {
            $entry->setSorceryId($data->getSorceryId())
                  ->setTimerT1($data->getTimerT1())
                  ->setTimerB($data->getTimerB())
                  ->setCompactHeaders($data->getCompactHeaders())
                  ->setThreadpoolInitialSize($data->getThreadpoolInitialSize())
                  ->setThreadpoolAutoIncrement($data->getThreadpoolAutoIncrement())
                  ->setThreadpoolIdleTimeout($data->getThreadpoolIdleTimeout())
                  ->setThreadpoolMaxSize($data->getThreadpoolMaxSize());
