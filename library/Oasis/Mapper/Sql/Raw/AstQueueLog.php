...
/**
 * Data Mapper implementation for Oasis\Model\AstQueueLog
 *
 * @package Oasis\Mapper\Sql
 * @subpackage Raw
 * @author Luis Felipe Garcia
...
class AstQueueLog extends MapperAbstract
{
    protected $_modelName = 'Oasis\\Model\\AstQueueLog';
     ...
     *
     * @param Oasis\Model\Raw\AstQueueLog $model
     * @return array
     */
    public function toArray($model, $fields = array())
    {

        if (!$model instanceof \Oasis\Model\Raw\AstQueueLog) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Oasis\Model\Raw\AstQueueLog object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Oasis\Model\\AstQueueLog object in toArray for " . get_class($this);
            }
        ...
        if (empty($fields)) {
            $result = array(
                ...
                'data2' => $model->getData2(),
                'data3' => $model->getData3(),
                'data4' => $model->getData4(),
                'data5' => $model->getData5(),
            );
        } else {
            $result = array();
     ...
     *
     * @return Oasis\\Mapper\\Sql\\DbTable\\AstQueueLog
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Oasis\\Mapper\\Sql\\DbTable\\AstQueueLog');
        }
     ...
     *
     * @param Oasis\Model\Raw\AstQueueLog $model The model to delete
    ...
    public function delete(\Oasis\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Oasis\Model\Raw\AstQueueLog) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Oasis\\Model\\AstQueueLog object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Oasis\\Model\\AstQueueLog object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Oasis\\Mapper\\Sql\\DbTable\\AstQueueLog', $capitalizedFk);
     ...
     */
    public function save(\Oasis\Model\Raw\AstQueueLog $model, $forceInsert = false)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Oasis\Model\Raw\AstQueueLog $model, $useTransaction = true,
            $transactionTag = null, $forceInsert = false)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag, $forceInsert);
    }

    protected function _save(\Oasis\Model\Raw\AstQueueLog $model,
     ...
     * @param Oasis\Model\Raw\AstQueueLog|null $entry The object to load the data into, or null to have one created
     * @return Oasis\Model\Raw\AstQueueLog The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Oasis\Model\AstQueueLog();
        }
