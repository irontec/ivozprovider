...
/**
 * Data Mapper implementation for Oasis\Model\KamUsersLocationAttrs
 *
 * @package Oasis\Mapper\Sql
 * @subpackage Raw
 * @author Luis Felipe Garcia
...
class KamUsersLocationAttrs extends MapperAbstract
{
    protected $_modelName = 'Oasis\\Model\\KamUsersLocationAttrs';
     ...
     *
     * @param Oasis\Model\Raw\KamUsersLocationAttrs $model
     * @return array
     */
    public function toArray($model, $fields = array())
    {

        if (!$model instanceof \Oasis\Model\Raw\KamUsersLocationAttrs) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Oasis\Model\Raw\KamUsersLocationAttrs object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Oasis\Model\\KamUsersLocationAttrs object in toArray for " . get_class($this);
            }
        ...
        if (empty($fields)) {
            $result = array(
                ...
                'aname' => $model->getAname(),
                'atype' => $model->getAtype(),
     ...
     *
     * @return Oasis\\Mapper\\Sql\\DbTable\\KamUsersLocationAttrs
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Oasis\\Mapper\\Sql\\DbTable\\KamUsersLocationAttrs');
        }
     ...
     *
     * @param Oasis\Model\Raw\KamUsersLocationAttrs $model The model to delete
    ...
    public function delete(\Oasis\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Oasis\Model\Raw\KamUsersLocationAttrs) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Oasis\\Model\\KamUsersLocationAttrs object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Oasis\\Model\\KamUsersLocationAttrs object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Oasis\\Mapper\\Sql\\DbTable\\KamUsersLocationAttrs', $capitalizedFk);
     ...
     */
    public function save(\Oasis\Model\Raw\KamUsersLocationAttrs $model, $forceInsert = false)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Oasis\Model\Raw\KamUsersLocationAttrs $model, $useTransaction = true,
            $transactionTag = null, $forceInsert = false)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag, $forceInsert);
    }

    protected function _save(\Oasis\Model\Raw\KamUsersLocationAttrs $model,
     ...
     * @param Oasis\Model\Raw\KamUsersLocationAttrs|null $entry The object to load the data into, or null to have one created
     * @return Oasis\Model\Raw\KamUsersLocationAttrs The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Oasis\Model\KamUsersLocationAttrs();
        }
        ...
        } else if ($data instanceof \Oasis\Model\Raw\KamUsersLocationAttrs) {
            $entry->setId($data->getId())
                  ->setRuid($data->getRuid())
                  ->setUsername($data->getUsername())
                  ->setDomain($data->getDomain())
                  ->setAname($data->getAname())
                  ->setAtype($data->getAtype())
                  ->setAvalue($data->getAvalue())
