...
/**
 * Data Mapper implementation for Oasis\Model\KamTrunksHtable
 *
 * @package Oasis\Mapper\Sql
 * @subpackage Raw
 * @author Luis Felipe Garcia
...
class KamTrunksHtable extends MapperAbstract
{
    protected $_modelName = 'Oasis\\Model\\KamTrunksHtable';
     ...
     *
     * @param Oasis\Model\Raw\KamTrunksHtable $model
     * @return array
     */
    public function toArray($model)
    {
        if (!$model instanceof \Oasis\Model\Raw\KamTrunksHtable) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Oasis\Model\Raw\KamTrunksHtable object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Oasis\Model\\KamTrunksHtable object in toArray for " . get_class($this);
            }
        ...
        $result = array(
            ...
            'value_type' => $model->getValueType(),
            'key_value' => $model->getKeyValue(),
     ...
     *
     * @return Oasis\\Mapper\\Sql\\DbTable\\KamTrunksHtable
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Oasis\\Mapper\\Sql\\DbTable\\KamTrunksHtable');
        }
     ...
     *
     * @param Oasis\Model\Raw\KamTrunksHtable $model The model to delete
    ...
    public function delete(\Oasis\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Oasis\Model\Raw\KamTrunksHtable) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Oasis\\Model\\KamTrunksHtable object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Oasis\\Model\\KamTrunksHtable object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Oasis\\Mapper\\Sql\\DbTable\\KamTrunksHtable', $capitalizedFk);
     ...
     */
    public function save(\Oasis\Model\Raw\KamTrunksHtable $model)
    {
     ...
     * @return boolean If the save action was successful
     */
    public function saveRecursive(\Oasis\Model\Raw\KamTrunksHtable $model, $useTransaction = true, $transactionTag = null)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag);
    }

    protected function _save(\Oasis\Model\Raw\KamTrunksHtable $model,
     ...
     * @param Oasis\Model\Raw\KamTrunksHtable|null $entry The object to load the data into, or null to have one created
     * @return Oasis\Model\Raw\KamTrunksHtable The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Oasis\Model\KamTrunksHtable();
        }
        ...
        } else if ($data instanceof \Oasis\Model\Raw\KamTrunksHtable) {
            $entry->setId($data->getId())
                  ->setKeyName($data->getKeyName())
                  ->setKeyType($data->getKeyType())
                  ->setValueType($data->getValueType())
                  ->setKeyValue($data->getKeyValue())
