...
/**
 * Data Mapper implementation for Oasis\Model\CompaniesRelPricingPlans
 *
 * @package Oasis\Mapper\Sql
 * @subpackage Raw
 * @author Luis Felipe Garcia
...
class CompaniesRelPricingPlans extends MapperAbstract
{
    protected $_modelName = 'Oasis\\Model\\CompaniesRelPricingPlans';
     ...
     *
     * @param Oasis\Model\Raw\CompaniesRelPricingPlans $model
     * @return array
     */
    public function toArray($model, $fields = array())
    {

        if (!$model instanceof \Oasis\Model\Raw\CompaniesRelPricingPlans) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Oasis\Model\Raw\CompaniesRelPricingPlans object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Oasis\Model\\CompaniesRelPricingPlans object in toArray for " . get_class($this);
            }
     ...
     *
     * @return Oasis\\Mapper\\Sql\\DbTable\\CompaniesRelPricingPlans
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Oasis\\Mapper\\Sql\\DbTable\\CompaniesRelPricingPlans');
        }
     ...
     *
     * @param Oasis\Model\Raw\CompaniesRelPricingPlans $model The model to delete
    ...
    public function delete(\Oasis\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Oasis\Model\Raw\CompaniesRelPricingPlans) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Oasis\\Model\\CompaniesRelPricingPlans object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Oasis\\Model\\CompaniesRelPricingPlans object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Oasis\\Mapper\\Sql\\DbTable\\CompaniesRelPricingPlans', $capitalizedFk);
     ...
     */
    public function save(\Oasis\Model\Raw\CompaniesRelPricingPlans $model)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Oasis\Model\Raw\CompaniesRelPricingPlans $model, $useTransaction = true, $transactionTag = null)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag);
    }

    protected function _save(\Oasis\Model\Raw\CompaniesRelPricingPlans $model,
     ...
     * @param Oasis\Model\Raw\CompaniesRelPricingPlans|null $entry The object to load the data into, or null to have one created
     * @return Oasis\Model\Raw\CompaniesRelPricingPlans The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Oasis\Model\CompaniesRelPricingPlans();
        }
        ...
        } else if ($data instanceof \Oasis\Model\Raw\CompaniesRelPricingPlans) {
