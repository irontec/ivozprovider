...
/**
 * Data Mapper implementation for Oasis\Model\AstPsSubscriptionPersistence
 *
 * @package Oasis\Mapper\Sql
 * @subpackage Raw
 * @author Luis Felipe Garcia
...
class AstPsSubscriptionPersistence extends MapperAbstract
{
    protected $_modelName = 'Oasis\\Model\\AstPsSubscriptionPersistence';
     ...
     *
     * @param Oasis\Model\Raw\AstPsSubscriptionPersistence $model
     * @return array
     */
    public function toArray($model)
    {
        if (!$model instanceof \Oasis\Model\Raw\AstPsSubscriptionPersistence) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Oasis\Model\Raw\AstPsSubscriptionPersistence object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Oasis\Model\\AstPsSubscriptionPersistence object in toArray for " . get_class($this);
            }
        ...
        $result = array(
            ...
            'src_name' => $model->getSrcName(),
            'src_port' => $model->getSrcPort(),
            'transport_key' => $model->getTransportKey(),
            'local_name' => $model->getLocalName(),
            'local_port' => $model->getLocalPort(),
            'cseq' => $model->getCseq(),
            'tag' => $model->getTag(),
     ...
     *
     * @return Oasis\\Mapper\\Sql\\DbTable\\AstPsSubscriptionPersistence
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Oasis\\Mapper\\Sql\\DbTable\\AstPsSubscriptionPersistence');
        }
     ...
     *
     * @param Oasis\Model\Raw\AstPsSubscriptionPersistence $model The model to delete
    ...
    public function delete(\Oasis\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Oasis\Model\Raw\AstPsSubscriptionPersistence) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Oasis\\Model\\AstPsSubscriptionPersistence object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Oasis\\Model\\AstPsSubscriptionPersistence object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Oasis\\Mapper\\Sql\\DbTable\\AstPsSubscriptionPersistence', $capitalizedFk);
     ...
     */
    public function save(\Oasis\Model\Raw\AstPsSubscriptionPersistence $model)
    {
     ...
     * @return boolean If the save action was successful
     */
    public function saveRecursive(\Oasis\Model\Raw\AstPsSubscriptionPersistence $model, $useTransaction = true, $transactionTag = null)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag);
    }

    protected function _save(\Oasis\Model\Raw\AstPsSubscriptionPersistence $model,
     ...
     * @param Oasis\Model\Raw\AstPsSubscriptionPersistence|null $entry The object to load the data into, or null to have one created
     * @return Oasis\Model\Raw\AstPsSubscriptionPersistence The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Oasis\Model\AstPsSubscriptionPersistence();
        }
        ...
        if (is_array($data)) {
            $entry->setSorceryId($data['sorcery_id'])
                  ...
                  ->setSrcName($data['src_name'])
                  ->setSrcPort($data['src_port'])
                  ->setTransportKey($data['transport_key'])
                  ->setLocalName($data['local_name'])
                  ->setLocalPort($data['local_port'])
                  ->setCseq($data['cseq'])
                  ->setTag($data['tag'])
        ...
        } else if ($data instanceof \Oasis\Model\Raw\AstPsSubscriptionPersistence) {
            $entry->setSorceryId($data->getSorceryId())
                  ->setPacket($data->getPacket())
                  ->setSrcName($data->getSrcName())
                  ->setSrcPort($data->getSrcPort())
                  ->setTransportKey($data->getTransportKey())
                  ->setLocalName($data->getLocalName())
                  ->setLocalPort($data->getLocalPort())
