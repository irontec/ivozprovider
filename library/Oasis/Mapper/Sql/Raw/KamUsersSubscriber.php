...
/**
 * Data Mapper implementation for Oasis\Model\KamUsersSubscriber
 *
 * @package Oasis\Mapper\Sql
 * @subpackage Raw
 * @author Luis Felipe Garcia
...
class KamUsersSubscriber extends MapperAbstract
{
    protected $_modelName = 'Oasis\\Model\\KamUsersSubscriber';
     ...
     *
     * @param Oasis\Model\Raw\KamUsersSubscriber $model
     * @return array
     */
    public function toArray($model)
    {
        if (!$model instanceof \Oasis\Model\Raw\KamUsersSubscriber) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Oasis\Model\Raw\KamUsersSubscriber object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Oasis\Model\\KamUsersSubscriber object in toArray for " . get_class($this);
            }
     ...
     *
     * @return Oasis\\Mapper\\Sql\\DbTable\\KamUsersSubscriber
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Oasis\\Mapper\\Sql\\DbTable\\KamUsersSubscriber');
        }
     ...
     *
     * @param Oasis\Model\Raw\KamUsersSubscriber $model The model to delete
    ...
    public function delete(\Oasis\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Oasis\Model\Raw\KamUsersSubscriber) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Oasis\\Model\\KamUsersSubscriber object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Oasis\\Model\\KamUsersSubscriber object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Oasis\\Mapper\\Sql\\DbTable\\KamUsersSubscriber', $capitalizedFk);
     ...
     */
    public function save(\Oasis\Model\Raw\KamUsersSubscriber $model)
    {
     ...
     * @return boolean If the save action was successful
     */
    public function saveRecursive(\Oasis\Model\Raw\KamUsersSubscriber $model, $useTransaction = true, $transactionTag = null)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag);
    }

    protected function _save(\Oasis\Model\Raw\KamUsersSubscriber $model,
     ...
     * @param Oasis\Model\Raw\KamUsersSubscriber|null $entry The object to load the data into, or null to have one created
     * @return Oasis\Model\Raw\KamUsersSubscriber The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Oasis\Model\KamUsersSubscriber();
        }
        ...
        if (is_array($data)) {
        ...
        } else if ($data instanceof \Oasis\Model\Raw\KamUsersSubscriber) {
            $entry->setId($data->getId())
                  ...
                  ->setHa1($data->getHa1())
                  ->setHa1b($data->getHa1b())
                  ->setRpid($data->getRpid());
