...
/**
 * Data Mapper implementation for Oasis\Model\GenericNumericTransformRules
 *
 * @package Oasis\Mapper\Sql
 * @subpackage Raw
 * @author Luis Felipe Garcia
...
class GenericNumericTransformRules extends MapperAbstract
{
    protected $_modelName = 'Oasis\\Model\\GenericNumericTransformRules';
     ...
     *
     * @param Oasis\Model\Raw\GenericNumericTransformRules $model
     * @return array
     */
    public function toArray($model)
    {
        if (!$model instanceof \Oasis\Model\Raw\GenericNumericTransformRules) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Oasis\Model\Raw\GenericNumericTransformRules object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Oasis\Model\\GenericNumericTransformRules object in toArray for " . get_class($this);
            }
     ...
     *
     * @return Oasis\\Mapper\\Sql\\DbTable\\GenericNumericTransformRules
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Oasis\\Mapper\\Sql\\DbTable\\GenericNumericTransformRules');
        }
     ...
     *
     * @param Oasis\Model\Raw\GenericNumericTransformRules $model The model to delete
    ...
    public function delete(\Oasis\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Oasis\Model\Raw\GenericNumericTransformRules) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Oasis\\Model\\GenericNumericTransformRules object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Oasis\\Model\\GenericNumericTransformRules object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Oasis\\Mapper\\Sql\\DbTable\\GenericNumericTransformRules', $capitalizedFk);
     ...
     */
    public function save(\Oasis\Model\Raw\GenericNumericTransformRules $model)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Oasis\Model\Raw\GenericNumericTransformRules $model, $useTransaction = true, $transactionTag = null)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag);
    }

    protected function _save(\Oasis\Model\Raw\GenericNumericTransformRules $model,
     ...
     * @param Oasis\Model\Raw\GenericNumericTransformRules|null $entry The object to load the data into, or null to have one created
     * @return Oasis\Model\Raw\GenericNumericTransformRules The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Oasis\Model\GenericNumericTransformRules();
        }
        ...
        } else if ($data instanceof \Oasis\Model\Raw\GenericNumericTransformRules) {
