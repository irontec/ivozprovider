...
/**
 * Data Mapper implementation for Oasis\Model\ExternalCallFilterRelSchedules
 *
 * @package Oasis\Mapper\Sql
 * @subpackage Raw
 * @author Luis Felipe Garcia
...
{
    protected $_modelName = 'Oasis\\Model\\ExternalCallFilterRelSchedules';
     ...
     *
     * @param Oasis\Model\Raw\ExternalCallFilterRelSchedules $model
     * @return array
     */
    public function toArray($model, $fields = array())
    {

        if (!$model instanceof \Oasis\Model\Raw\ExternalCallFilterRelSchedules) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Oasis\Model\Raw\ExternalCallFilterRelSchedules object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Oasis\Model\\ExternalCallFilterRelSchedules object in toArray for " . get_class($this);
            }
     ...
     *
     * @return Oasis\\Mapper\\Sql\\DbTable\\ExternalCallFilterRelSchedules
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Oasis\\Mapper\\Sql\\DbTable\\ExternalCallFilterRelSchedules');
        }
     ...
     *
     * @param Oasis\Model\Raw\ExternalCallFilterRelSchedules $model The model to delete
    ...
    public function delete(\Oasis\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Oasis\Model\Raw\ExternalCallFilterRelSchedules) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Oasis\\Model\\ExternalCallFilterRelSchedules object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Oasis\\Model\\ExternalCallFilterRelSchedules object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Oasis\\Mapper\\Sql\\DbTable\\ExternalCallFilterRelSchedules', $capitalizedFk);
     ...
     */
    public function save(\Oasis\Model\Raw\ExternalCallFilterRelSchedules $model, $forceInsert = false)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Oasis\Model\Raw\ExternalCallFilterRelSchedules $model, $useTransaction = true,
            $transactionTag = null, $forceInsert = false)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag, $forceInsert);
    }

    protected function _save(\Oasis\Model\Raw\ExternalCallFilterRelSchedules $model,
     ...
     * @param Oasis\Model\Raw\ExternalCallFilterRelSchedules|null $entry The object to load the data into, or null to have one created
     * @return Oasis\Model\Raw\ExternalCallFilterRelSchedules The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Oasis\Model\ExternalCallFilterRelSchedules();
        }
        ...
        } else if ($data instanceof \Oasis\Model\Raw\ExternalCallFilterRelSchedules) {
