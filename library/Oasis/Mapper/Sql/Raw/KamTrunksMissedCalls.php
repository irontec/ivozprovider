...
/**
 * Data Mapper implementation for Oasis\Model\KamTrunksMissedCalls
 *
 * @package Oasis\Mapper\Sql
 * @subpackage Raw
 * @author Luis Felipe Garcia
...
class KamTrunksMissedCalls extends MapperAbstract
{
    protected $_modelName = 'Oasis\\Model\\KamTrunksMissedCalls';
     ...
     *
     * @param Oasis\Model\Raw\KamTrunksMissedCalls $model
     * @return array
     */
    public function toArray($model)
    {
        if (!$model instanceof \Oasis\Model\Raw\KamTrunksMissedCalls) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Oasis\Model\Raw\KamTrunksMissedCalls object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Oasis\Model\\KamTrunksMissedCalls object in toArray for " . get_class($this);
            }
        ...
        $result = array(
            ...
            'sip_code' => $model->getSipCode(),
            'sip_reason' => $model->getSipReason(),
            'time' => $model->getTime(),
            'src_ip' => $model->getSrcIp(),
            'from_user' => $model->getFromUser(),
            'from_domain' => $model->getFromDomain(),
            'ruri_user' => $model->getRuriUser(),
            'ruri_domain' => $model->getRuriDomain(),
            'cseq' => $model->getCseq(),
        );
     ...
     *
     * @return Oasis\\Mapper\\Sql\\DbTable\\KamTrunksMissedCalls
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Oasis\\Mapper\\Sql\\DbTable\\KamTrunksMissedCalls');
        }
     ...
     *
     * @param Oasis\Model\Raw\KamTrunksMissedCalls $model The model to delete
    ...
    public function delete(\Oasis\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Oasis\Model\Raw\KamTrunksMissedCalls) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Oasis\\Model\\KamTrunksMissedCalls object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Oasis\\Model\\KamTrunksMissedCalls object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Oasis\\Mapper\\Sql\\DbTable\\KamTrunksMissedCalls', $capitalizedFk);
     ...
     */
    public function save(\Oasis\Model\Raw\KamTrunksMissedCalls $model)
    {
     ...
     * @return boolean If the save action was successful
     */
    public function saveRecursive(\Oasis\Model\Raw\KamTrunksMissedCalls $model, $useTransaction = true, $transactionTag = null)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag);
    }

    protected function _save(\Oasis\Model\Raw\KamTrunksMissedCalls $model,
     ...
     * @param Oasis\Model\Raw\KamTrunksMissedCalls|null $entry The object to load the data into, or null to have one created
     * @return Oasis\Model\Raw\KamTrunksMissedCalls The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Oasis\Model\KamTrunksMissedCalls();
        }
        ...
        if (is_array($data)) {
            $entry->setId($data['id'])
                  ...
                  ->setSipCode($data['sip_code'])
                  ->setSipReason($data['sip_reason'])
                  ->setTime($data['time'])
                  ->setSrcIp($data['src_ip'])
                  ->setFromUser($data['from_user'])
                  ->setFromDomain($data['from_domain'])
                  ->setRuriUser($data['ruri_user'])
                  ->setRuriDomain($data['ruri_domain'])
                  ->setCseq($data['cseq']);
