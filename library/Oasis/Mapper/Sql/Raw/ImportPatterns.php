...
/**
 * Data Mapper implementation for Oasis\Model\ImportPatterns
 *
 * @package Oasis\Mapper\Sql
 * @subpackage Raw
 * @author Luis Felipe Garcia
...
class ImportPatterns extends MapperAbstract
{
    protected $_modelName = 'Oasis\\Model\\ImportPatterns';
     ...
     *
     * @param Oasis\Model\Raw\ImportPatterns $model
     * @return array
     */
    public function toArray($model, $fields = array())
    {

        if (!$model instanceof \Oasis\Model\Raw\ImportPatterns) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Oasis\Model\Raw\ImportPatterns object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Oasis\Model\\ImportPatterns object in toArray for " . get_class($this);
            }
        ...
        if (empty($fields)) {
            $result = array(
                ...
                'importFileId' => $model->getImportFileId(),
     ...
     *
     * @return Oasis\\Mapper\\Sql\\DbTable\\ImportPatterns
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Oasis\\Mapper\\Sql\\DbTable\\ImportPatterns');
        }
     ...
     *
     * @param Oasis\Model\Raw\ImportPatterns $model The model to delete
    ...
    public function delete(\Oasis\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Oasis\Model\Raw\ImportPatterns) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Oasis\\Model\\ImportPatterns object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Oasis\\Model\\ImportPatterns object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Oasis\\Mapper\\Sql\\DbTable\\ImportPatterns', $capitalizedFk);
     ...
     */
    public function save(\Oasis\Model\Raw\ImportPatterns $model)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Oasis\Model\Raw\ImportPatterns $model, $useTransaction = true, $transactionTag = null)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag);
    }

    protected function _save(\Oasis\Model\Raw\ImportPatterns $model,
     ...
     * @param Oasis\Model\Raw\ImportPatterns|null $entry The object to load the data into, or null to have one created
     * @return Oasis\Model\Raw\ImportPatterns The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Oasis\Model\ImportPatterns();
        }
        ...
        if (is_array($data)) {
            $entry->setId($data['id'])
                  ...
                  ->setImportFileId($data['importFileId'])
        ...
        } else if ($data instanceof \Oasis\Model\Raw\ImportPatterns) {
            $entry->setId($data->getId())
                  ->setName($data->getName())
                  ->setRegExp($data->getRegExp())
                  ->setMetric($data->getMetric())
                  ->setImportFileId($data->getImportFileId())
