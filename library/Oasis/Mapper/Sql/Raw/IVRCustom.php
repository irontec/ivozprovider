...
/**
 * Data Mapper implementation for Oasis\Model\IVRCustom
 *
 * @package Oasis\Mapper\Sql
 * @subpackage Raw
 * @author Luis Felipe Garcia
...
{
    protected $_modelName = 'Oasis\\Model\\IVRCustom';
     ...
     *
     * @param Oasis\Model\Raw\IVRCustom $model
     * @return array
     */
    public function toArray($model, $fields = array())
    {

        if (!$model instanceof \Oasis\Model\Raw\IVRCustom) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Oasis\Model\Raw\IVRCustom object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Oasis\Model\\IVRCustom object in toArray for " . get_class($this);
            }
     ...
     *
     * @return Oasis\\Mapper\\Sql\\DbTable\\IVRCustom
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Oasis\\Mapper\\Sql\\DbTable\\IVRCustom');
        }
     ...
     *
     * @param Oasis\Model\Raw\IVRCustom $model The model to delete
    ...
    public function delete(\Oasis\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Oasis\Model\Raw\IVRCustom) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Oasis\\Model\\IVRCustom object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Oasis\\Model\\IVRCustom object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Oasis\\Mapper\\Sql\\DbTable\\IVRCustom', $capitalizedFk);
     ...
     */
    public function save(\Oasis\Model\Raw\IVRCustom $model, $forceInsert = false)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Oasis\Model\Raw\IVRCustom $model, $useTransaction = true,
            $transactionTag = null, $forceInsert = false)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag, $forceInsert);
    }

    protected function _save(\Oasis\Model\Raw\IVRCustom $model,
     ...
     * @param Oasis\Model\Raw\IVRCustom|null $entry The object to load the data into, or null to have one created
     * @return Oasis\Model\Raw\IVRCustom The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Oasis\Model\IVRCustom();
        }
        ...
        } else if ($data instanceof \Oasis\Model\Raw\IVRCustom) {
