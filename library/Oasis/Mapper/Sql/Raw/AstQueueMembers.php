...
/**
 * Data Mapper implementation for Oasis\Model\AstQueueMembers
 *
 * @package Oasis\Mapper\Sql
 * @subpackage Raw
 * @author Luis Felipe Garcia
...
{
    protected $_modelName = 'Oasis\\Model\\AstQueueMembers';
     ...
     *
     * @param Oasis\Model\Raw\AstQueueMembers $model
     * @return array
     */
    public function toArray($model)
    {
        if (!$model instanceof \Oasis\Model\Raw\AstQueueMembers) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Oasis\Model\Raw\AstQueueMembers object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Oasis\Model\\AstQueueMembers object in toArray for " . get_class($this);
            }
     ...
     *
     * @return Oasis\\Mapper\\Sql\\DbTable\\AstQueueMembers
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Oasis\\Mapper\\Sql\\DbTable\\AstQueueMembers');
        }
     ...
     *
     * @param Oasis\Model\Raw\AstQueueMembers $model The model to delete
    ...
    public function delete(\Oasis\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Oasis\Model\Raw\AstQueueMembers) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Oasis\\Model\\AstQueueMembers object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Oasis\\Model\\AstQueueMembers object in delete for " . get_class($this);
            }
        ...
        try {
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Oasis\\Mapper\\Sql\\DbTable\\AstQueueMembers', $capitalizedFk);
            ...
            $where = $dbAdapter->quoteInto($dbAdapter->quoteIdentifier('queue_name') . ' = ?', $model->getQueueName());
     ...
     */
    public function save(\Oasis\Model\Raw\AstQueueMembers $model)
    {
     ...
     * @return boolean If the save action was successful
     */
    public function saveRecursive(\Oasis\Model\Raw\AstQueueMembers $model, $useTransaction = true, $transactionTag = null)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag);
    }

    protected function _save(\Oasis\Model\Raw\AstQueueMembers $model,
    ...
    )
    {
        ...
        try {
            if (is_null($primaryKey) || empty($primaryKey)) {
                ...
                $this->getDbTable()
                     ->update(
                         $data,
                         array(
                             $this->getDbTable()->getAdapter()->quoteIdentifier('queue_name') . ' = ?' => $primaryKey
                         )
                     );
            }
            ...
            if (!empty($primaryKey)) {
                $message .= ' id:';
                $message .= ' queue_name => ' . $primaryKey['queue_name'];
                $message .= ' interface => ' . $primaryKey['interface'];
            } else {
     ...
     * @param Oasis\Model\Raw\AstQueueMembers|null $entry The object to load the data into, or null to have one created
     * @return Oasis\Model\Raw\AstQueueMembers The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Oasis\Model\AstQueueMembers();
        }
        ...
        if (is_array($data)) {
            $entry->setQueueName($data['queue_name'])
        ...
        } else if ($data instanceof \Oasis\Model\Raw\AstQueueMembers) {
            $entry->setQueueName($data->getQueueName())
