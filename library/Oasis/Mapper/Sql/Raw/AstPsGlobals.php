...
/**
 * Data Mapper implementation for Oasis\Model\AstPsGlobals
 *
 * @package Oasis\Mapper\Sql
 * @subpackage Raw
 * @author Luis Felipe Garcia
...
class AstPsGlobals extends MapperAbstract
{
    protected $_modelName = 'Oasis\\Model\\AstPsGlobals';
     ...
     *
     * @param Oasis\Model\Raw\AstPsGlobals $model
     * @return array
     */
    public function toArray($model)
    {
        if (!$model instanceof \Oasis\Model\Raw\AstPsGlobals) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Oasis\Model\Raw\AstPsGlobals object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Oasis\Model\\AstPsGlobals object in toArray for " . get_class($this);
            }
        ...
        $result = array(
            ...
            'max_forwards' => $model->getMaxForwards(),
            'user_agent' => $model->getUserAgent(),
            'default_outbound_endpoint' => $model->getDefaultOutboundEndpoint(),
     ...
     *
     * @return Oasis\\Mapper\\Sql\\DbTable\\AstPsGlobals
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Oasis\\Mapper\\Sql\\DbTable\\AstPsGlobals');
        }
     ...
     *
     * @param Oasis\Model\Raw\AstPsGlobals $model The model to delete
    ...
    public function delete(\Oasis\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Oasis\Model\Raw\AstPsGlobals) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Oasis\\Model\\AstPsGlobals object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Oasis\\Model\\AstPsGlobals object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Oasis\\Mapper\\Sql\\DbTable\\AstPsGlobals', $capitalizedFk);
     ...
     */
    public function save(\Oasis\Model\Raw\AstPsGlobals $model)
    {
     ...
     * @return boolean If the save action was successful
     */
    public function saveRecursive(\Oasis\Model\Raw\AstPsGlobals $model, $useTransaction = true, $transactionTag = null)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag);
    }

    protected function _save(\Oasis\Model\Raw\AstPsGlobals $model,
     ...
     * @param Oasis\Model\Raw\AstPsGlobals|null $entry The object to load the data into, or null to have one created
     * @return Oasis\Model\Raw\AstPsGlobals The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Oasis\Model\AstPsGlobals();
        }
        ...
        if (is_array($data)) {
            $entry->setSorceryId($data['sorcery_id'])
                  ->setMaxForwards($data['max_forwards'])
                  ->setUserAgent($data['user_agent'])
                  ->setDefaultOutboundEndpoint($data['default_outbound_endpoint'])
                  ->setDebug($data['debug']);
        ...
        } else if ($data instanceof \Oasis\Model\Raw\AstPsGlobals) {
            $entry->setSorceryId($data->getSorceryId())
                  ->setMaxForwards($data->getMaxForwards())
                  ->setUserAgent($data->getUserAgent())
                  ->setDefaultOutboundEndpoint($data->getDefaultOutboundEndpoint())
