...
{
    protected $_modelName = 'Oasis\\Model\\TransformationRulesetGroupsUsers';
     ...
     *
     * @param Oasis\Model\Raw\TransformationRulesetGroupsUsers $model
     * @return array
     */
    public function toArray($model, $fields = array())
    {

        if (!$model instanceof \Oasis\Model\Raw\TransformationRulesetGroupsUsers) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Oasis\Model\Raw\TransformationRulesetGroupsUsers object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Oasis\Model\\TransformationRulesetGroupsUsers object in toArray for " . get_class($this);
            }
     ...
     *
     * @return Oasis\\Mapper\\Sql\\DbTable\\TransformationRulesetGroupsUsers
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Oasis\\Mapper\\Sql\\DbTable\\TransformationRulesetGroupsUsers');
        }
     ...
     *
     * @param Oasis\Model\Raw\TransformationRulesetGroupsUsers $model The model to delete
    ...
    public function delete(\Oasis\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Oasis\Model\Raw\TransformationRulesetGroupsUsers) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Oasis\\Model\\TransformationRulesetGroupsUsers object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Oasis\\Model\\TransformationRulesetGroupsUsers object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Oasis\\Mapper\\Sql\\DbTable\\TransformationRulesetGroupsUsers', $capitalizedFk);
     ...
     */
    public function save(\Oasis\Model\Raw\TransformationRulesetGroupsUsers $model, $forceInsert = false)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Oasis\Model\Raw\TransformationRulesetGroupsUsers $model, $useTransaction = true,
            $transactionTag = null, $forceInsert = false)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag, $forceInsert);
    }

    protected function _save(\Oasis\Model\Raw\TransformationRulesetGroupsUsers $model,
     ...
     * @param Oasis\Model\Raw\TransformationRulesetGroupsUsers|null $entry The object to load the data into, or null to have one created
     * @return Oasis\Model\Raw\TransformationRulesetGroupsUsers The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Oasis\Model\TransformationRulesetGroupsUsers();
        }
        ...
        } else if ($data instanceof \Oasis\Model\Raw\TransformationRulesetGroupsUsers) {
