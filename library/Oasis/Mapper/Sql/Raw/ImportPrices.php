...
/**
 * Data Mapper implementation for Oasis\Model\ImportPrices
 *
 * @package Oasis\Mapper\Sql
 * @subpackage Raw
 * @author Luis Felipe Garcia
...
class ImportPrices extends MapperAbstract
{
    protected $_modelName = 'Oasis\\Model\\ImportPrices';
     ...
     *
     * @param Oasis\Model\Raw\ImportPrices $model
     * @return array
     */
    public function toArray($model, $fields = array())
    {

        if (!$model instanceof \Oasis\Model\Raw\ImportPrices) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Oasis\Model\Raw\ImportPrices object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Oasis\Model\\ImportPrices object in toArray for " . get_class($this);
            }
     ...
     *
     * @return Oasis\\Mapper\\Sql\\DbTable\\ImportPrices
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Oasis\\Mapper\\Sql\\DbTable\\ImportPrices');
        }
     ...
     *
     * @param Oasis\Model\Raw\ImportPrices $model The model to delete
    ...
    public function delete(\Oasis\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Oasis\Model\Raw\ImportPrices) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Oasis\\Model\\ImportPrices object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Oasis\\Model\\ImportPrices object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Oasis\\Mapper\\Sql\\DbTable\\ImportPrices', $capitalizedFk);
     ...
     */
    public function save(\Oasis\Model\Raw\ImportPrices $model)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Oasis\Model\Raw\ImportPrices $model, $useTransaction = true, $transactionTag = null)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag);
    }

    protected function _save(\Oasis\Model\Raw\ImportPrices $model,
     ...
     * @param Oasis\Model\Raw\ImportPrices|null $entry The object to load the data into, or null to have one created
     * @return Oasis\Model\Raw\ImportPrices The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Oasis\Model\ImportPrices();
        }
        ...
        } else if ($data instanceof \Oasis\Model\Raw\ImportPrices) {
