...
/**
 * Data Mapper implementation for Oasis\Model\KamTrunksDialogVars
 *
 * @package Oasis\Mapper\Sql
 * @subpackage Raw
 * @author Luis Felipe Garcia
...
class KamTrunksDialogVars extends MapperAbstract
{
    protected $_modelName = 'Oasis\\Model\\KamTrunksDialogVars';
     ...
     *
     * @param Oasis\Model\Raw\KamTrunksDialogVars $model
     * @return array
     */
    public function toArray($model)
    {
        if (!$model instanceof \Oasis\Model\Raw\KamTrunksDialogVars) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Oasis\Model\Raw\KamTrunksDialogVars object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Oasis\Model\\KamTrunksDialogVars object in toArray for " . get_class($this);
            }
        ...
        $result = array(
            ...
            'dialog_key' => $model->getDialogKey(),
            'dialog_value' => $model->getDialogValue(),
        );
     ...
     *
     * @return Oasis\\Mapper\\Sql\\DbTable\\KamTrunksDialogVars
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Oasis\\Mapper\\Sql\\DbTable\\KamTrunksDialogVars');
        }
     ...
     *
     * @param Oasis\Model\Raw\KamTrunksDialogVars $model The model to delete
    ...
    public function delete(\Oasis\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Oasis\Model\Raw\KamTrunksDialogVars) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Oasis\\Model\\KamTrunksDialogVars object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Oasis\\Model\\KamTrunksDialogVars object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Oasis\\Mapper\\Sql\\DbTable\\KamTrunksDialogVars', $capitalizedFk);
     ...
     */
    public function save(\Oasis\Model\Raw\KamTrunksDialogVars $model)
    {
     ...
     * @return boolean If the save action was successful
     */
    public function saveRecursive(\Oasis\Model\Raw\KamTrunksDialogVars $model, $useTransaction = true, $transactionTag = null)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag);
    }

    protected function _save(\Oasis\Model\Raw\KamTrunksDialogVars $model,
     ...
     * @param Oasis\Model\Raw\KamTrunksDialogVars|null $entry The object to load the data into, or null to have one created
     * @return Oasis\Model\Raw\KamTrunksDialogVars The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Oasis\Model\KamTrunksDialogVars();
        }
        ...
        if (is_array($data)) {
            $entry->setId($data['id'])
                  ...
                  ->setDialogKey($data['dialog_key'])
                  ->setDialogValue($data['dialog_value']);
        ...
        } else if ($data instanceof \Oasis\Model\Raw\KamTrunksDialogVars) {
            $entry->setId($data->getId())
                  ->setHashEntry($data->getHashEntry())
                  ->setHashId($data->getHashId())
                  ->setDialogKey($data->getDialogKey())
                  ->setDialogValue($data->getDialogValue());
