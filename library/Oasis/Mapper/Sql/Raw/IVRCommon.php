...
{
    protected $_modelName = 'Oasis\\Model\\IVRCommon';
     ...
     *
     * @param Oasis\Model\Raw\IVRCommon $model
     * @return array
     */
    public function toArray($model, $fields = array())
    {

        if (!$model instanceof \Oasis\Model\Raw\IVRCommon) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Oasis\Model\Raw\IVRCommon object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Oasis\Model\\IVRCommon object in toArray for " . get_class($this);
            }
     ...
     *
     * @return Oasis\\Mapper\\Sql\\DbTable\\IVRCommon
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Oasis\\Mapper\\Sql\\DbTable\\IVRCommon');
        }
     ...
     *
     * @param Oasis\Model\Raw\IVRCommon $model The model to delete
    ...
    public function delete(\Oasis\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Oasis\Model\Raw\IVRCommon) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Oasis\\Model\\IVRCommon object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Oasis\\Model\\IVRCommon object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Oasis\\Mapper\\Sql\\DbTable\\IVRCommon', $capitalizedFk);
     ...
     */
    public function save(\Oasis\Model\Raw\IVRCommon $model, $forceInsert = false)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Oasis\Model\Raw\IVRCommon $model, $useTransaction = true,
     ...
     * @param Oasis\Model\Raw\IVRCommon|null $entry The object to load the data into, or null to have one created
     * @return Oasis\Model\Raw\IVRCommon The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Oasis\Model\IVRCommon();
        }
        ...
        } else if ($data instanceof \Oasis\Model\Raw\IVRCommon) {
