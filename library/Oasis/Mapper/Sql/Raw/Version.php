...
/**
 * Data Mapper implementation for Oasis\Model\Version
 *
 * @package Oasis\Mapper\Sql
 * @subpackage Raw
 * @author Luis Felipe Garcia
...
class Version extends MapperAbstract
{
    protected $_modelName = 'Oasis\\Model\\Version';
     ...
     *
     * @param Oasis\Model\Raw\Version $model
     * @return array
     */
    public function toArray($model, $fields = array())
    {

        if (!$model instanceof \Oasis\Model\Raw\Version) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Oasis\Model\Raw\Version object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Oasis\Model\\Version object in toArray for " . get_class($this);
            }
        ...
        if (empty($fields)) {
            $result = array(
                ...
                'table_version' => $model->getTableVersion(),
            );
        } else {
            $result = array();
     ...
     *
     * @return Oasis\\Mapper\\Sql\\DbTable\\Version
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Oasis\\Mapper\\Sql\\DbTable\\Version');
        }
     ...
     *
     * @param Oasis\Model\Raw\Version $model The model to delete
    ...
    public function delete(\Oasis\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Oasis\Model\Raw\Version) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Oasis\\Model\\Version object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Oasis\\Model\\Version object in delete for " . get_class($this);
            }
        ...
        try {
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Oasis\\Mapper\\Sql\\DbTable\\Version', $capitalizedFk);
            ...
            $where = $dbAdapter->quoteInto($dbAdapter->quoteIdentifier('table_name') . ' = ?', $model->getTableName());
     ...
     */
    public function save(\Oasis\Model\Raw\Version $model, $forceInsert = false)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Oasis\Model\Raw\Version $model, $useTransaction = true,
            $transactionTag = null, $forceInsert = false)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag, $forceInsert);
    }

    protected function _save(\Oasis\Model\Raw\Version $model,
            ...
            if (is_null($primaryKey) || empty($primaryKey) || $forceInsert) {
                ...
                $this->getDbTable()
                     ->update(
                         $data,
                         array(
                             $this->getDbTable()->getAdapter()->quoteIdentifier('table_name') . ' = ?' => $primaryKey
                         )
                     );
            }
     ...
     * @param Oasis\Model\Raw\Version|null $entry The object to load the data into, or null to have one created
     * @return Oasis\Model\Raw\Version The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Oasis\Model\Version();
        }
        ...
        if (is_array($data)) {
            $entry->setTableName($data['table_name'])
                  ->setTableVersion($data['table_version']);
        } else if ($data instanceof \Zend_Db_Table_Row_Abstract || $data instanceof \stdClass) {
            $entry->setTableName($data->{'table_name'})
                  ->setTableVersion($data->{'table_version'});

        } else if ($data instanceof \Oasis\Model\Raw\Version) {
            $entry->setTableName($data->getTableName())
                  ->setTableVersion($data->getTableVersion());
