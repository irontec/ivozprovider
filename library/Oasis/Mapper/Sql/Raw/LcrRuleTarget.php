...
/**
 * Data Mapper implementation for Oasis\Model\LcrRuleTarget
 *
 * @package Oasis\Mapper\Sql
 * @subpackage Raw
 * @author Luis Felipe Garcia
...
{
    protected $_modelName = 'Oasis\\Model\\LcrRuleTarget';
     ...
     *
     * @param Oasis\Model\Raw\LcrRuleTarget $model
     * @return array
     */
    public function toArray($model, $fields = array())
    {

        if (!$model instanceof \Oasis\Model\Raw\LcrRuleTarget) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Oasis\Model\Raw\LcrRuleTarget object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Oasis\Model\\LcrRuleTarget object in toArray for " . get_class($this);
            }
        ...
        if (empty($fields)) {
            $result = array(
                ...
                'peeringContractsRelLcrRulesId' => $model->getPeeringContractsRelLcrRulesId(),
            );
        } else {
            $result = array();
     ...
     *
     * @return Oasis\\Mapper\\Sql\\DbTable\\LcrRuleTarget
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Oasis\\Mapper\\Sql\\DbTable\\LcrRuleTarget');
        }
     ...
     *
     * @param Oasis\Model\Raw\LcrRuleTarget $model The model to delete
    ...
    public function delete(\Oasis\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Oasis\Model\Raw\LcrRuleTarget) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Oasis\\Model\\LcrRuleTarget object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Oasis\\Model\\LcrRuleTarget object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Oasis\\Mapper\\Sql\\DbTable\\LcrRuleTarget', $capitalizedFk);
     ...
     */
    public function save(\Oasis\Model\Raw\LcrRuleTarget $model, $forceInsert = false)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Oasis\Model\Raw\LcrRuleTarget $model, $useTransaction = true,
            $transactionTag = null, $forceInsert = false)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag, $forceInsert);
    }

    protected function _save(\Oasis\Model\Raw\LcrRuleTarget $model,
     ...
     * @param Oasis\Model\Raw\LcrRuleTarget|null $entry The object to load the data into, or null to have one created
     * @return Oasis\Model\Raw\LcrRuleTarget The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Oasis\Model\LcrRuleTarget();
        }
        ...
        if (is_array($data)) {
            $entry->setId($data['id'])
                  ...
                  ->setPeeringContractsRelLcrRulesId($data['peeringContractsRelLcrRulesId']);
        ...
        } else if ($data instanceof \Oasis\Model\Raw\LcrRuleTarget) {
            $entry->setId($data->getId())
                  ...
                  ->setPeeringContractsRelLcrRulesId($data->getPeeringContractsRelLcrRulesId());
