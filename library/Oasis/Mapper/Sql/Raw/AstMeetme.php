...
/**
 * Data Mapper implementation for Oasis\Model\AstMeetme
 *
 * @package Oasis\Mapper\Sql
 * @subpackage Raw
 * @author Luis Felipe Garcia
...
class AstMeetme extends MapperAbstract
{
    protected $_modelName = 'Oasis\\Model\\AstMeetme';
     ...
     *
     * @param Oasis\Model\Raw\AstMeetme $model
     * @return array
     */
    public function toArray($model)
    {
        if (!$model instanceof \Oasis\Model\Raw\AstMeetme) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Oasis\Model\Raw\AstMeetme object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Oasis\Model\\AstMeetme object in toArray for " . get_class($this);
            }
        ...
        $result = array(
            ...
            'confno' => $model->getConfno(),
            'starttime' => $model->getStarttime(),
            'endtime' => $model->getEndtime(),
            'pin' => $model->getPin(),
            'adminpin' => $model->getAdminpin(),
            'opts' => $model->getOpts(),
            'adminopts' => $model->getAdminopts(),
            'recordingfilename' => $model->getRecordingfilename(),
            'recordingformat' => $model->getRecordingformat(),
     ...
     *
     * @return Oasis\\Mapper\\Sql\\DbTable\\AstMeetme
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Oasis\\Mapper\\Sql\\DbTable\\AstMeetme');
        }
     ...
     *
     * @param Oasis\Model\Raw\AstMeetme $model The model to delete
    ...
    public function delete(\Oasis\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Oasis\Model\Raw\AstMeetme) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Oasis\\Model\\AstMeetme object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Oasis\\Model\\AstMeetme object in delete for " . get_class($this);
            }
        ...
        try {
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Oasis\\Mapper\\Sql\\DbTable\\AstMeetme', $capitalizedFk);
            ...
            $where = $dbAdapter->quoteInto($dbAdapter->quoteIdentifier('bookid') . ' = ?', $model->getBookid());
     ...
     */
    public function save(\Oasis\Model\Raw\AstMeetme $model)
    {
     ...
     * @return boolean If the save action was successful
     */
    public function saveRecursive(\Oasis\Model\Raw\AstMeetme $model, $useTransaction = true, $transactionTag = null)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag);
    }

    protected function _save(\Oasis\Model\Raw\AstMeetme $model,
            ...
            if (is_null($primaryKey) || empty($primaryKey)) {
                ...
                $this->getDbTable()
                     ->update(
                         $data,
                         array(
                             $this->getDbTable()->getAdapter()->quoteIdentifier('bookid') . ' = ?' => $primaryKey
                         )
                     );
            }
     ...
     * @param Oasis\Model\Raw\AstMeetme|null $entry The object to load the data into, or null to have one created
     * @return Oasis\Model\Raw\AstMeetme The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Oasis\Model\AstMeetme();
        }
        ...
        if (is_array($data)) {
            $entry->setBookid($data['bookid'])
                  ->setConfno($data['confno'])
                  ->setStarttime($data['starttime'])
                  ->setEndtime($data['endtime'])
                  ->setPin($data['pin'])
                  ->setAdminpin($data['adminpin'])
                  ->setOpts($data['opts'])
                  ->setAdminopts($data['adminopts'])
                  ->setRecordingfilename($data['recordingfilename'])
                  ->setRecordingformat($data['recordingformat'])
        ...
        } else if ($data instanceof \Oasis\Model\Raw\AstMeetme) {
            $entry->setBookid($data->getBookid())
                  ->setConfno($data->getConfno())
                  ->setStarttime($data->getStarttime())
                  ->setEndtime($data->getEndtime())
                  ->setPin($data->getPin())
                  ->setAdminpin($data->getAdminpin())
                  ->setOpts($data->getOpts())
                  ->setAdminopts($data->getAdminopts())
                  ->setRecordingfilename($data->getRecordingfilename())
                  ->setRecordingformat($data->getRecordingformat())
                  ->setMaxusers($data->getMaxusers())
