...
/**
 * Data Mapper implementation for Oasis\Model\KamTrunksLcrRule
 *
 * @package Oasis\Mapper\Sql
 * @subpackage Raw
 * @author Luis Felipe Garcia
...
class KamTrunksLcrRule extends MapperAbstract
{
    protected $_modelName = 'Oasis\\Model\\KamTrunksLcrRule';
     ...
     *
     * @param Oasis\Model\Raw\KamTrunksLcrRule $model
     * @return array
     */
    public function toArray($model)
    {
        if (!$model instanceof \Oasis\Model\Raw\KamTrunksLcrRule) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Oasis\Model\Raw\KamTrunksLcrRule object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Oasis\Model\\KamTrunksLcrRule object in toArray for " . get_class($this);
            }
     ...
     *
     * @return Oasis\\Mapper\\Sql\\DbTable\\KamTrunksLcrRule
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Oasis\\Mapper\\Sql\\DbTable\\KamTrunksLcrRule');
        }
     ...
     *
     * @param Oasis\Model\Raw\KamTrunksLcrRule $model The model to delete
    ...
    public function delete(\Oasis\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Oasis\Model\Raw\KamTrunksLcrRule) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Oasis\\Model\\KamTrunksLcrRule object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Oasis\\Model\\KamTrunksLcrRule object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Oasis\\Mapper\\Sql\\DbTable\\KamTrunksLcrRule', $capitalizedFk);
     ...
     */
    public function save(\Oasis\Model\Raw\KamTrunksLcrRule $model)
    {
     ...
     * @return boolean If the save action was successful
     */
    public function saveRecursive(\Oasis\Model\Raw\KamTrunksLcrRule $model, $useTransaction = true, $transactionTag = null)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag);
    }

    protected function _save(\Oasis\Model\Raw\KamTrunksLcrRule $model,
     ...
     * @param Oasis\Model\Raw\KamTrunksLcrRule|null $entry The object to load the data into, or null to have one created
     * @return Oasis\Model\Raw\KamTrunksLcrRule The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Oasis\Model\KamTrunksLcrRule();
        }
        ...
        } else if ($data instanceof \Oasis\Model\Raw\KamTrunksLcrRule) {
