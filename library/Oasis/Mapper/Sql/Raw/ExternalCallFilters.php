...
{
    protected $_modelName = 'Oasis\\Model\\ExternalCallFilters';
     ...
     *
     * @param Oasis\Model\Raw\ExternalCallFilters $model
     * @return array
     */
    public function toArray($model, $fields = array())
    {

        if (!$model instanceof \Oasis\Model\Raw\ExternalCallFilters) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Oasis\Model\Raw\ExternalCallFilters object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Oasis\Model\\ExternalCallFilters object in toArray for " . get_class($this);
            }
     ...
     *
     * @return Oasis\\Mapper\\Sql\\DbTable\\ExternalCallFilters
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Oasis\\Mapper\\Sql\\DbTable\\ExternalCallFilters');
        }
     ...
     *
     * @param Oasis\Model\Raw\ExternalCallFilters $model The model to delete
    ...
    public function delete(\Oasis\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Oasis\Model\Raw\ExternalCallFilters) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Oasis\\Model\\ExternalCallFilters object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Oasis\\Model\\ExternalCallFilters object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Oasis\\Mapper\\Sql\\DbTable\\ExternalCallFilters', $capitalizedFk);
     ...
     */
    public function save(\Oasis\Model\Raw\ExternalCallFilters $model, $forceInsert = false)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Oasis\Model\Raw\ExternalCallFilters $model, $useTransaction = true,
     ...
     * @param Oasis\Model\Raw\ExternalCallFilters|null $entry The object to load the data into, or null to have one created
     * @return Oasis\Model\Raw\ExternalCallFilters The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Oasis\Model\ExternalCallFilters();
        }
        ...
        } else if ($data instanceof \Oasis\Model\Raw\ExternalCallFilters) {
