...
/**
 * Data Mapper implementation for Oasis\Model\AstExtensions
 *
 * @package Oasis\Mapper\Sql
 * @subpackage Raw
 * @author Luis Felipe Garcia
...
class AstExtensions extends MapperAbstract
{
    protected $_modelName = 'Oasis\\Model\\AstExtensions';
     ...
     *
     * @param Oasis\Model\Raw\AstExtensions $model
     * @return array
     */
    public function toArray($model)
    {
        if (!$model instanceof \Oasis\Model\Raw\AstExtensions) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Oasis\Model\Raw\AstExtensions object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Oasis\Model\\AstExtensions object in toArray for " . get_class($this);
            }
     ...
     *
     * @return Oasis\\Mapper\\Sql\\DbTable\\AstExtensions
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Oasis\\Mapper\\Sql\\DbTable\\AstExtensions');
        }
     ...
     *
     * @param Oasis\Model\Raw\AstExtensions $model The model to delete
    ...
    public function delete(\Oasis\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Oasis\Model\Raw\AstExtensions) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Oasis\\Model\\AstExtensions object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Oasis\\Model\\AstExtensions object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Oasis\\Mapper\\Sql\\DbTable\\AstExtensions', $capitalizedFk);
     ...
     */
    public function save(\Oasis\Model\Raw\AstExtensions $model)
    {
     ...
     * @return boolean If the save action was successful
     */
    public function saveRecursive(\Oasis\Model\Raw\AstExtensions $model, $useTransaction = true, $transactionTag = null)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag);
    }

    protected function _save(\Oasis\Model\Raw\AstExtensions $model,
    ...
    )
    {
        ...
        try {
            ...
            if (!empty($primaryKey)) {
                $message .= ' id:';
                $message .= ' id => ' . $primaryKey['id'];
                $message .= ' context => ' . $primaryKey['context'];
                $message .= ' exten => ' . $primaryKey['exten'];
                $message .= ' priority => ' . $primaryKey['priority'];
            } else {
     ...
     * @param Oasis\Model\Raw\AstExtensions|null $entry The object to load the data into, or null to have one created
     * @return Oasis\Model\Raw\AstExtensions The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Oasis\Model\AstExtensions();
        }
        ...
        } else if ($data instanceof \Oasis\Model\Raw\AstExtensions) {
            $entry->setId($data->getId())
                  ->setContext($data->getContext())
                  ->setExten($data->getExten())
                  ->setPriority($data->getPriority())
                  ->setApp($data->getApp())
                  ->setAppdata($data->getAppdata());
