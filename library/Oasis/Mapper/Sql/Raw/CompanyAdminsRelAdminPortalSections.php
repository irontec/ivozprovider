...
/**
 * Data Mapper implementation for Oasis\Model\CompanyAdminsRelAdminPortalSections
 *
 * @package Oasis\Mapper\Sql
 * @subpackage Raw
 * @author Luis Felipe Garcia
...
class CompanyAdminsRelAdminPortalSections extends MapperAbstract
{
    protected $_modelName = 'Oasis\\Model\\CompanyAdminsRelAdminPortalSections';
     ...
     *
     * @param Oasis\Model\Raw\CompanyAdminsRelAdminPortalSections $model
     * @return array
     */
    public function toArray($model)
    {
        if (!$model instanceof \Oasis\Model\Raw\CompanyAdminsRelAdminPortalSections) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Oasis\Model\Raw\CompanyAdminsRelAdminPortalSections object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Oasis\Model\\CompanyAdminsRelAdminPortalSections object in toArray for " . get_class($this);
            }
        ...
        $result = array(
            'id' => $model->getId(),
            'companyAdminId' => $model->getCompanyAdminId(),
            'adminPortalSectionId' => $model->getAdminPortalSectionId(),
            'accessPrivileges' => $model->getAccessPrivileges(),
        );
     ...
     *
     * @return Oasis\\Mapper\\Sql\\DbTable\\CompanyAdminsRelAdminPortalSections
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Oasis\\Mapper\\Sql\\DbTable\\CompanyAdminsRelAdminPortalSections');
        }
     ...
     *
     * @param Oasis\Model\Raw\CompanyAdminsRelAdminPortalSections $model The model to delete
    ...
    public function delete(\Oasis\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Oasis\Model\Raw\CompanyAdminsRelAdminPortalSections) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Oasis\\Model\\CompanyAdminsRelAdminPortalSections object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Oasis\\Model\\CompanyAdminsRelAdminPortalSections object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Oasis\\Mapper\\Sql\\DbTable\\CompanyAdminsRelAdminPortalSections', $capitalizedFk);
     ...
     */
    public function save(\Oasis\Model\Raw\CompanyAdminsRelAdminPortalSections $model)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Oasis\Model\Raw\CompanyAdminsRelAdminPortalSections $model, $useTransaction = true, $transactionTag = null)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag);
    }

    protected function _save(\Oasis\Model\Raw\CompanyAdminsRelAdminPortalSections $model,
     ...
     * @param Oasis\Model\Raw\CompanyAdminsRelAdminPortalSections|null $entry The object to load the data into, or null to have one created
     * @return Oasis\Model\Raw\CompanyAdminsRelAdminPortalSections The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Oasis\Model\CompanyAdminsRelAdminPortalSections();
        }
        ...
        if (is_array($data)) {
            $entry->setId($data['id'])
                  ->setCompanyAdminId($data['companyAdminId'])
                  ->setAdminPortalSectionId($data['adminPortalSectionId'])
                  ->setAccessPrivileges($data['accessPrivileges']);
        ...
        } else if ($data instanceof \Oasis\Model\Raw\CompanyAdminsRelAdminPortalSections) {
            $entry->setId($data->getId())
                  ->setCompanyAdminId($data->getCompanyAdminId())
                  ->setAdminPortalSectionId($data->getAdminPortalSectionId())
                  ->setAccessPrivileges($data->getAccessPrivileges());
