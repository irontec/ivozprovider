...
/**
 * Data Mapper implementation for Oasis\Model\KamTrunksLcrRuleTarget
 *
 * @package Oasis\Mapper\Sql
 * @subpackage Raw
 * @author Luis Felipe Garcia
...
class KamTrunksLcrRuleTarget extends MapperAbstract
{
    protected $_modelName = 'Oasis\\Model\\KamTrunksLcrRuleTarget';
     ...
     *
     * @param Oasis\Model\Raw\KamTrunksLcrRuleTarget $model
     * @return array
     */
    public function toArray($model)
    {
        if (!$model instanceof \Oasis\Model\Raw\KamTrunksLcrRuleTarget) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Oasis\Model\Raw\KamTrunksLcrRuleTarget object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Oasis\Model\\KamTrunksLcrRuleTarget object in toArray for " . get_class($this);
            }
     ...
     *
     * @return Oasis\\Mapper\\Sql\\DbTable\\KamTrunksLcrRuleTarget
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Oasis\\Mapper\\Sql\\DbTable\\KamTrunksLcrRuleTarget');
        }
     ...
     *
     * @param Oasis\Model\Raw\KamTrunksLcrRuleTarget $model The model to delete
    ...
    public function delete(\Oasis\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Oasis\Model\Raw\KamTrunksLcrRuleTarget) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Oasis\\Model\\KamTrunksLcrRuleTarget object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Oasis\\Model\\KamTrunksLcrRuleTarget object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Oasis\\Mapper\\Sql\\DbTable\\KamTrunksLcrRuleTarget', $capitalizedFk);
     ...
     */
    public function save(\Oasis\Model\Raw\KamTrunksLcrRuleTarget $model)
    {
     ...
     * @return boolean If the save action was successful
     */
    public function saveRecursive(\Oasis\Model\Raw\KamTrunksLcrRuleTarget $model, $useTransaction = true, $transactionTag = null)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag);
    }

    protected function _save(\Oasis\Model\Raw\KamTrunksLcrRuleTarget $model,
     ...
     * @param Oasis\Model\Raw\KamTrunksLcrRuleTarget|null $entry The object to load the data into, or null to have one created
     * @return Oasis\Model\Raw\KamTrunksLcrRuleTarget The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Oasis\Model\KamTrunksLcrRuleTarget();
        }
        ...
        } else if ($data instanceof \Oasis\Model\Raw\KamTrunksLcrRuleTarget) {
