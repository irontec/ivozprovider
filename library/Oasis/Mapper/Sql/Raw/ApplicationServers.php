...
{
    protected $_modelName = 'Oasis\\Model\\ApplicationServers';
     ...
     *
     * @param Oasis\Model\Raw\ApplicationServers $model
     * @return array
     */
    public function toArray($model, $fields = array())
    {

        if (!$model instanceof \Oasis\Model\Raw\ApplicationServers) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Oasis\Model\Raw\ApplicationServers object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Oasis\Model\\ApplicationServers object in toArray for " . get_class($this);
            }
     ...
     *
     * @return Oasis\\Mapper\\Sql\\DbTable\\ApplicationServers
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Oasis\\Mapper\\Sql\\DbTable\\ApplicationServers');
        }
     ...
     *
     * @param Oasis\Model\Raw\ApplicationServers $model The model to delete
    ...
    public function delete(\Oasis\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Oasis\Model\Raw\ApplicationServers) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Oasis\\Model\\ApplicationServers object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Oasis\\Model\\ApplicationServers object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Oasis\\Mapper\\Sql\\DbTable\\ApplicationServers', $capitalizedFk);
     ...
     */
    public function save(\Oasis\Model\Raw\ApplicationServers $model, $forceInsert = false)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Oasis\Model\Raw\ApplicationServers $model, $useTransaction = true,
     ...
     * @param Oasis\Model\Raw\ApplicationServers|null $entry The object to load the data into, or null to have one created
     * @return Oasis\Model\Raw\ApplicationServers The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Oasis\Model\ApplicationServers();
        }
        ...
        } else if ($data instanceof \Oasis\Model\Raw\ApplicationServers) {
