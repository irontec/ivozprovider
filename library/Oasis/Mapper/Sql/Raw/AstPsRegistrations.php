...
/**
 * Data Mapper implementation for Oasis\Model\AstPsRegistrations
 *
 * @package Oasis\Mapper\Sql
 * @subpackage Raw
 * @author Luis Felipe Garcia
...
class AstPsRegistrations extends MapperAbstract
{
    protected $_modelName = 'Oasis\\Model\\AstPsRegistrations';
     ...
     *
     * @param Oasis\Model\Raw\AstPsRegistrations $model
     * @return array
     */
    public function toArray($model)
    {
        if (!$model instanceof \Oasis\Model\Raw\AstPsRegistrations) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Oasis\Model\Raw\AstPsRegistrations object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Oasis\Model\\AstPsRegistrations object in toArray for " . get_class($this);
            }
        ...
        $result = array(
            ...
            'auth_rejection_permanent' => $model->getAuthRejectionPermanent(),
            'client_uri' => $model->getClientUri(),
            'contact_user' => $model->getContactUser(),
            'expiration' => $model->getExpiration(),
            'max_retries' => $model->getMaxRetries(),
            'outbound_auth' => $model->getOutboundAuth(),
            'outbound_proxy' => $model->getOutboundProxy(),
            'retry_interval' => $model->getRetryInterval(),
            'forbidden_retry_interval' => $model->getForbiddenRetryInterval(),
            'server_uri' => $model->getServerUri(),
     ...
     *
     * @return Oasis\\Mapper\\Sql\\DbTable\\AstPsRegistrations
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Oasis\\Mapper\\Sql\\DbTable\\AstPsRegistrations');
        }
     ...
     *
     * @param Oasis\Model\Raw\AstPsRegistrations $model The model to delete
    ...
    public function delete(\Oasis\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Oasis\Model\Raw\AstPsRegistrations) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Oasis\\Model\\AstPsRegistrations object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Oasis\\Model\\AstPsRegistrations object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Oasis\\Mapper\\Sql\\DbTable\\AstPsRegistrations', $capitalizedFk);
     ...
     */
    public function save(\Oasis\Model\Raw\AstPsRegistrations $model)
    {
     ...
     * @return boolean If the save action was successful
     */
    public function saveRecursive(\Oasis\Model\Raw\AstPsRegistrations $model, $useTransaction = true, $transactionTag = null)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag);
    }

    protected function _save(\Oasis\Model\Raw\AstPsRegistrations $model,
     ...
     * @param Oasis\Model\Raw\AstPsRegistrations|null $entry The object to load the data into, or null to have one created
     * @return Oasis\Model\Raw\AstPsRegistrations The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Oasis\Model\AstPsRegistrations();
        }
        ...
        if (is_array($data)) {
            $entry->setSorceryId($data['sorcery_id'])
                  ->setAuthRejectionPermanent($data['auth_rejection_permanent'])
                  ->setClientUri($data['client_uri'])
                  ->setContactUser($data['contact_user'])
                  ->setExpiration($data['expiration'])
                  ->setMaxRetries($data['max_retries'])
                  ->setOutboundAuth($data['outbound_auth'])
                  ->setOutboundProxy($data['outbound_proxy'])
                  ->setRetryInterval($data['retry_interval'])
                  ->setForbiddenRetryInterval($data['forbidden_retry_interval'])
                  ->setServerUri($data['server_uri'])
        ...
        } else if ($data instanceof \Oasis\Model\Raw\AstPsRegistrations) {
            $entry->setSorceryId($data->getSorceryId())
                  ->setAuthRejectionPermanent($data->getAuthRejectionPermanent())
                  ->setClientUri($data->getClientUri())
                  ->setContactUser($data->getContactUser())
                  ->setExpiration($data->getExpiration())
                  ->setMaxRetries($data->getMaxRetries())
                  ->setOutboundAuth($data->getOutboundAuth())
                  ->setOutboundProxy($data->getOutboundProxy())
                  ->setRetryInterval($data->getRetryInterval())
                  ->setForbiddenRetryInterval($data->getForbiddenRetryInterval())
                  ->setServerUri($data->getServerUri())
