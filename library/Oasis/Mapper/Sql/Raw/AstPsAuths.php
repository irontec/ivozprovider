...
/**
 * Data Mapper implementation for Oasis\Model\AstPsAuths
 *
 * @package Oasis\Mapper\Sql
 * @subpackage Raw
 * @author Luis Felipe Garcia
...
class AstPsAuths extends MapperAbstract
{
    protected $_modelName = 'Oasis\\Model\\AstPsAuths';
     ...
     *
     * @param Oasis\Model\Raw\AstPsAuths $model
     * @return array
     */
    public function toArray($model)
    {
        if (!$model instanceof \Oasis\Model\Raw\AstPsAuths) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Oasis\Model\Raw\AstPsAuths object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Oasis\Model\\AstPsAuths object in toArray for " . get_class($this);
            }
        ...
        $result = array(
            ...
            'auth_type' => $model->getAuthType(),
            'nonce_lifetime' => $model->getNonceLifetime(),
            'md5_cred' => $model->getMd5Cred(),
     ...
     *
     * @return Oasis\\Mapper\\Sql\\DbTable\\AstPsAuths
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Oasis\\Mapper\\Sql\\DbTable\\AstPsAuths');
        }
     ...
     *
     * @param Oasis\Model\Raw\AstPsAuths $model The model to delete
    ...
    public function delete(\Oasis\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Oasis\Model\Raw\AstPsAuths) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Oasis\\Model\\AstPsAuths object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Oasis\\Model\\AstPsAuths object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Oasis\\Mapper\\Sql\\DbTable\\AstPsAuths', $capitalizedFk);
     ...
     */
    public function save(\Oasis\Model\Raw\AstPsAuths $model)
    {
     ...
     * @return boolean If the save action was successful
     */
    public function saveRecursive(\Oasis\Model\Raw\AstPsAuths $model, $useTransaction = true, $transactionTag = null)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag);
    }

    protected function _save(\Oasis\Model\Raw\AstPsAuths $model,
     ...
     * @param Oasis\Model\Raw\AstPsAuths|null $entry The object to load the data into, or null to have one created
     * @return Oasis\Model\Raw\AstPsAuths The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Oasis\Model\AstPsAuths();
        }
        ...
        if (is_array($data)) {
            $entry->setSorceryId($data['sorcery_id'])
                  ...
                  ->setNonceLifetime($data['nonce_lifetime'])
                  ->setMd5Cred($data['md5_cred'])
        ...
        } else if ($data instanceof \Oasis\Model\Raw\AstPsAuths) {
            $entry->setSorceryId($data->getSorceryId())
                  ->setAuthType($data->getAuthType())
                  ->setNonceLifetime($data->getNonceLifetime())
                  ->setMd5Cred($data->getMd5Cred())
