...
/**
 * Data Mapper implementation for Oasis\Model\KamTrunksTrusted
 *
 * @package Oasis\Mapper\Sql
 * @subpackage Raw
 * @author Luis Felipe Garcia
...
class KamTrunksTrusted extends MapperAbstract
{
    protected $_modelName = 'Oasis\\Model\\KamTrunksTrusted';
     ...
     *
     * @param Oasis\Model\Raw\KamTrunksTrusted $model
     * @return array
     */
    public function toArray($model)
    {
        if (!$model instanceof \Oasis\Model\Raw\KamTrunksTrusted) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Oasis\Model\Raw\KamTrunksTrusted object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Oasis\Model\\KamTrunksTrusted object in toArray for " . get_class($this);
            }
     ...
     *
     * @return Oasis\\Mapper\\Sql\\DbTable\\KamTrunksTrusted
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Oasis\\Mapper\\Sql\\DbTable\\KamTrunksTrusted');
        }
     ...
     *
     * @param Oasis\Model\Raw\KamTrunksTrusted $model The model to delete
    ...
    public function delete(\Oasis\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Oasis\Model\Raw\KamTrunksTrusted) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Oasis\\Model\\KamTrunksTrusted object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Oasis\\Model\\KamTrunksTrusted object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Oasis\\Mapper\\Sql\\DbTable\\KamTrunksTrusted', $capitalizedFk);
     ...
     */
    public function save(\Oasis\Model\Raw\KamTrunksTrusted $model)
    {
     ...
     * @return boolean If the save action was successful
     */
    public function saveRecursive(\Oasis\Model\Raw\KamTrunksTrusted $model, $useTransaction = true, $transactionTag = null)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag);
    }

    protected function _save(\Oasis\Model\Raw\KamTrunksTrusted $model,
     ...
     * @param Oasis\Model\Raw\KamTrunksTrusted|null $entry The object to load the data into, or null to have one created
     * @return Oasis\Model\Raw\KamTrunksTrusted The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Oasis\Model\KamTrunksTrusted();
        }
        ...
        } else if ($data instanceof \Oasis\Model\Raw\KamTrunksTrusted) {
