...
/**
 * Data Mapper implementation for Oasis\Model\NumericTransformRules
 *
 * @package Oasis\Mapper\Sql
 * @subpackage Raw
 * @author Luis Felipe Garcia
...
class NumericTransformRules extends MapperAbstract
{
    protected $_modelName = 'Oasis\\Model\\NumericTransformRules';
     ...
     *
     * @param Oasis\Model\Raw\NumericTransformRules $model
     * @return array
     */
    public function toArray($model)
    {
        if (!$model instanceof \Oasis\Model\Raw\NumericTransformRules) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Oasis\Model\Raw\NumericTransformRules object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Oasis\Model\\NumericTransformRules object in toArray for " . get_class($this);
            }
     ...
     *
     * @return Oasis\\Mapper\\Sql\\DbTable\\NumericTransformRules
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Oasis\\Mapper\\Sql\\DbTable\\NumericTransformRules');
        }
     ...
     *
     * @param Oasis\Model\Raw\NumericTransformRules $model The model to delete
    ...
    public function delete(\Oasis\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Oasis\Model\Raw\NumericTransformRules) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Oasis\\Model\\NumericTransformRules object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Oasis\\Model\\NumericTransformRules object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Oasis\\Mapper\\Sql\\DbTable\\NumericTransformRules', $capitalizedFk);
     ...
     */
    public function save(\Oasis\Model\Raw\NumericTransformRules $model)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Oasis\Model\Raw\NumericTransformRules $model, $useTransaction = true, $transactionTag = null)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag);
    }

    protected function _save(\Oasis\Model\Raw\NumericTransformRules $model,
    ...
    )
    {
        ...
        try {
            ...
            if ($recursive) {
                if ($model->getPeerServesRelNumericTransformRules(null, null, true) !== null) {
                    $peerServesRelNumericTransformRules = $model->getPeerServesRelNumericTransformRules();

                    if (!is_array($peerServesRelNumericTransformRules)) {

                        $peerServesRelNumericTransformRules = array($peerServesRelNumericTransformRules);
                    }

                    foreach ($peerServesRelNumericTransformRules as $value) {
                        $value->setNumericTransformRuleId($primaryKey)
     ...
     * @param Oasis\Model\Raw\NumericTransformRules|null $entry The object to load the data into, or null to have one created
     * @return Oasis\Model\Raw\NumericTransformRules The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Oasis\Model\NumericTransformRules();
        }
        ...
        } else if ($data instanceof \Oasis\Model\Raw\NumericTransformRules) {
