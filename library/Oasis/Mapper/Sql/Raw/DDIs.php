...
{
    protected $_modelName = 'Oasis\\Model\\DDIs';
     ...
     *
     * @param Oasis\Model\Raw\DDIs $model
     * @return array
     */
    public function toArray($model, $fields = array())
    {

        if (!$model instanceof \Oasis\Model\Raw\DDIs) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Oasis\Model\Raw\DDIs object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Oasis\Model\\DDIs object in toArray for " . get_class($this);
            }
     ...
     *
     * @return Oasis\\Mapper\\Sql\\DbTable\\DDIs
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Oasis\\Mapper\\Sql\\DbTable\\DDIs');
        }
     ...
     *
     * @param Oasis\Model\Raw\DDIs $model The model to delete
    ...
    public function delete(\Oasis\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Oasis\Model\Raw\DDIs) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Oasis\\Model\\DDIs object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Oasis\\Model\\DDIs object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Oasis\\Mapper\\Sql\\DbTable\\DDIs', $capitalizedFk);
     ...
     */
    public function save(\Oasis\Model\Raw\DDIs $model, $forceInsert = false)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Oasis\Model\Raw\DDIs $model, $useTransaction = true,
     ...
     * @param Oasis\Model\Raw\DDIs|null $entry The object to load the data into, or null to have one created
     * @return Oasis\Model\Raw\DDIs The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Oasis\Model\DDIs();
        }
        ...
        } else if ($data instanceof \Oasis\Model\Raw\DDIs) {
