...
/**
 * Data Mapper implementation for Oasis\Model\CDR
 *
 * @package Oasis\Mapper\Sql
 * @subpackage Raw
 * @author Luis Felipe Garcia
...
class CDR extends MapperAbstract
{
    protected $_modelName = 'Oasis\\Model\\CDR';
     ...
     *
     * @param Oasis\Model\Raw\CDR $model
     * @return array
     */
    public function toArray($model)
    {
        if (!$model instanceof \Oasis\Model\Raw\CDR) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Oasis\Model\Raw\CDR object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Oasis\Model\\CDR object in toArray for " . get_class($this);
            }
     ...
     *
     * @return Oasis\\Mapper\\Sql\\DbTable\\CDR
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Oasis\\Mapper\\Sql\\DbTable\\CDR');
        }
     ...
     *
     * @param Oasis\Model\Raw\CDR $model The model to delete
    ...
    public function delete(\Oasis\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Oasis\Model\Raw\CDR) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Oasis\\Model\\CDR object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Oasis\\Model\\CDR object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Oasis\\Mapper\\Sql\\DbTable\\CDR', $capitalizedFk);
     ...
     */
    public function save(\Oasis\Model\Raw\CDR $model)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Oasis\Model\Raw\CDR $model, $useTransaction = true, $transactionTag = null)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag);
    }

    protected function _save(\Oasis\Model\Raw\CDR $model,
     ...
     * @param Oasis\Model\Raw\CDR|null $entry The object to load the data into, or null to have one created
     * @return Oasis\Model\Raw\CDR The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Oasis\Model\CDR();
        }
        ...
        } else if ($data instanceof \Oasis\Model\Raw\CDR) {
