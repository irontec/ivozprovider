...
/**
 * Data Mapper implementation for Oasis\Model\Brands
 *
 * @package Oasis\Mapper\Sql
 * @subpackage Raw
 * @author Luis Felipe Garcia
...
{
    protected $_modelName = 'Oasis\\Model\\Brands';
     ...
     *
     * @param Oasis\Model\Raw\Brands $model
     * @return array
     */
    public function toArray($model, $fields = array())
    {

        if (!$model instanceof \Oasis\Model\Raw\Brands) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Oasis\Model\Raw\Brands object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Oasis\Model\\Brands object in toArray for " . get_class($this);
            }
     ...
     *
     * @return Oasis\\Mapper\\Sql\\DbTable\\Brands
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Oasis\\Mapper\\Sql\\DbTable\\Brands');
        }
     ...
     *
     * @param Oasis\Model\Raw\Brands $model The model to delete
    ...
    public function delete(\Oasis\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Oasis\Model\Raw\Brands) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Oasis\\Model\\Brands object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Oasis\\Model\\Brands object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Oasis\\Mapper\\Sql\\DbTable\\Brands', $capitalizedFk);
     ...
     */
    public function save(\Oasis\Model\Raw\Brands $model, $forceInsert = false)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Oasis\Model\Raw\Brands $model, $useTransaction = true,
            $transactionTag = null, $forceInsert = false)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag, $forceInsert);
    }

    protected function _save(\Oasis\Model\Raw\Brands $model,
     ...
     * @param Oasis\Model\Raw\Brands|null $entry The object to load the data into, or null to have one created
     * @return Oasis\Model\Raw\Brands The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Oasis\Model\Brands();
        }
        ...
        } else if ($data instanceof \Oasis\Model\Raw\Brands) {
