...
/**
 * Data Mapper implementation for Oasis\Model\TerminalManufacturers
 *
 * @package Oasis\Mapper\Sql
 * @subpackage Raw
 * @author Luis Felipe Garcia
...
{
    protected $_modelName = 'Oasis\\Model\\TerminalManufacturers';
     ...
     *
     * @param Oasis\Model\Raw\TerminalManufacturers $model
     * @return array
     */
    public function toArray($model, $fields = array())
    {

        if (!$model instanceof \Oasis\Model\Raw\TerminalManufacturers) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Oasis\Model\Raw\TerminalManufacturers object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Oasis\Model\\TerminalManufacturers object in toArray for " . get_class($this);
            }
     ...
     *
     * @return Oasis\\Mapper\\Sql\\DbTable\\TerminalManufacturers
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Oasis\\Mapper\\Sql\\DbTable\\TerminalManufacturers');
        }
     ...
     *
     * @param Oasis\Model\Raw\TerminalManufacturers $model The model to delete
    ...
    public function delete(\Oasis\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Oasis\Model\Raw\TerminalManufacturers) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Oasis\\Model\\TerminalManufacturers object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Oasis\\Model\\TerminalManufacturers object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Oasis\\Mapper\\Sql\\DbTable\\TerminalManufacturers', $capitalizedFk);
     ...
     */
    public function save(\Oasis\Model\Raw\TerminalManufacturers $model, $forceInsert = false)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Oasis\Model\Raw\TerminalManufacturers $model, $useTransaction = true,
            $transactionTag = null, $forceInsert = false)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag, $forceInsert);
    }

    protected function _save(\Oasis\Model\Raw\TerminalManufacturers $model,
     ...
     * @param Oasis\Model\Raw\TerminalManufacturers|null $entry The object to load the data into, or null to have one created
     * @return Oasis\Model\Raw\TerminalManufacturers The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Oasis\Model\TerminalManufacturers();
        }
        ...
        } else if ($data instanceof \Oasis\Model\Raw\TerminalManufacturers) {
