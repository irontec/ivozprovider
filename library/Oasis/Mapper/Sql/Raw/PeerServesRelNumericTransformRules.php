...
/**
 * Data Mapper implementation for Oasis\Model\PeerServesRelNumericTransformRules
 *
 * @package Oasis\Mapper\Sql
 * @subpackage Raw
 * @author Luis Felipe Garcia
...
class PeerServesRelNumericTransformRules extends MapperAbstract
{
    protected $_modelName = 'Oasis\\Model\\PeerServesRelNumericTransformRules';
     ...
     *
     * @param Oasis\Model\Raw\PeerServesRelNumericTransformRules $model
     * @return array
     */
    public function toArray($model)
    {
        if (!$model instanceof \Oasis\Model\Raw\PeerServesRelNumericTransformRules) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Oasis\Model\Raw\PeerServesRelNumericTransformRules object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Oasis\Model\\PeerServesRelNumericTransformRules object in toArray for " . get_class($this);
            }
        ...
        $result = array(
            ...
            'numericTransformRuleId' => $model->getNumericTransformRuleId(),
            'type' => $model->getType(),
            'applyTo' => $model->getApplyTo(),
        );
     ...
     *
     * @return Oasis\\Mapper\\Sql\\DbTable\\PeerServesRelNumericTransformRules
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Oasis\\Mapper\\Sql\\DbTable\\PeerServesRelNumericTransformRules');
        }
     ...
     *
     * @param Oasis\Model\Raw\PeerServesRelNumericTransformRules $model The model to delete
    ...
    public function delete(\Oasis\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Oasis\Model\Raw\PeerServesRelNumericTransformRules) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Oasis\\Model\\PeerServesRelNumericTransformRules object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Oasis\\Model\\PeerServesRelNumericTransformRules object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Oasis\\Mapper\\Sql\\DbTable\\PeerServesRelNumericTransformRules', $capitalizedFk);
     ...
     */
    public function save(\Oasis\Model\Raw\PeerServesRelNumericTransformRules $model)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Oasis\Model\Raw\PeerServesRelNumericTransformRules $model, $useTransaction = true, $transactionTag = null)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag);
    }

    protected function _save(\Oasis\Model\Raw\PeerServesRelNumericTransformRules $model,
     ...
     * @param Oasis\Model\Raw\PeerServesRelNumericTransformRules|null $entry The object to load the data into, or null to have one created
     * @return Oasis\Model\Raw\PeerServesRelNumericTransformRules The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Oasis\Model\PeerServesRelNumericTransformRules();
        }
        ...
        if (is_array($data)) {
            $entry->setId($data['id'])
                  ...
                  ->setNumericTransformRuleId($data['numericTransformRuleId'])
                  ->setType($data['type'])
        ...
        } else if ($data instanceof \Oasis\Model\Raw\PeerServesRelNumericTransformRules) {
            $entry->setId($data->getId())
                  ->setPeerServerId($data->getPeerServerId())
                  ->setNumericTransformRuleId($data->getNumericTransformRuleId())
                  ->setType($data->getType())
                  ->setApplyTo($data->getApplyTo());
