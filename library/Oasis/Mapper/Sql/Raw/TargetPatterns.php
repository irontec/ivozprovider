...
/**
 * Data Mapper implementation for Oasis\Model\TargetPatterns
 *
 * @package Oasis\Mapper\Sql
 * @subpackage Raw
 * @author Luis Felipe Garcia
...
{
    protected $_modelName = 'Oasis\\Model\\TargetPatterns';
     ...
     *
     * @param Oasis\Model\Raw\TargetPatterns $model
     * @return array
     */
    public function toArray($model, $fields = array())
    {

        if (!$model instanceof \Oasis\Model\Raw\TargetPatterns) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Oasis\Model\Raw\TargetPatterns object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Oasis\Model\\TargetPatterns object in toArray for " . get_class($this);
            }
     ...
     *
     * @return Oasis\\Mapper\\Sql\\DbTable\\TargetPatterns
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Oasis\\Mapper\\Sql\\DbTable\\TargetPatterns');
        }
     ...
     *
     * @param Oasis\Model\Raw\TargetPatterns $model The model to delete
    ...
    public function delete(\Oasis\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Oasis\Model\Raw\TargetPatterns) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Oasis\\Model\\TargetPatterns object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Oasis\\Model\\TargetPatterns object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Oasis\\Mapper\\Sql\\DbTable\\TargetPatterns', $capitalizedFk);
     ...
     */
    public function save(\Oasis\Model\Raw\TargetPatterns $model, $forceInsert = false)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Oasis\Model\Raw\TargetPatterns $model, $useTransaction = true,
            $transactionTag = null, $forceInsert = false)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag, $forceInsert);
    }

    protected function _save(\Oasis\Model\Raw\TargetPatterns $model,
     ...
     * @param Oasis\Model\Raw\TargetPatterns|null $entry The object to load the data into, or null to have one created
     * @return Oasis\Model\Raw\TargetPatterns The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Oasis\Model\TargetPatterns();
        }
        ...
        } else if ($data instanceof \Oasis\Model\Raw\TargetPatterns) {
