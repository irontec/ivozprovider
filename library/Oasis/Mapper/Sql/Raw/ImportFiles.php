...
/**
 * Data Mapper implementation for Oasis\Model\ImportFiles
 *
 * @package Oasis\Mapper\Sql
 * @subpackage Raw
 * @author Luis Felipe Garcia
...
class ImportFiles extends MapperAbstract
{
    protected $_modelName = 'Oasis\\Model\\ImportFiles';
     ...
     *
     * @param Oasis\Model\Raw\ImportFiles $model
     * @return array
     */
    public function toArray($model, $fields = array())
    {

        if (!$model instanceof \Oasis\Model\Raw\ImportFiles) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Oasis\Model\Raw\ImportFiles object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Oasis\Model\\ImportFiles object in toArray for " . get_class($this);
            }
        ...
        if (empty($fields)) {
            $result = array(
                ...
                'type' => $model->getType(),
                'nunRows' => $model->getNunRows(),
                'nunCols' => $model->getNunCols(),
                'uploadedOn' => $model->getUploadedOn(),
                'importedOn' => $model->getImportedOn(),
     ...
     *
     * @return Oasis\\Mapper\\Sql\\DbTable\\ImportFiles
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Oasis\\Mapper\\Sql\\DbTable\\ImportFiles');
        }
     ...
     *
     * @param Oasis\Model\Raw\ImportFiles $model The model to delete
    ...
    public function delete(\Oasis\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Oasis\Model\Raw\ImportFiles) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Oasis\\Model\\ImportFiles object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Oasis\\Model\\ImportFiles object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Oasis\\Mapper\\Sql\\DbTable\\ImportFiles', $capitalizedFk);
     ...
     */
    public function save(\Oasis\Model\Raw\ImportFiles $model)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\Oasis\Model\Raw\ImportFiles $model, $useTransaction = true, $transactionTag = null)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag);
    }

    protected function _save(\Oasis\Model\Raw\ImportFiles $model,
    ...
    )
    {
        ...
        try {
            ...
            if ($recursive) {
                if ($model->getImportPatterns(null, null, true) !== null) {
                    $importPatterns = $model->getImportPatterns();

                    if (!is_array($importPatterns)) {

                        $importPatterns = array($importPatterns);
                    }

                    foreach ($importPatterns as $value) {
                        $value->setImportFileId($primaryKey)
                              ->saveRecursive(false, $transactionTag);
                    }
                }

                if ($model->getImportPlans(null, null, true) !== null) {
                    $importPlans = $model->getImportPlans();

                    if (!is_array($importPlans)) {

                        $importPlans = array($importPlans);
                    }

                    foreach ($importPlans as $value) {
                        $value->setImportFileId($primaryKey)
                              ->saveRecursive(false, $transactionTag);
                    }
                }

                if ($model->getImportPrices(null, null, true) !== null) {
                    $importPrices = $model->getImportPrices();

                    if (!is_array($importPrices)) {

                        $importPrices = array($importPrices);
                    }

                    foreach ($importPrices as $value) {
                        $value->setImportFileId($primaryKey)
                              ->saveRecursive(false, $transactionTag);
                    }
                }
     ...
     * @param Oasis\Model\Raw\ImportFiles|null $entry The object to load the data into, or null to have one created
     * @return Oasis\Model\Raw\ImportFiles The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Oasis\Model\ImportFiles();
        }
        ...
        if (is_array($data)) {
            $entry->setId($data['id'])
                  ...
                  ->setType($data['type'])
                  ->setNunRows($data['nunRows'])
                  ->setNunCols($data['nunCols'])
                  ->setUploadedOn($data['uploadedOn'])
                  ->setImportedOn($data['importedOn'])
