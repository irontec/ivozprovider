...
/**
 * Data Mapper implementation for Oasis\Model\KamTrunksDialog
 *
 * @package Oasis\Mapper\Sql
 * @subpackage Raw
 * @author Luis Felipe Garcia
...
class KamTrunksDialog extends MapperAbstract
{
    protected $_modelName = 'Oasis\\Model\\KamTrunksDialog';
     ...
     *
     * @param Oasis\Model\Raw\KamTrunksDialog $model
     * @return array
     */
    public function toArray($model)
    {
        if (!$model instanceof \Oasis\Model\Raw\KamTrunksDialog) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \Oasis\Model\Raw\KamTrunksDialog object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\Oasis\Model\\KamTrunksDialog object in toArray for " . get_class($this);
            }
        ...
        $result = array(
            'id' => $model->getId(),
            'hash_entry' => $model->getHashEntry(),
            'hash_id' => $model->getHashId(),
            'callid' => $model->getCallid(),
            'from_uri' => $model->getFromUri(),
            'from_tag' => $model->getFromTag(),
            'to_uri' => $model->getToUri(),
            'to_tag' => $model->getToTag(),
            'caller_cseq' => $model->getCallerCseq(),
            'callee_cseq' => $model->getCalleeCseq(),
            'caller_route_set' => $model->getCallerRouteSet(),
            'callee_route_set' => $model->getCalleeRouteSet(),
            'caller_contact' => $model->getCallerContact(),
            'callee_contact' => $model->getCalleeContact(),
            'caller_sock' => $model->getCallerSock(),
            'callee_sock' => $model->getCalleeSock(),
            'state' => $model->getState(),
            'start_time' => $model->getStartTime(),
            'timeout' => $model->getTimeout(),
            'sflags' => $model->getSflags(),
            'iflags' => $model->getIflags(),
            'toroute_name' => $model->getTorouteName(),
            'req_uri' => $model->getReqUri(),
            'xdata' => $model->getXdata(),
        );
     ...
     *
     * @return Oasis\\Mapper\\Sql\\DbTable\\KamTrunksDialog
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('Oasis\\Mapper\\Sql\\DbTable\\KamTrunksDialog');
        }
     ...
     *
     * @param Oasis\Model\Raw\KamTrunksDialog $model The model to delete
    ...
    public function delete(\Oasis\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \Oasis\Model\Raw\KamTrunksDialog) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\Oasis\\Model\\KamTrunksDialog object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\Oasis\\Model\\KamTrunksDialog object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('Oasis\\Mapper\\Sql\\DbTable\\KamTrunksDialog', $capitalizedFk);
     ...
     */
    public function save(\Oasis\Model\Raw\KamTrunksDialog $model)
    {
     ...
     * @return boolean If the save action was successful
     */
    public function saveRecursive(\Oasis\Model\Raw\KamTrunksDialog $model, $useTransaction = true, $transactionTag = null)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag);
    }

    protected function _save(\Oasis\Model\Raw\KamTrunksDialog $model,
     ...
     * @param Oasis\Model\Raw\KamTrunksDialog|null $entry The object to load the data into, or null to have one created
     * @return Oasis\Model\Raw\KamTrunksDialog The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \Oasis\Model\KamTrunksDialog();
        }
        ...
        if (is_array($data)) {
            $entry->setId($data['id'])
                  ->setHashEntry($data['hash_entry'])
                  ->setHashId($data['hash_id'])
                  ->setCallid($data['callid'])
                  ->setFromUri($data['from_uri'])
                  ->setFromTag($data['from_tag'])
                  ->setToUri($data['to_uri'])
                  ->setToTag($data['to_tag'])
                  ->setCallerCseq($data['caller_cseq'])
                  ->setCalleeCseq($data['callee_cseq'])
                  ->setCallerRouteSet($data['caller_route_set'])
                  ->setCalleeRouteSet($data['callee_route_set'])
                  ->setCallerContact($data['caller_contact'])
                  ->setCalleeContact($data['callee_contact'])
                  ->setCallerSock($data['caller_sock'])
                  ->setCalleeSock($data['callee_sock'])
                  ->setState($data['state'])
                  ->setStartTime($data['start_time'])
                  ->setTimeout($data['timeout'])
                  ->setSflags($data['sflags'])
                  ->setIflags($data['iflags'])
                  ->setTorouteName($data['toroute_name'])
                  ->setReqUri($data['req_uri'])
                  ->setXdata($data['xdata']);
        ...
        } else if ($data instanceof \Oasis\Model\Raw\KamTrunksDialog) {
            $entry->setId($data->getId())
                  ->setHashEntry($data->getHashEntry())
                  ->setHashId($data->getHashId())
                  ->setCallid($data->getCallid())
                  ->setFromUri($data->getFromUri())
                  ->setFromTag($data->getFromTag())
                  ->setToUri($data->getToUri())
                  ->setToTag($data->getToTag())
                  ->setCallerCseq($data->getCallerCseq())
                  ->setCalleeCseq($data->getCalleeCseq())
                  ->setCallerRouteSet($data->getCallerRouteSet())
                  ->setCalleeRouteSet($data->getCalleeRouteSet())
                  ->setCallerContact($data->getCallerContact())
                  ->setCalleeContact($data->getCalleeContact())
                  ->setCallerSock($data->getCallerSock())
                  ->setCalleeSock($data->getCalleeSock())
                  ->setState($data->getState())
                  ->setStartTime($data->getStartTime())
                  ->setTimeout($data->getTimeout())
                  ->setSflags($data->getSflags())
                  ->setIflags($data->getIflags())
                  ->setTorouteName($data->getTorouteName())
                  ->setReqUri($data->getReqUri())
                  ->setXdata($data->getXdata());
