...
abstract class TpCdrAbstract
{
    ...
    public function toDto($depth = 0)
    {
        return self::createDto()
            ...
            ->setOriginHost($this->getOriginHost())
            ->setSource($this->getSource())
            ->setOriginId($this->getOriginId())
            ->setTor($this->getTor())
            ->setRequestType($this->getRequestType())
            ->setTenant($this->getTenant())
            ->setCategory($this->getCategory())
            ->setAccount($this->getAccount())
            ->setSubject($this->getSubject())
            ->setDestination($this->getDestination())
            ->setSetupTime($this->getSetupTime())
            ->setAnswerTime($this->getAnswerTime())
            ->setUsage($this->getUsage())
            ->setExtraFields($this->getExtraFields())
            ->setCostSource($this->getCostSource())
            ->setCost($this->getCost())
            ->setCostDetails($this->getCostDetails())
            ->setExtraInfo($this->getExtraInfo())
            ->setCreatedAt($this->getCreatedAt())
            ->setUpdatedAt($this->getUpdatedAt())
            ->setDeletedAt($this->getDeletedAt());
    }
