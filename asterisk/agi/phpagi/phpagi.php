...
class AGI
{
    ...
    {
        // return $this->evaluate("DATABASE GET \"$family\ \"$key\""); // <-- ibc: BUG
    ...
    /**
     * NOOP que no hace nada tampoco, pero al menos veo en "agi debug" el texto de la cadena como argumento.
     */
    ...
    function evaluate($command)
    {
        ...
        // Irontec. Replace multiples spaces with only one.
     ...
     */
    function source_location($file = null, $line = null)
    {

        if (is_null($file)) {
            $backtrace = debug_backtrace();

            foreach ($backtrace as $frame => $data) {
                // Ignore this file
                ...
                if (strpos($data['file'], 'Wrapper') !== false) {
                    continue;
                }

                // Skip Cached Classes
                if (strpos($data['file'], 'cache') !== false) {
                    continue;
                }
...
function phpagi_error_handler($level, $message, $file, $line, $context)
{
    ...
    openlog("phpagi", LOG_ODELAY, LOG_LOCAL6);
