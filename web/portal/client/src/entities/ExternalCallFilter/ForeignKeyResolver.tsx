  ...
  for (const idx in data) {
    switch (data[idx].holidayTargetType) {
      ...
      case 'number':
        data[
          idx
        ].holidayTarget = `${data[idx].holidayNumberCountry} ${data[idx].holidayNumberValue}`;
        break;
      default:
    ...
    switch (data[idx].outOfScheduleTargetType) {
      ...
      case 'number':
        data[
          idx
        ].outOfScheduleTarget = `${data[idx].outOfScheduleNumberCountry} ${data[idx].outOfScheduleNumberValue}`;
        break;
      default:
        // eslint-disable-next-line no-console
