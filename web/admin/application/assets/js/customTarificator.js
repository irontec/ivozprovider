;(function load($) {
    ...
    var __namespace__ = "custom.Tarificator";
    $.custom = $.custom || {};

    $.widget("custom.Tarificator", $.klearmatrix.genericdialog,  {
        options: {
            data : null,
            moduleName: 'Tarificator'
        },
        _create: function() {
        ...
        _checkValidality: function () {
            ...
            var self = this;
            var sendButtons = $("#tabs-RatingPlansList button");
            if (sendButtons.length < 2) {
                return;
            }

            var sendButton = $(sendButtons.get(1));
            self._disableSubmitButton(sendButton);

            $('input#number:visible').on('keyup', function (e) {
    ...
    $.widget.bridge("customTarificator", $.custom.Tarificator);
